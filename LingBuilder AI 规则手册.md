...
- **无头浏览器三形态选型（2026-09-19 落地，AI 必读）**：用户要「无头/后台浏览器」时按需求分流，禁止混用或声称 WebView2 有官方无头 API（`CreateCoreWebView2Controller` 必须挂 HWND，release SDK 到 1.0.4191.47 无任何 Headless 接口）。① **EdgeView 无头实例**（隐窗伪无头）：`EdgeView_创建无头实例(实例编号, 地址, 独立缓存目录)` / `EdgeView_创建无头实例代理(实例编号, 地址, 独立缓存目录, 用户代理, 代理地址)`——内部挂从不调用 ShowWindow 的离屏 WS_POPUP 工具窗口，任务栏与界面零显示，289+ 条实例编号命令（导航实例/执行JS实例/绑定事件/等待事件/会话 Cookie 族/UA/独立代理）照常可用；rAF/IntersectionObserver 等按隐藏页节流；**截图/PDF 类命令在该形态不可用**（宿主不渲染像素），需要页面图像必须改走 ②。设计器非可视组件「EdgeView无头浏览器」（属性：实例编号/地址/缓存目录/UA/代理/自动启动）在窗口创建期自动建立实例；控制台项目不实例化设计器资源，改在“启动”里调代码命令。② **CDP 真无头**（独立进程，零窗口，截图/PDF/网络拦截全真）：`CDP_启动浏览器(参数JSON, &就绪处理器)` 自动定位本机 msedge/chrome 并以 `--headless=new --remote-debugging-port=0 --user-data-dir=独立临时目录` 启动、解析 `DevToolsActivePort` 后自动连接返回受管 `CDP连接`；参数 JSON 字段 `可执行文件/用户数据目录/额外参数/起始地址/等待超时毫秒` 全可缺省；同步失败（返回 0）立即 `CDP_取最后错误()` 读中文原因；`CDP_停止浏览器(连接)` 先 `Browser.close` 优雅退出再超时强杀并清理目录，`CDP_停止全部浏览器()` 批量回收；进程退出自动回收，禁止生成不管停止的裸启动。独立浏览器进程与 WebView2 实例 Profile/Cookie 互不相通，代理用 `额外参数:"--proxy-server=..."`、UA 用 `CDP_设置UserAgent`。③ **FBro 后台浏览器**（CEF 进程级 headless，既有能力）：`FBro_启用无头模式()` + `FBro_后台创建(地址, 缓存目录, 附加信息JSON)`，与可见进程内 FBroBrowser 控件进程级互斥。**控制台/无界面程序通用底座**：控制台入口自动创建无头泵窗口（hwnd_ 指向离屏工具窗口），CDP/HTTP/WS/线程完成处理器与 `EdgeView_泵消息(等待毫秒)`（0=只清队列）配合即可派发；AI 生成控制台异步链时用「发起 → 泵消息/等待事件轮询」，不得照搬窗口项目的纯事件驱动写法。弹窗闪窗治理：需要建窗但暂不显示用 `EdgeView_创建弹窗浏览器初始隐藏(代理)`，唤出走 `EdgeView_置实例可见`。改这些命令语义必须同步本节、`electron/docs/modules/cdp-client/README.md`、`electron/docs/modules/edgeview/API.md`（`npm run module:edgeview-api-docs` 重生成）、`electron/tests/edgeViewHeadless.test.ts` 与当天更新记录。
