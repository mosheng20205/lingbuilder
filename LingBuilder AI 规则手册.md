...
- 当前项目一旦启用 `lingbuilder.new_emoji.ui`，设计器中的按钮、编辑框、文本、复选框、单选框、列表框、图片框、进度条和容器会直接生成真实 new_emoji 控件；AI 不应再额外创建第二个 new_emoji 窗口，也不应把这些控件改写成手工 Win32 API。列表框项目与图片源/填充方式必须从设计器属性确定性生成。未适配的高级控件应保留模型并报告中文诊断，不能伪造为已支持。
- 常见类型：`整数型`、`文本型`、`逻辑型`、`空`
...
- 系统通用对话框使用高级控件模块已绑定的 `打开文件`、`保存文件`、`选择文件夹`、`选择颜色`、`选择字体`、`查找文本`、`替换文本`、`打印`、`页面设置`、`任务对话框`，不得让 AI 拼接任意 shell 或隐藏宿主调用。
