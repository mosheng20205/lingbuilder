...
- **windows-dll 模板项目创建即带 `outputType: "dll"`（2026-09-22 起）**：模板创建的解决方案项目在 buildProperties 写入 dll 输出类型，IDE 工具栏据此隐藏运行入口并显示「生成动态库」。AI Bridge `project.create` 同日新增 `outputType` 参数（MCP_INSTRUCTIONS 第 18 条同步）：窗口模板（blank-window / hello-window / sqlite-crud-window）传 `"dll"` 直接创建「公开子程序导出为 DLL 接口」的项目；控制台模板与 new_emoji 模板传 dll、windows-dll 模板传 exe 都会在创建前被中文报错拒绝。outputType 是解决方案项目属性，创建后改回 exe 由用户在 IDE「构建属性」对话框操作，外部 AI 无对应工具。
- **项目 DLL 命令的架构声明必须是双架构（2026-09-23 真机实测）**：`DLL命令库` 里同时写 `Win32 = "dll/Win32/xxx.dll"` 与 `x64 = "dll/x64/xxx.dll"`（路径相对调用方项目 sourceRoot，构建时自动复制目标架构 DLL 到 exe 旁）。原因：AI Bridge / CLI 的 build.run 构建目标架构跟随**本机编译器**（x64 宿主的 cl → x64 目标），IDE 内 F5 才跟随工作区构建配置；只声明 Win32 会在 x64 编译环境被中文阻断「未声明 x64 架构的 DLL 文件」。调用方项目若默认启用 process-memory 等 x64 模块也会被自动切到 x64。实测样例：DLL 工作区里建控制台调用方，声明 `整数型 LingBuilder_GetApiVersion()`，启动里 `调试输出(到文本(LingBuilder_GetApiVersion()))`，构建运行输出 `[调试输出] 1`。
- **MSBuild 定位与缺失诊断（2026-09-22 修复）**：DLL 工程与外部 MSBuild/CMake 工程的构建经 `resolveMsBuildCommandAsync` 定位 MSBuild——LINGBUILDER_MSBUILD_PATH 环境变量优先，其次 vswhere 探测最新 Visual Studio（内部版本号不限，VS2026 目录名是 18 而非 2026），再次固定路径扫描（18/2022/2019），最后回落 PATH；全部落空时 spawn ENOENT 会输出中文修复指引（安装 Build Tools C++ 工作负载或配置 LINGBUILDER_MSBUILD_PATH），外部 AI 看到该指引应转告用户安装或配置，不要反复重试构建。
...
- Cookie 能力分两层，AI 不得混用：`HTTP客户端_设置Cookie(客户端, 启用)` 只是 WinHTTP 自动罐（接收 `Set-Cookie` 并自动回送）的开关；把外部取得的已有 Cookie 发出去必须用 2.1 的注入命令——按请求整体注入用 `HTTP客户端_请求置Cookie(请求, Cookie串)`，按域/路径长期回送用 `HTTP客户端_置Cookie(客户端, 名称, 值, 域, 路径)`。禁止再生成"用 `设置请求头` 提交 Cookie"的写法（该头是受管头会被拒绝）；读取手工注入项用 `HTTP客户端_取CookieJSON`，清空用 `HTTP客户端_删除全部Cookie`。任一注入生效时该请求（含重定向）自动关闭罐回送，不得再叠加第二份 Cookie。
...
- `CEF3_取资源地址(相对路径)` 是 `.lcpp` 加载本地测试页与本地资源的**唯一可移植入口**：把相对路径解析为随程序一起部署在 exe 同级 `assets/` 目录下的文件，返回已按 UTF-8 百分号转义的 `file:///` 地址；传入盘符绝对路径、UNC 路径或已带协议的完整地址时原样返回，因此可对同一结果重复套用；它不依赖浏览器控件实例，可在控件创建前调用。**2026-09-19 起生成器对 wideString 形参的调用形态实参统一经 `LingCppWideArg` 归一**，`CEF3_导航(浏览器1, CEF3_取资源地址("index.html"))` 这类内联嵌套可直接生成可编译 C++；先落文本型局部变量再传参的写法仍然合法，示例与模板可继续采用。「先声明后赋值」的提升时序红线（见功能库节）不受此影响。**AI 生成示例、项目模板、教程代码或修复建议时同样禁止写死 `file:///` 绝对路径、工作区路径或任何机器特定目录**——那会把本机路径烧进随仓库分发的示例，违反「源码可复制给别人使用」；需要加载本地页面时一律改用本命令，且只引用确实随 `assets/` 一起部署的文件。
...
- **JS 交互（cefQuery）通道（2026-09-12 补）**：页面调用原生使用 CEF 官方 MessageRouter（cefQuery 风格）。**注册时机是硬约束**：查询通道必须在 `LB_CEF3_Initialize` 之前注册（渲染进程在 `OnWebKitInitialized` 时按 `CefMessageRouterConfig` 把查询函数注入 `window`，初始化完成后再注册的通道对新浏览器不生效），因此标准用法是 **CEF3 浏览器控件属性 `jsQueryFunctions`**（格式 `查询函数名,取消函数名`，如 `cefQuery,cefQueryCancel`，多条通道之间用 `;` 分隔，留空不启用），生成器把它烘焙进 `CEF3_初始化` 并在 `LB_CEF3_Initialize` 前逐条发 `LB_CEF3_EnableJsQuery`；设计器属性面板对 CEF3Browser 提供与 FBro 同一个多通道结构化弹窗（`FbroJsQueryEditorDialog` mode=cef3：可增删/排序多条通道，实时预览逐条烘焙 `LB_CEF3_EnableJsQuery` 与页面调用写法，取消函数名留空时预览按桥默认 `cefQueryCancel` 展示），保存回写控件属性字符串，项目保存时才落盘 `window-designer.json`（属性编辑只更新内存+localStorage）。`.lcpp` 命令 `CEF3_启用JS扩展(控件名, 查询函数名, 取消函数名)` 只在初始化前调用有效，在「创建完毕」等事件里调用必然失败（返回 0）——AI 不得生成这种调用，也不得宣称运行期可注册通道。CEF3 与 FBro 一样支持**多条**查询通道（每进程上限 16 条，各通道的查询函数名互不重复；同名重复注册幂等成功，查询名/取消名冲突或非法标识符注册失败），每条通道对应页面上一对独立的 `window` 函数，「查询请求」事件的 `channelIndex` 字段（0 起）标明来源通道；同一通道内仍建议用 request 载荷自带业务标记。应答链：`CEF3_绑定事件(浏览器1, "查询请求", &处理器)` → 处理器内 `CEF3_取事件字段(浏览器1, "queryId")` 取数字文本 → `CEF3_查询应答(浏览器1, 查询ID, 结果文本)` 或 `CEF3_查询应答失败(浏览器1, 查询ID, 错误码, 错误文本)`；每条查询只能应答一次，未应答的查询 120 秒后由桥对页面自动回错误码 -4（页面 `onFailure` 收到）。「查询已取消」（OnQueryCanceled，页面取消/导航离开/浏览器关闭/渲染进程终止）会通知 `.lcpp`，收到后无需再应答；该查询已被应答或已清理时桥**不派发**此事件（避免把宿主引到另一条挂着的外部查询上）；CEF 150 中页面主动 `cefQueryCancel` 只触发宿主侧取消通知，不再回调页面自身的 `onCanceled`，这是 CEF 内建语义，AI 不得虚构页面端取消回调行为。事件字段有 `queryId`（由桥全局递增分配的数字文本，跨通道不重号，应答时原样传回）、`channelIndex`（来源通道序号，**不是通道名**）、`request`、`persistent`。处理器运行在宿主事件链路上，应答可在处理器内同步完成，也可延后（例如等 `CEF3_读资源响应正文` 完成后再应答）。

- **跨域/同源策略放开三内核选型（2026-09-19/20 落地，AI 必读）**：用户要「跨域取 iframe / 放开同源限制」时按内核分流，禁止把某一内核的做法当成三内核通用。① **CEF3**：CEF 官方跨域白名单三条命令 `CEF3平台_添加跨域白名单(源站点, 目标协议, 目标域名, 允许目标子域)` / `CEF3平台_删除跨域白名单(...)` / `CEF3平台_清空跨域白名单()`——按**精确源→目标**逐条放行（advanced），不得生成「全局关闭安全策略」的假命令；跨帧取元素/填表走 `CEF3框架_*` + `CEF3填表_*`（按帧句柄）。② **EdgeView（WebView2）**：没有跨域白名单 API；用创建期属性「关闭跨域安全策略」（`disableWebSecurity`）或 `EdgeView创建选项_置附加参数` 写 Chromium 开关（`--disable-web-security --disable-features=IsolateOrigins,site-per-process`），实现在 `CreateCoreWebView2EnvironmentWithOptions` **之前** `put_AdditionalBrowserArguments` + 环境变量 `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 双通道（前者会被 release SDK 过滤掉安全类开关，环境变量才真正生效，二者都保留）。③ **FBro**：`FBro_设置启动开关JSON("{\"enableCrossFrame\":true}")` 或控件属性「启用跨域模式」，桥调官方 `FBroHsCommandLine_EnableCrossFrame`，真机回读确认实际写入 `--disable-web-security --disable-site-isolation-trials`；跨域读回必须靠 `FBro框架_*`/`FBro填表_*`。**三内核共同红线**：开关都是**进程级/创建期**，只在初始化前生效，运行期切换一律无效（FBro 的桥导出在初始化后直接返回失败）；禁止为了跨域去注入 JS、开 `--no-sandbox` 之类不受控开关，也禁止向用户宣称「按域名精细放开」在 EdgeView/FBro 上可用（只有 CEF3 是逐条白名单）。放开跨域后同进程**所有**实例都受影响，文档与 AI 回复必须提示只在受控内网/自有页面使用。真机验收：`cd electron && npm run smoke:fbro-startup-switches`（跨源 iframe 读回子页标题，正反对照逐键归因）。

- CEF3 全局代理与实例代理（2026-09-20 补齐，与 EdgeView 同口径）：`CEF3_设置全局代理(代理地址)` 只影响之后新建实例（控件/弹窗/区域/无头创建时未自带代理即回落），`CEF3_清除全局代理()` 与空文本设置等价，`CEF3_取全局代理()` 与 `CEF3_取实例代理(实例编号)` 只做回读；实例编号段覆盖弹窗/区域（1000000+）与无头（2000000+）实例，设计器控件用 `CEF3会话_取代理(控件名)`。代理地址一律 `scheme://host:port`，**禁止把用户名密码拼进地址**；带认证的代理当前不被 CEF3 支持（真机证伪两条官方通道，见「外部 AI 浏览器代理 IP 三内核选型」条），AI 不得生成 `CEF3_设置代理认证` 等不存在命令，需要认证时改推 FBro（`FBro会话_设置代理认证`）或 EdgeView。
...
- 事件处理器名称要与窗口设计器绑定保持一致，例如 `按钮1_被单击`。**常规命名的控件事件无需设计器绑定也能触发（2026-09-21 起）**：源码里按 `_控件名_事件中文后缀` 命名的事件处理器（如 `_按钮1_被单击`、`_列表框1_选择项被改变`、`_编辑框1_内容被改变`），在该控件对应事件未被设计器绑定时，生成器会自动把常规命名处理器并入该控件的事件派发表（命名规则与设计器事件面板自动生成的处理器名一致，控件名中的空白去除后拼接）；设计器显式绑定仍优先且不产生重复派发。此前「只在源码里写事件、设计器没绑定就静默不触发」的坑已结构性根治，配套的「未绑定 Click」启动警告已移除。AI 生成事件代码仍建议同时在设计器绑定（语义显式、可在设计器里看到），但不得再宣称「不绑定就不生效」。
- **`控件_设置文本` 对全部控件即时生效（2026-09-21 起）**：自绘 BUTTON 家族（`Button`、`CheckBox`、`RadioButton`）的可见标题由运行时自绘（`BS_OWNERDRAW`），此前 `控件_设置文本` 只改 HWND 窗口文本、画面仍画设计器固化文本，表现为「命令返回真但界面不变」（`控件_取文本` 能取到新值，进一步掩盖问题）。现在运行时在 `WM_SETTEXT` 把新文本镜像进覆盖文本并强制重绘，与 GroupBox 标题同一机制。`控件_设置文本` 对按钮/复选/单选/分组框/标签/编辑框全部即时可见；TextBox 附加行为（多行自动滚动到底）保持不变。
...
- 返回语句统一写括号形态：带返回值一律写 `返回(表达式)`（如 `返回(合计)`、`返回(数值 * 数值工具.阶乘(数值 - 1))`），不带值提前结束写 `返回()`。不带括号的 `返回 表达式` 当前也合法且 C++ 生成结果相同，但官方示例、教程与演示项目已统一为括号形态；AI 生成或修改 `.lcpp` 时一律写括号形态，保持与示例风格一致（2026-09-20 定为统一写法口径）。
...
- 控制流：条件使用 `如果` / `如果真`、`否则如果`、`否则`、`如果结束` / `如果真结束`；多路选择使用 `选择`、`分支`、`默认`、`选择结束`；循环使用 `循环`、`判断循环首/判断循环尾`、`循环判断首/循环判断尾`、`计次循环首/计次循环尾`、`变量循环首/变量循环尾`、`枚举循环首/枚举循环尾`，循环内可用 `跳出循环`、`到循环尾` / `继续循环`；异常处理使用 `尝试`、`捕获`、`最终`、`尝试结束`、`抛出`；子程序提前结束继续使用 `返回`（带值统一写 `返回(表达式)` 括号形态，无值写 `返回()`，见基本规则节）。这些命令均由本地规则确定性生成 C++，新手编辑器会显示与 `如果/否则` 相同的括号流程控制线。AI 不得用原生 C++ `if` / `for` / `try`、`L""` 或删除结构结束标记来规避类型问题。
...
- 文本转其它数值类型使用基础命令族，参数口径与 `到整数` 完全一致：都只接收单个 `文本` 参数（`wideString`），本期不提供数值重载。`到长整数(文本)` 返回 64 位长整数，空文本或无法转换返回 0，不做 32 位截断，适合时间戳、文件大小、地址等大数值；`到小数(文本)` 返回双精度小数、`到单精度小数(文本)` 返回单精度小数，两者忽略首尾空白、无法转换返回 0；`到逻辑(文本)` 返回逻辑值，空文本、“0”、“假” 为假，“真” 与非零数值为真，其余非空文本（如“是”“yes”）也为真。需要把数值实参交给这四条命令时必须显式写 `到文本(数值)`（与 `到整数` 一样不会自动补转换），需要把 `进程_按名称取ID列表` 之类的文本结果转回整数时用 `到长整数`，不要用 `到整数` 截断大数值。四条命令与 `到整数` 同属 `lingbuilder.win32.basic`，普通 Win32 与 new_emoji 两后端都必须通过该 binding 生成真实 C++ 调用，不得只在编辑器里高亮或让 AI 自行模拟转换。
...
- 等待与定时（2026-09-21 起，win32 基础模块）：在事件处理器里需要「等一会儿再做事」时，禁止用 `线程_协作等待` / `线程_等待`（多线程模块命令，在界面线程调用会冻结窗口），必须用三种不冻结界面的方式——① `延时(等待毫秒)`：同步等待但等待期间持续泵送界面消息（易语言 `延时`/`程序_延时` 同款语义），窗口不冻结、控件仍可点击，等待期间触发的事件处理器先执行再回来继续下一行；② `延迟调用(等待毫秒, &处理器)`：一次性延迟调用，到期后在界面线程调用 `&处理器`（无参子程序，`&` 引用语法），只触发一次，窗口销毁后不再触发；③ 设计器「时钟」非可视组件 + `时钟_启动` / `时钟_停止` / `时钟_置周期` / `时钟_取周期` / `时钟_是否已启动` 命令族与「周期到期」事件（`时钟_置周期(组件, 0)` 表示停止计时，与易语言时钟周期语义一致），组件属性可设周期毫秒与「窗口创建后自动计时」，适合周期性任务。时钟组件只在窗口项目计时；控制台项目请在「启动」里配合 `EdgeView_泵消息` 使用 `延迟调用`，或改用多线程模块命令。
- 定时任务（2026-09-21 起，`lingbuilder.cron` 定时任务模块）：按日历时刻或表达式调度（每 5 分钟、每天 02:00、工作日 9 点、秒级等）一律用 `cron_定时_` 命令族，不要用「时钟」组件拼日历逻辑（时钟是固定毫秒周期）。三条路径按需选：① `cron_定时_启动(表达式, &处理器)`——到点在注册窗口 UI 线程执行，处理器签名 `空 处理器(定时任务 任务)`，可直接更新控件，注册窗口关闭时任务自动停止；② `cron_定时_提交线程(表达式, &工作处理器, &完成处理器)`——工作处理器在 cron 后台线程执行，禁止调用任何 UI/controlRef 命令（语言服务会报错阻断），界面更新放完成处理器；③ crontab 表命令型——`cron_定时_表添加(表达式, 命令行)` 写表、`cron_定时_表载入运行()` 注册为命令任务、`cron_定时_守护启动()` 自动载表并开启 0.5 秒守护线程；同一张系统表全机只允许一个进程持有守护（跨进程命名互斥体保证，重复启动返回假并记入 `cron_定时_取错误()`）。表达式支持五段「分 时 日 月 周」、六段秒级（首段为秒）、`*/n`、`a-b/n`、逗号列表与 `@yearly/@monthly/@weekly/@daily/@hourly/@reboot` 简写（月/周可用 JAN~DEC、SUN~SAT 英文名）；日与周同时受限时命中其一即触发（标准 cron 语义）；写错用 `cron_定时_取表达式错误` 查中文原因，`cron_定时_表达式说明` 生成人话。系统级 crontab 表位于 `%APPDATA%\LingBuilder\cron\crontab.txt`（UTF-8，`#` 注释，`MAILTO=` 行指定通知邮箱，表命令可传路径参数改用项目私有表）。常驻等价 crontab 守护：`cron_定时_开机自启(真)` 写当前用户 Run 键（不需要管理员），登录后系统以 `--lingbuilder-cron-daemon` 参数隐藏拉起本程序进入守护模式（窗口不显示、`cron_定时_守护启动()` 自动执行）。MAILTO 邮件通知必须先 `cron_定时_邮件配置(SMTP服务器, 端口, 账号, 授权码, 收件地址)`——Windows 没有本机邮件代理，必须配真实 SMTP 账号（如邮箱授权码）；命令型任务退出码非 0 或有输出时发送通知邮件（含命令行、退出码与输出截断），`cron_定时_邮件测试` 验证配置。命令任务经 `cmd.exe /c` 执行、输出被管道捕获，运行在后台线程，不得在命令行里依赖当前工作目录（先 `cd /d` 或用绝对路径）。
...
- 2026-09-16 易语言迁移批次其余补齐：`lingbuilder.std.text@1.1.0`（当时共 24 条）新增 `文本_取左边/取右边(文本, 长度)`、`文本_码点转字符(码点 0～0x10FFFF，代理区与超界返回空，>0xFFFF 自动代理对)`、`文本_取码点(文本, [位置=0]，0 基、越界 0、代理对合成完整码点)`、`文本_删首空白/删尾空白`；`lingbuilder.std.math@1.1.0`（共 18 条）新增 `数学_取整(floor，-7.8=-8)/绝对取整(trunc，-7.8=-7)/四舍五入(数值,[位置=0]，四舍五入远离零)/取符号/正弦/余弦/正切/反正切(弧度)/自然对数(≤0 返回 0)/反对数(e^x，溢出 0)/置随机种子([种子=-1 取系统时钟]，与 数学_随机整数 共用线程本地引擎)`；`lingbuilder.std.bytes@1.1.0`（共 25 条）新增 `字节集_从文本(UTF-8)/字节集_重复(次数, 数据，结果超 256MiB 返回空)/字节集_分割(数据, 分隔, 结果数组, [数目=0 全部]，分隔为空按单字节 0，调用方声明 局部 字节集 分段[])` 与进制转换 `数值_到十六进制文本(负数输出 8 位 32 位补码)/到八进制文本/十六进制解析(容忍 0x 前缀)/八进制解析`（解析失败返回 0，超 8 位取低 32 位按补码）。
- 2026-09-23 零内嵌 C++ 能力批次：`lingbuilder.std.text@1.2.0`（共 29 条）新增定宽填充族与数组合并——`文本_填充右边(文本, 目标显示宽度, [填充字符 = " "])`（左对齐、右侧补齐）、`文本_填充左边(...)`（右对齐）、`文本_居中(...)`（左侧取剩余列数的一半向下取整）、`文本_取显示宽度(文本, [中日韩按 2 列 = 真])`、`文本_连接(文本数组, 分隔符, [起始下标 = 0], [数量 = -1 表示连到末尾])`（`文本_分割` 的反向操作；文本元素按值连接，数值/逻辑元素按 `到文本` 规则转换），`文本_分割` 追加可选第 4 参 `忽略末尾空段`（默认 假＝保留尾空段，传 真 只丢弃末尾连续空段，中间空段保留）。红线：定宽族的「目标显示宽度」是显示列数不是字符数（中日韩全角 2 列、组合符号 0 列、控制字符 0 列、其余 1 列），中英文混排表格必须用 `文本_取显示宽度` 算宽度，不得用 `文本_取长度` 冒充；文本已够宽时原样返回、不截断。`lingbuilder.fs.core@1.2.0`（共 41 条）新增文件时间族——`文件_取修改时间(路径)` / `文件_取创建时间(路径)` / `文件_取访问时间(路径)`返回名义类型 `日期时间`（与 `lingbuilder.std.datetime` 同一 64 位打包表示，可直接与 `时间_取间隔` / `时间_增减` / `时间_取年份` 配合比较）、`文件_取最新文件(目录, 通配符, 结果数组)`（只写入 1 条修改时间最新的完整路径，返回 1 成功 / 0 失败），失败返回 0 并用 `文件_取错误()` 读中文原因（成功会清空）。红线：目录不存在、无匹配、读不到时间都会给出中文原因，禁止用内联 C++ 的 GetFileAttributesExW / FindFirstFileW 替代；`文件_取最新文件` 只枚举本层目录、修改时间并列时保留先枚举到的那个。配套生成端修复：`到文本` 的文本结果类型收敛为 `std::wstring`（不再是带隐式 const wchar_t* 转换的派生类型），`到文本(值) + 文本变量`、`到文本(a) + 到文本(b)` 等写法不再触发 C2666 重载二义；`数组_取成员` 取文本成员返回 `std::wstring` 值（不再返回指向轮转缓冲区的裸指针），文本数组成员可直接参与拼接与比较；`控件_设置文本` 对多行 TextBox / RichEdit 自动把裸 LF 补成 CRLF（源码里直接写 `"
"` 即可）。功能库需要跨调用缓存（例如只枚举一次磁盘）时用「项目全局变量.lcpp」的项目全局变量（`全局 文本型 名称 = 初值`，漏写「全局」关键字会被中文诊断拦住），不要为此把状态塞进内联 C++。功能库内仍不能把窗口事件处理器作为 `&处理器` 参数传入（首版限制：处理器名字符串由窗口类的分派表解析，功能库方法不在表内），异步回调请在窗口事件中绑定、再调用功能库的普通功能。
...
- 2026-09-23 实参类型门禁与十六进制命令改名：`lingbuilder.std.bytes@1.2.0`（共 26 条）把 `字节集_十六进制编码` 改名为 **`字节集_到十六进制字节集`**（旧名保留为别名，既有工程照常编译，生成 C++ 恒用新名），并新增 **`字节集_到十六进制文本(数据)`**（返回文本型，一步拿到可显示/拼接的大写十六进制文本）。红线：`字节集_到十六进制字节集` 返回的是**字节集**而不是文本——旧名里的「编码」曾让 AI 误把返回值当文本传给 `文本_取左边` 等文本形参；要文本就用 `字节集_到十六进制文本`，或对字节集结果再调 `编码_字节集转文本(结果, "ANSI")`。配套生成链新增**模块命令实参类型门禁**：实参推导类型与 binding 形参声明类型都已知且不相容（典型：把字节集传给文本型形参）时，`build.run`/`native.preview`/`native.export` 在生成前直接中文阻断（诊断含源码行号列号、命令名、第几个实参、形参要求、实参实际类型与转换建议），编辑器语义诊断同步报同一问题（id 前缀 `lingcpp-argument-type-`），生成器也不再对已知非文本返回值生成 `LingCppWideArg(...)` 必坏包装——MSVC 的 C2665/C2660 错误恢复级联（「函数不接受 1 个参数」之类）不允许再外泄给用户。AI 收到该诊断后按建议先转换实参类型，禁止改用 `raw` 参数或内联 C++ 绕过。
...
- **EdgeView 构建缺 WebView2 SDK 会中文阻断，桩报错与「Runtime 版本不足」已区分（2026-09-23）**：编译期 SDK（Microsoft.Web.WebView2 1.0.4078.44 的 WebView2.h + WebView2LoaderStatic.lib）已随 IDE 安装包分发，运行期按 `LINGBUILDER_WEBVIEW2_SDK_ROOT` → 随包副本 → NuGet 全局缓存顺序解析；缺失时构建直接被「EdgeView 模块缺少固定版本 Microsoft.Web.WebView2 1.0.4078.44…」阻断，不会再静默生成 EdgeView 空实现程序。运行期诊断两分法：见「**EdgeView 命令未编译进本程序**…构建时缺少 WebView2 SDK…与已安装的 WebView2 Runtime 版本无关，安装 Runtime 无法解决」= 程序是在缺 SDK 环境下编译的旧产物，应恢复 SDK 后重新构建，**不要建议安装 Runtime**；见「EdgeView 命令不可用…当前 WebView2 Runtime 版本不足；v1 最低需要 141，v2 完整能力需要 150」= 真实运行时接口缺失，才建议装 Runtime 150+。环境检测的「WebView2 SDK（EdgeView 构建）」项只查编译期 SDK，与「WebView2 Runtime」项（注册表查浏览器运行时）相互独立，两者都 ✓ 才能完成 EdgeView 构建。
...
- **无头浏览器四形态选型（2026-09-19 落地，2026-09-20 增 CEF3 OSR 第四形态，AI 必读）**：用户要「无头/后台浏览器」时按需求分流，禁止混用或声称 WebView2 有官方无头 API（`CreateCoreWebView2Controller` 必须挂 HWND，release SDK 到 1.0.4191.47 无任何 Headless 接口）。① **EdgeView 无头实例**（隐窗伪无头）：`EdgeView_创建无头实例(实例编号, 地址, 独立缓存目录)` / `EdgeView_创建无头实例代理(实例编号, 地址, 独立缓存目录, 用户代理, 代理地址)`——内部挂从不调用 ShowWindow 的离屏 WS_POPUP 工具窗口，任务栏与界面零显示，289+ 条实例编号命令（导航实例/执行JS实例/绑定事件/等待事件/会话 Cookie 族/UA/独立代理）照常可用；rAF/IntersectionObserver 等按隐藏页节流；**截图/PDF 类命令在该形态不可用**（宿主不渲染像素），需要页面图像必须改走 ②。设计器非可视组件「EdgeView无头浏览器」（属性：实例编号/地址/缓存目录/UA/代理/自动启动）在窗口创建期自动建立实例；控制台项目不实例化设计器资源，改在“启动”里调代码命令。② **CDP 真无头**（独立进程，零窗口，截图/PDF/网络拦截全真）：`CDP_启动浏览器(参数JSON, &就绪处理器)` 自动定位本机 msedge/chrome 并以 `--headless=new --remote-debugging-port=0 --user-data-dir=独立临时目录` 启动、解析 `DevToolsActivePort` 后自动连接返回受管 `CDP连接`；参数 JSON 字段 `可执行文件/用户数据目录/额外参数/起始地址/等待超时毫秒` 全可缺省；同步失败（返回 0）立即 `CDP_取最后错误()` 读中文原因；`CDP_停止浏览器(连接)` 先 `Browser.close` 优雅退出再超时强杀并清理目录，`CDP_停止全部浏览器()` 批量回收；进程退出自动回收，禁止生成不管停止的裸启动。独立浏览器进程与 WebView2 实例 Profile/Cookie 互不相通，代理用 `额外参数:"--proxy-server=..."`、UA 用 `CDP_设置UserAgent`。③ **FBro 后台浏览器**（CEF 进程级 headless，既有能力）：`FBro_启用无头模式()` + `FBro_后台创建(地址, 缓存目录, 附加信息JSON)`，与可见进程内 FBroBrowser 控件进程级互斥。**控制台/无界面程序通用底座**：控制台入口自动创建无头泵窗口（hwnd_ 指向离屏工具窗口），CDP/HTTP/WS/线程完成处理器与 `EdgeView_泵消息(等待毫秒)`（0=只清队列）配合即可派发；AI 生成控制台异步链时用「发起 → 泵消息/等待事件轮询」，不得照搬窗口项目的纯事件驱动写法。弹窗闪窗治理：需要建窗但暂不显示用 `EdgeView_创建弹窗浏览器初始隐藏(代理)`，唤出走 `EdgeView_置实例可见`。④ **CEF3 无头（官方 OSR，2026-09-20）**：真无头且不创建任何窗口，创建签名、取内容两条路、句柄/等待/截图三条红线与事件轮询口径统一见「9. AI Bridge 外部客户端规则」节「CEF3 无头浏览器（OSR）」条；命令面 = `CEF3_创建无头浏览器` / `CEF3无头_*`（`lingbuilder.cef3.browser`）+ `CEF3OSR_*`/`CEF3离屏_*`（`lingbuilder.cef3.osr`，按浏览器句柄寻址）。改这些命令语义必须同步本节、`electron/docs/modules/cdp-client/README.md`、`electron/docs/modules/edgeview/API.md`（`npm run module:edgeview-api-docs` 重生成）、`electron/tests/edgeViewHeadless.test.ts` 与当天更新记录。
...
- **EdgeView 区域浏览器窗口缩放（1.6.0，2026-09-24 落地，AI 必读）**：`EdgeView_创建区域` 的承载区域只在**创建时位于 (0,0) 且铺满整个窗口**时才随窗口缩放自动拉伸；给浏览器留导航条/边距（如顶部 54 像素工具条）时，必须在「大小被改变」事件里调用 `EdgeView_置区域位置(实例编号, 左, 顶, 宽, 高)`（窗口内逻辑坐标、按 DPI 自动缩放，成功返回 1）同步区域几何，拖拽缩放才跟手、不会白屏——旧版每次 `WM_SIZE` 无条件把网页对齐到承载区域客户区、窗口放大时页面按超大宽度排版造成「白屏」，已修（`EdgeView_调整全部大小` 现按创建时布局过滤，弹窗实例本就不参与）。坐标口径：普通 Win32 窗口项目 `窗口_取事件宽度()/窗口_取事件高度()` 返回设计器逻辑坐标（DIP，已按窗口 DPI 还原），可直接作为 `EdgeView_置区域位置` 等逻辑坐标命令的实参（new_emoji 后端返回物理像素，需按 `窗口_取事件DPI` 自行换算）。标准布局模板：`EdgeView_创建区域(1, 0, 54, 客户宽, 客户高 - 54, 地址, 缓存目录)` + 「大小被改变」里 `EdgeView_置区域位置(1, 0, 54, 窗口_取事件宽度(), 窗口_取事件高度() - 54)`。配套窗口事件接线（Win32 后端 2026-09-24 起）：源码按 `_窗口类名_事件中文后缀` 常规命名（如 `事件 _BrowserWindow_大小被改变()`）的窗口级事件自动接线，无需在设计器窗口事件表登记；裸名（`事件 大小被改变()`）不自动接线（多窗口会互相覆盖）；按键类事件按注册表形参声明即可真实收到键码等实参。弹窗实例的顶层窗口请用 `EdgeView_置实例大小`，`EdgeView_置区域位置` 对弹窗返回 0 并给中文诊断。改这些语义必须同步 `electron/src/services/aiBridge/mcpServer.ts` 第 3 条（`tests/aiBridge.test.ts` 关键词断言 + 重建 `dist/cli.cjs`）、`electron/docs/modules/edgeview/API.md`（`npm run module:edgeview-api-docs` 重生成）与 `electron/tests/modules.test.ts`「EdgeView 1.4.0 多店铺弹窗」用例。
- **CEF3 / FBro 多店铺能力对齐（2026-09-19，与 EdgeView 同口径）**：CEF3 新增**设计器无关**的 `CEF3_创建弹窗浏览器(实例编号, 地址, 独立缓存目录, 代理地址)`（走已出货 `LB_CEF3_BrowserCreateChrome` Chrome Runtime 建独立顶层窗，不同 `独立缓存目录`=独立 profile、代理非空=独立出口 IP）与 `CEF3_创建区域(实例编号, 左,顶,宽,高, 地址, 独立缓存目录, 代理地址)`（运行时自建 WS_CHILD 承载内嵌多实例）；实例登记进 `cefBrowsers_`（合成 controlId=1000000+实例编号，与设计器控件空间隔离），`CEF3_枚举实例JSON()` 与 `CEF3_关闭全部实例()`（公开化）自动覆盖。CEF3 内嵌固定数量多缓存（每控件独立 RequestContext）本已具备；**CEF3 per-browser User-Agent（实例级 UA）已落地（纯运行时，无需重编桥）**：新增 `CEF3_设置用户代理(控件名, 用户代理)`（设计器控件）与 `CEF3_设置实例用户代理(实例编号, 用户代理)`（弹窗/区域），置非空 UA 时点亮桥 `LB_CEF3_ResourceRequestHandlerSubscribeBeforeResourceLoad` 订阅位，运行时在「资源加载前」事件里对 `packet->subject`（桥 `RegisterRequest` 出的活请求句柄）调 `LB_CEF3_RequestSetHeaderByName(..,"User-Agent",..,1)` 逐实例改写请求头，置空清除覆盖；`CEF3_取用户代理`/`CEF3_取实例用户代理` 回读。CEF 本身无 per-browser settings，请求头改写即官方范式，AI 需要多店铺各异 UA 时用这些命令（在首次导航前设置）。FBro 侧：`浏览器外壳_设置实例Cookie(稳定ID,地址,Cookie文本)` 现透传 `HttpOnly/Secure/Domain/Path`（host 本就支持），新增 `浏览器外壳_新建独立实例代理(稳定ID, 地址, 标题, 缓存目录, 代理地址, 用户代理)` 提供每店铺独立代理/UA（独立进程天然隔离出口 IP）；**内嵌区域动态**（1.4.0）新增 `浏览器外壳_新建内嵌实例区域(稳定ID, 左, 顶, 宽, 高, 地址, 缓存目录, 代理地址, 用户代理)` 与 `浏览器外壳_取内嵌区域实例JSON()`——复用已出货 `LING_FBRO_PROCESS_EMBEDDED` 跨进程内嵌路径（不改桥），在宿主窗口指定矩形新建 WS_CHILD 承载、把独立进程浏览器作为其子窗挂入，可同时内嵌多个各自独立 profile/代理/UA、随主窗口缩放/DPI 跟随各自矩形，与 EdgeView_创建区域 / CEF3_创建区域 同口径（区域实例以稳定ID 供 导航实例/设置实例Cookie/取实例状态/关闭实例 按 ID 操作，不进多标签视口状态机）；动态多实例/枚举/关闭全部复用既有 `浏览器外壳_新建独立实例 / 取运行快照JSON / 浏览器外壳_销毁`。**普通 Win32 原生动态内嵌区域（`lingbuilder.fbro.browser` 2.8.0，2026-09-19 落地，不依赖 new_emoji）**：给 `FBro_创建区域(实例编号, 左, 顶, 宽, 高, 地址, 缓存目录, 代理地址, 用户代理)`（返回 1/0）、`FBro_取区域实例JSON()`、`FBro_关闭全部区域()` 三条命令，复用同一已出货 `LING_FBRO_PROCESS_EMBEDDED` 跨进程内嵌路径（不改桥），在普通 Win32 宿主窗口客户区指定矩形新建 WS_CHILD 承载、把独立进程 FBro 浏览器作为其子窗挂入；`实例编号` 为正整数唯一键（重复拒绝），坐标尺寸为逻辑像素按 DPI 自动缩放，缓存目录留空按编号派生、逐实例独立 profile/进程，随 `浏览器管理器_调整页面()` 在 WM_SIZE/DPI 变化时跟随各自矩形——与 `CEF3_创建区域` / `EdgeView_创建区域` 同口径，让纯 Win32（win32.basic + common-controls + fbro.browser）项目无需 new_emoji 即可动态内嵌多店铺 FBro。真机验收：CLI 构建 `编译成功`，运行期 1 主窗 + 2 独立 `LingBuilderFbroHost.exe`，两区域各自渲染不同页面/独立缓存。**UA 边界（AI 必读）**：`用户代理` 透传到 host `LB_FBro_CreateEx2` 第 4 参只改 `navigator.userAgent`，**不改出站 HTTP `User-Agent` 请求头**（Chromium 语义，与 CEF3 需「资源加载前」头改写同理；FBro 桥未暴露该订阅位，故区域/独立进程 FBro 暂不提供请求头级 UA 覆盖）。AI 需要多店铺各异「请求头 UA」时应改用 CEF3 的 `CEF3_设置实例用户代理`，不得声称 FBro 区域命令能改请求头 UA。三内核「每店铺独立代理 IP」共同前提不变：底层代理提供不同出口 + 同引擎各实例彼此隔离。
...
- 「用 AI 生成模块（不需要会 C++）」当前共有三条入口，最终都落到 `.lingbuilder/module-build/<模块ID>`，再经校验、导出 `.lbmod`、`/api/modules/package/preview` + `/install` 由项目启用并 F5。前两条由 AI 自己用 `module.writeFiles` 直接写 module-build，第三条走 `aiModuleImportParser` 拆「### 文件：」后经 `/api/modules/developer/import-ai-files` 导入。(1) 交给本机 Agent（推荐）：模块页「AI 生成模块」把需求经事件总线交给面板内嵌的 DeepSeek Harness 运行时，由它调用 `lingbuilder.module.scaffold → module.writeFiles → module.validate` 在 `.lingbuilder/module-build/<模块ID>` 里创作模块并迭代到校验通过，再由用户在模块面板导出与安装。（历史上的一键生成双通道——系统 AI 由 renderer 编排 `cloudAi.start('chat')` 按「清单 → 其余文件 → 缺失补全」三阶段流式生成、自定义 API 走 `POST /api/modules/ai-generate` 由服务端一步生成导入——已于 2026-09-22 随 AI 面板引擎收敛为本机 Agent 一并下线；规范文档路径 `LINGBUILDER_AI_MODULE_SPEC_PATH` 仍供「复制 / 打开 AI 开发规范」按钮使用。）(2) AI Bridge MCP：外部 AI 通过 `lingbuilder.module.scaffold/writeFiles/validate/pack/installPreview/install` 六个受控工具端到端生成、校验、打包、预览并安装模块（安装必须先 installPreview 拿 previewId，再显式 `approved=true`）。(3) 手动复制粘贴（降级）：「复制 AI 开发规范」按钮读入剪贴板，用户粘贴给任意外部 AI 后把回复粘回 IDE。本机 Agent 通道依赖面板「模型通道」里配置的 DeepSeek 官方 Key 或自定义 OpenAI 兼容网关，不消耗 LingBuilder 云端点数；手动链路不依赖云端账号、API Key 或 AI Bridge 写入权限，AI 不得声称手动链路需要联网授权或额外服务端才能导入模块。。
...
## 内存命中点形态校验范式：宽松解码 + 正则（2026-09-23，实测锚定）

在中文源码里对「进程内存/文件/网络载荷」的命中点做**内容形态校验**（例如确认一段内存是形如 `windows_1-<base64>_<数字>_<数字>` 的令牌）时，**必须用「宽松解码（GB18030/ANSI）+ 正则」范式，禁止写逐字节状态机**。该范式已由独立最小复现工程实测：同一份数据、同一个目标，**正则法 200/200 命中，逐字节状态机 0/200 命中**（窗口里放的就是合法样本仍然判不出来）；真机 5 个进程命中 11 次并经接口验证有效。
...
为什么可靠：`GB18030` / `ANSI` 都是宽松编码，ASCII 段（令牌本体）原样保留，内存里的任意二进制不会让整体解码失败；两种都失败就跳过该命中。正则把「形态判断」交给 `std.regex`，不存在逐字节状态被写错的可能。

**为什么禁止逐字节状态机**（实测根因，不是猜测）：`状态` 变量在某个 `如果 (状态 == N)` 块内部被改写后，**同一轮循环里后面的 `如果 (状态 == N+1)` 块会立刻再跑一遍**——分隔符字节于是被当成「数据结束」处理，结束位置停在分隔符上，最终段长算出负数、令牌被丢弃。这类错误极难从运行现象看出来（不崩、不卡、只是判不出）。同样的形态校验任务用正则一行完成，没有这类状态污染。

适用边界：本条针对「命中点内容是否长得像某类字符串」的**形态校验**；需要按字节语义处理二进制协议时仍应使用 `字节集_寻找` / `字节集_截取` 等模块命令，不要退化成手写下标循环。
...
- **字符串字面量区间扫描只有一个实现（2026-09-24 起）**：`electron/src/services/lingCpp/stringLiteralRegions.ts` 的 `collectLingCppStringLiteralRegions` / `buildLingCppStringLiteralMask`。ASCII 双引号字符串内 `\"` 是完全合法的转义（不结束字符串），连续反斜杠按奇偶配对（`"a\\"` 闭合、`"a\\\""` 未闭合）；中文引号 `“...”` 不解释反斜杠转义，遇第一个 `”` 结束。表达式二元拆分（`splitEplBinaryExpression`）、实参拆分（`splitCallArguments`）、形参表与默认值拆分（parser）一律复用该工具，**禁止再手写引号状态机**——此前 `splitEplBinaryExpression` 不处理转义，拼接表达式里一出现 `\"` 就引号奇偶翻转、整句中文源码被原样吐进 C++（报 MSVC C2065）。
- **生成端兜底不许静默吐中文（2026-09-24 起）**：`translateLingCppExpression` 对无法翻译的表达式（含引号残留、全角标点、中文引号等必然生成坏代码的形态）一律降级为 `L"" /* 无法翻译的表达式：<原文> */`，并经 sourceMap 反查产出**构建前中文阻断诊断**（`<源文件> 第 N 行：表达式「…」无法翻译成 C++，已按空文本降级…`），`build.run` / F5 在编译前即报中文原因，用户不应再看到指向 main.cpp 的 C2065；只有透传后就是合法 C++ 的形态（`甲[0]`、`-1` 等）保持原样透传。未闭合的字符串字面量（缺收尾引号）另有语言服务错误级中文诊断 `lingcpp-string-unterminated-*`（带行列），与该降级互为双保险。AI 遇到该阻断时，应改写表达式为已支持形态或改用等价模块命令，而不是绕过门禁。
...
- 新建文件（2026-09-21）：系统 AI 编辑请求可以携带 `newFiles` 新建白名单（显式 `paths`，或 `directories`+`extensions`+`maxCount` 目录许可）。只有白名单声明范围内的工作区相对路径允许作为新建文件出现在 `files[]` 中；范围外路径会被云端与 IDE 双重过滤并给中文报错，报错会点名被拒路径。写盘安全仍由 IDE 的路径策略与可写扩展名白名单在应用阶段强制。
- 设计器联动判定（2026-09-22 收口）：`edit.propose` 在系统 AI（strict）路径上**不再按指令关键词判断「这是不是布局需求」**。旧口径把「窗口/按钮/显示/移动」等词命中即要求设计器模型必须变化，导致「给按钮1加点击计数」这类纯行为需求被误拒（报「AI 返回的设计器模型与当前模型完全相同」或「未返回完整设计器模型」）。现在的唯一判据是事实：把提案落盘后的源码跑 `controlRef` 语义诊断（与 `edit.apply`、`build.run` 共用 `collectControlReferenceAdmissionProblems`），引用了模型中不存在的控件才中文阻断并指向 `updatedDesignerProject`；否则按纯源码提案受理，AI 原样回传的等价模型直接丢弃、不写回设计器。「美化界面」类宽泛视觉请求仍保留本地产出可预览布局的降级方案。
- 模块生成不走编辑链（2026-09-21）：用户要求「生成/封装/做一个 .lbmod 模块」时，AI 助手聊天面板必须路由到「AI 生成模块」专用链（多阶段生成 → 「### 文件：」契约解析 → 导入 `.lingbuilder/module-build/<moduleId>`），绝不进入普通代码编辑提案——编辑链只允许修改已有文件，模块文件是全新产物，进入编辑链必然被白名单过滤为空并报「AI 未返回有效的完整文件修改结果」。
...
## 当前窗口自身句柄与标题（2026-09-22 已落地）

- `lingbuilder.win32.basic` 提供三条免句柄的当前窗口命令：`窗口_取自身句柄()`（长整数型 HWND）、`窗口_取自身标题()`（文本型）、`窗口_设置自身标题(标题)`（逻辑型）。普通 Win32 与 new_emoji 两个后端都有真实运行时实现，行为一致。
- 改当前窗口标题一律写 `窗口_设置自身标题(格式化文本("已点击 {} 次", 计数))`，禁止生成 `控件_设置文本(当前窗口, ...)`（`当前窗口` 不是可视控件引用，会报 `lingcpp-control-reference-missing`），也禁止生成 `当前窗口.标题 = ...`（窗口没有可写 `标题` 成员语法，会被判「变量 当前窗口 尚未声明」）。
- `窗口_取自身句柄()` 是纯 Win32 窗口项目取得自身 HWND 的**唯一入口**。`lingbuilder.win32.window-utils` 的 `窗口_设置标题`、`窗口_移动`、`窗口_置前台`、`窗口_开始拖拽`、`窗口_开始边缘缩放` 首参都是 HWND，句柄只能由它提供；该模块默认未启用，需要这些能力时先启用到项目，或直接改用上面的免句柄三条。
- 红线：禁止用 `窗口_按标题查找("设计器里的标题文本")` 反查自身句柄——用户在画布上改一次标题，功能就静默失效。句柄只在窗口存活期间有效，跨处理器要现取现用，不得长期缓存，也不得存进项目全局变量后跨窗口复用。
...
- 自动拉起的 stdio 宿主环境里没有模块授权与浏览器凭据，因此宿主 `command` 必须直连 LingBuilder 主程序 exe（`ELECTRON_RUN_AS_NODE=1`）、**禁止指向 `lingbuilder.cmd` 等启动器脚本**（会复活 GUI 进程并挡住安装器更新），主进程已在写入托管配置前做硬门禁。需要启用收费模块时，用户在「AI Bridge 连接中心 → Bridge 启动设置 → 允许外部 AI 客户端使用本机授权」显式开启后，宿主才向本机回环授权代理换取模块 Permit 与 FBro 授权码；该开关默认关闭，关闭或 IDE 未运行时必须保持 fail-closed（收费模块照旧被权益门禁拒绝，只给中文诊断，禁止静默放行）。凭据只驻内存、不落盘、不进日志，IDE 退出即撤销代理。**门禁报错三分类（2026-09-21 起，`electron/src/services/aiBridge/moduleAccessGate.ts` + `cli.ts` 启动分类）**：宿主启动先于用户开启授权开关 ≠ 未购买——CLI 侧保留启动换取结果（env 注入 / 换取成功 / 失败三态），`assertModuleAccess` 失败路径上惰性重换一次（3 秒冷却，成功即热更新 Permit 放行）；换到 IDE 最新授权状态仍无该模块凭据 → 维持「请先登录并购买」；启动时序导致状态缺失 → 报错必须指向「勾选开关后重启 AI 客户端或重连 MCP」，**禁止出现「请先登录并购买」**。过期/签名/信任锚诊断原样透出，任何路径不得静默放行。回归 `tests/moduleAccessGate.test.ts`（已进 `test:lingcpp`）。
- 灵码 Skill 离线快照随 IDE 安装包分发在 `resources/skill-kit/`（源文件 `electron/skill-kit/SKILL.md` + `manifest.json`），是一条「一键复制指令」指向的自安装指引：AI 客户端读它、自行把 stdio MCP 写进自己的配置，IDE 不代写任何第三方客户端文件。指引里 `# lingbuilder:managed-block:begin/end` 区间的托管配置必须与 `createManagedBlock` 逐要素同形（键集合、`args` 顺序与取值、`env`、审批模式），改生成器必须同步指引正文，反之亦然；`SKILL.md` 正文变更必须同步 `manifest.json` 的 `bytes`/`sha256` 并推进 `sequence`。指引正文必须显式写明：宿主禁指启动器脚本、stdio 模式无 Token、只写 `.lcpp` 不产生界面、控件裸名与 `&处理器`/`#常量` 红线、收费模块需用户勾选本机授权。**2026-09-21 版（sequence 2）新增「第 0.5 步：可行性前置检查」**：AI 在第一次 `project.create` 前必须自查收费模块/本机授权开关、目标架构（`build-configuration.json` 只认顶层 `mode`+`architecture`，new_emoji/浏览器内核项目需 x64）、全局还是工作区安装（全局落点：`%USERPROFILE%\.codex\config.toml` / `claude mcp add --scope user` / `%USERPROFILE%\.gemini\settings.json`）、失败先看哪条诊断；并写明 MCP `project.create` 挂收费门禁而 CLI `project build/run` 不挂的入口差异、接入产生的工作区文件是项目工件不应清理。回归见 `tests/skillKit.test.ts`。
- 工作区自省是第一动作（2026-09-21 起，`MCP_INSTRUCTIONS` 第 15 条同步）：`lingbuilder.workspace.list` 返回数组**首项是 `type="workspace"` 的合成根条目**，其 `workspaceRoot` 字段为当前 Bridge 工作区根绝对路径（stdio 模式下=客户端启动目录）；**2026-09-23 起根条目另有 `ideVersion` 字段**，`/api/ai-bridge/health` 响应与 MCP 握手 `serverInfo.version` 也动态回报同一 IDE 版本（`src/services/aiBridge/ideVersion.ts` 统一解析：主进程经 `LINGBUILDER_IDE_VERSION` 环境变量注入 → 就近 package.json 回退 → `0.0.0` 兜底；外部 AI 做兼容性判断或问题报告时引用它，禁止再回退到历史上硬编码的 `0.6.6`）；外部 AI 开始读写前先读首项确认工作区，与用户项目目录不符就停止并让客户端切目录重启 MCP 宿主。文件/项目源码目录不存在（ENOENT）时报错必须带「当前 AI Bridge 工作区是 <路径>」与两种修法（`wrapWorkspaceUnreachableError`，保留 `code='ENOENT'` 供内部新文件判定分支）。`resolveLingCppProjectSources` 的 sourceRoot 为 `'.'` 或空串时按项目根处理（与 `resolveAssetProject` 的 `|| '.'` 口径统一），路径越界报错必须带当前 sourceRoot 与两种修法（设为 `'.'` 或把 `.lcpp` 移入 `sourceRoot/`）。
- 目标项目必须在动手前确定（2026-09-23 起，`MCP_INSTRUCTIONS` 第 19 条同步）：`workspace.list` 首项的 `projects[]` 字段列出解决方案中全部已注册项目（`id`/`name`/`sourceRoot`/`type`/`isWindowProject`），`startupProjectId` 是当前启动项目；**多项目工作区禁止按目录名猜**——默认项目的 `src/` 占位与各项目的 `src/<项目id>/` 嵌套并存、同名 `MainWindow.lcpp` 很常见，猜错会把改动写进别的项目且 propose/apply/build 全链路静默成功；确定项目后调 `lingcpp.diagnostics` / `edit.propose` / `build.run` / `native.*` 必须显式传 `projectId`，并在提案与回复里写明「本次改的是项目 X（projectId）」。`projects[]` 缺失时读 `.lingbuilder/solution.json` 核对；单项目工作区的 `lingcpp.diagnostics` / `edit.propose` 响应自带 `projectHint`（projectId/name/sourceRoot + 中文说明），直接引用其中的 projectId。视图数据经 `solutionService.peekSolution()` 只读读取（`getSolution` 在空工作区会物化默认项目，只读接口禁止）；多项目工作区不出 `projectHint`（没有唯一答案就不给假答案）。
...
- 模块创作通道与欢迎页入口同源（2026-09-20 起）：`module.scaffold` 支持可选 `category`（只允许 界面/系统/网络/数据库/图像/AI/构建/其他）与 `description`，会写进 manifest，与用户在欢迎页「新建模块」看到的是同一 `createModuleTemplate` 骨架。用户也可能从欢迎页「新建模块 / 打开 .lbmod 模块包」自行把模块放进 `.lingbuilder/module-build/<id>` 并登记为开发源（`module-links.json`）：这类模块列表标注开发源、`installPath` 指向源目录，清单与源码改动即时生效、无需重新打包安装；外部 AI 看到这类模块时不要建议重复安装，编辑-发布闭环走模块页「模块包导出」（源码目录限 `.lingbuilder/module-build`、输出限 `.lingbuilder/module-packages`）。
...
- 等待与定时口径（2026-09-21 起，与 MCP instructions 第 13 条互为同步锚）：事件处理器里「等一会儿再做事」禁止用 `线程_协作等待` / `线程_等待`（界面线程调用会冻结窗口），改用 win32 基础模块的不冻结方式——`延时(等待毫秒)`（同步等待、期间泵送界面消息）、`延迟调用(等待毫秒, &处理器)`（一次性 WM_TIMER 到期在界面线程调用无参 `&处理器`）或设计器「时钟」非可视组件（`时钟_启动/时钟_停止/时钟_置周期/时钟_取周期/时钟_是否已启动` + 「周期到期」事件，`时钟_置周期(组件, 0)`=停止计时）。时钟组件只在窗口项目计时；控制台项目改用 `延迟调用` 配合 `EdgeView_泵消息` 或多线程模块命令。
...
- 设计器改动判据（2026-09-22 起，取代旧的关键词口径）：提案是否涉及界面，只看**事实**不看措辞——源码是否引用了模型中不存在的控件（上一条门禁），以及 `updatedDesignerProject` 与当前模型是否真的不同。因此：① 「给按钮1加上点击计数」「点击后隐藏窗口」「下载完成后更新进度条」这类纯行为需求按纯源码提案受理，**不要**为了通过校验去改无关控件的颜色、位置或尺寸；② AI 原样回传的等价模型会被直接丢弃（不写回设计器），IDE 会如实播报「界面布局未发生变化（本次仅源码改动）」，不要误判为失败；③ 只改界面、源码不动是合法提案（`files: []` + 真实布局变化），面板与云端草稿链路都放行；④ 源码与设计器**都**没变化的提案会被中文拒绝（`AI 未产生任何实际改动…`），此时应重新给出真实改动，而不是重复提交同一份内容。唯一仍强制布局变化的例外是「美化界面/界面太乱」这类宽泛视觉请求。
- 返回语句写法口径（2026-09-20 起）：外部 AI 生成或修改 `.lcpp` 时，带返回值的返回一律写 `返回(表达式)`（如 `返回(合计)`），无值提前结束写 `返回()`；不带括号的 `返回 表达式` 虽然合法等价，但示例、教程与演示项目已统一为括号形态，不要再按空格形态生成。
- 外部 AI 实参类型与内存形态校验口径（2026-09-23 起，与 §5「实参类型门禁」「内存命中点形态校验范式」两条互为同步锚）：生成模块命令调用时实参类型必须与形参声明一致（字节集不得直接传文本型形参，`build.run`/`native.preview` 会以 `lingcpp-argument-type-` 中文诊断在生成前阻断并给转换建议）；字节集转十六进制文本用 `字节集_到十六进制文本(数据)`（`字节集_到十六进制字节集` 旧名 `字节集_十六进制编码` 仍可用，但它返回的是字节集不是文本）；对内存/文件/网络载荷做「内容是否长得像某类字符串」的形态校验时，用「宽松解码（GB18030/ANSI）+ 正则_取首个」范式，禁止写逐字节状态机（实测 0/200 漏判：同一字节会被同一轮里相邻的 `如果 (状态 == …)` 块重复消费，分隔符被误判为数据结尾且极难排查）。
...
- 外部 AI 服务端项目规则（2026-09-19 起，`MCP_INSTRUCTIONS` 第 8 条同步）：HTTP 服务端项目（启用 `lingbuilder.http.server`）固定内容的响应必须优先用 `HTTP_添加静态路由` / `HTTP_添加静态文件路由`——工作线程直回、不占 UI 线程（实测吞吐比动态处理器高约 60%，同机回环 4.4 万请求/秒）；只有需要读写窗口控件或按请求计算时才用 `HTTP_添加路由 + &处理器`（动态处理器在 UI 线程执行，禁止长耗时同步操作）。HEAD 未显式登记时自动回落匹配 GET 路由。持续高负载用 `HTTP_设置连接轮转(服务, 请求数)` 放宽单连接 100 请求的默认强制关闭阈值。禁止生成绕过模块命令的裸 socket/HTTP 自实现来冒充服务端能力。
- 外部 AI HTTP 客户端 Cookie 注入规则（2026-09-19 起，`MCP_INSTRUCTIONS` 第 9 条同步）：`lingbuilder.net.http-client` 2.1 的 Cookie 能力分两层——`HTTP客户端_设置Cookie` 只是 WinHTTP 自动罐（接收 Set-Cookie、自动回送）开关，**没有注入口**；把外部已取得（内存/文件/浏览器）的 Cookie 发出去必须用注入命令：按请求整体注入 `HTTP客户端_请求置Cookie(请求, Cookie串)`（该请求含重定向只发这份，自动罐回送同时关闭），按域/路径长期回送 `HTTP客户端_置Cookie(客户端, 名称, 值, 域, 路径)`（空域匹配任意主机，前导点匹配该域及子域；路径前缀匹配；同名同域同路径覆盖），回读 `HTTP客户端_取CookieJSON`（只含手工注入项）、清空 `HTTP客户端_删除全部Cookie`。禁止生成「用 `设置请求头`/`设置默认请求头` 提交 Cookie」的写法（受管头会被运行时拒绝）；旧入口 `网页_访问_对象` 的 `提交Cookies` 参数仍可用，但新代码一律走受管客户端注入命令。
- 外部 AI 浏览器跨域与进程级启动开关规则（2026-09-20 起，`MCP_INSTRUCTIONS` 第 3 条同步）：用户要「跨域取 iframe / 放开同源限制」时按内核分流，禁止把某一内核的做法当成三内核通用。① **FBro**：`FBro_设置启动开关JSON`（把 `enableCrossFrame` 置 true）或控件属性「启用跨域模式」，桥内调官方 `FBroHsCommandLine_EnableCrossFrame`（真机回读实际写入 `--disable-web-security --disable-site-isolation-trials`），**仅进程内模式生效**；该命令的 9 键白名单（disableGpu、disableGpuCache、disableGpuBlockList、enableMediaStream、enableSpeechInput、enableAutoplay、headless、enableCrossFrame、disableProxy）是全部 CEF 进程级启动开关的唯一入口，开关文本必须写成调用处的字面 JSON（生成期烘焙进初始化，运行期调用只回报是否已烘焙、初始化之后登记一律返回失败），白名单外的键或非法 JSON 在生成前中文阻断，禁止借本命令传任意 Chromium 开关、也禁止宣称能按域名精细放开；是否真到达 CEF 用 `FBro_取启动命令行` 回读做正反对照。② **CEF3**：只有按精确源→目标逐条放行的 `CEF3平台_添加跨域白名单` / `CEF3平台_删除跨域白名单` / `CEF3平台_清空跨域白名单`，不存在「全局关闭安全策略」命令，跨帧取值与填表走 `CEF3框架_*` + `CEF3填表_*`。③ **EdgeView（WebView2）**：无白名单 API，只有创建期属性「关闭跨域安全策略」或 `EdgeView创建选项_置附加参数`，且安全类开关必须走 `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 环境变量通道才生效（`AdditionalBrowserArguments` 会被 release SDK 过滤）。共同红线：三者都是进程级/创建期，运行期切换一律无效，放开后同进程所有实例都受影响，只能用在受控内网或自有页面。另：CEF3 的 JS 交互（cefQuery）自 2026-09-19 起支持**多通道**——控件 `jsQueryFunctions` 属性按分号写多条「查询函数名,取消函数名」，查询ID 由桥全局递增分配不会串台，「查询请求/查询已取消」事件带 `channelIndex`（来源通道序号，0 起），因此生成期内部读取通道与用户自建通道可共存，外部 AI 不得再假设「一个程序只有一条通道」。细则见「跨域/同源策略放开三内核选型」与「无头浏览器三形态选型」两条。
- 外部 AI 浏览器代理 IP 三内核选型（2026-09-20 起）：用户要「多店铺独立出口 IP / 全局代理」时按内核分流，禁止把某一内核的做法当成三内核通用。① **CEF3**：`CEF3_设置全局代理(代理地址)` / `CEF3_清除全局代理()` / `CEF3_取全局代理()`（只影响之后新建实例，控件/弹窗/区域/无头未自带代理即回落；`CEF3_设置代理` 传空 = 显式直连、不参与回落），实例级用创建参数 `代理地址`（`CEF3_创建弹窗浏览器` / `CEF3_创建区域` / `CEF3_创建无头浏览器`）或 `CEF3_设置代理(控件名, 代理地址)`（创建前），回读用 `CEF3_取实例代理(实例编号)` / `CEF3会话_取代理(控件名)`。**CEF3 当前不支持带认证（用户名/密码）的代理**：2026-09-20 真机二分实验证伪 CEF 150 两条官方通道（代理 407 不投递 `GetAuthCredentials`；资源级注入 `Proxy-Authorization` 头会被 network service 静默吞请求），不得生成任何「CEF3 代理认证」调用或伪造命令，`fixed_servers` 也禁止拼 `user:pass@host:port`（Chromium 不支持），需要代理认证改用 FBro 或 EdgeView；中继方案登记于 `docs/FUTURE_OPTIMIZATIONS.md`。② **FBro**：实例级 `FBro_设置代理(控件名, 代理地址)` / `FBro会话_设置代理认证(控件名, 代理地址, 用户名, 密码)`（支持认证）；多店铺独立出口首选 `浏览器外壳_新建独立实例代理(稳定ID, 地址, 标题, 缓存目录, 代理地址, 用户代理)` 与 `浏览器外壳_新建内嵌实例区域(...)`（独立 Host 进程天然隔离，另有 `浏览器外壳_取当前代理` 回读）；全局统一出口用 `FBro指纹_设置启动代理(地址, 用户名, 密码)`（VIP 能力，必须在首个 FBro 运行时初始化前调用）。③ **EdgeView**：`EdgeView_设置全局代理(代理地址)` / `EdgeView_清除全局代理()` / `EdgeView_取全局代理()` + 实例级 `EdgeView_创建实例代理` / `EdgeView_创建区域代理` / `EdgeView_创建弹窗浏览器代理`，`EdgeView_取实例代理(实例编号)` 回读；代理随实例浏览器进程注入，**独立出口 IP 必须「不同独立缓存目录 + 不同代理」成对**（同缓存目录共享进程时代理不独立）。共同前提：出口 IP 是否真的各异取决于底层代理本身提供不同出口；三内核实例间彼此隔离。另两条运行期红线：CEF3/FBro 代理必须在实例创建前生效（CEF3 创建后不可改，FBro 独立进程随创建传入）；EdgeView 全局代理只影响之后新建实例、不追溯已建实例。
- 大能力模块的界面开发视图（2026-09-19 起，`lingbuilder.module.info`）：命令数千条、控件数十个的模块（`lingbuilder.new_emoji.ui` 为典型）不要整表翻页找写法，三个只读视图就是权威事实来源。① `designerControls` 是该模块贡献到设计器的控件清单（`type`/`namespacedType`/中文 `label`/是否容器/`lingCppType`/代码创建命令/允许的父级/属性与事件数量）；要知道某个控件的完整属性表（含枚举 `options` 与默认值）、事件与处理器命名（`handlerPattern`）、`defaultProps`、布局协议和代码创建参数，就传 `control`（如 `control: "按钮"` 或 `control: "Button"`）取 `designerControlDetails`。控件类型、属性名和事件处理器名一律以该视图为准，禁止凭记忆编造（编造的控件引用会被控件门禁阻断）。② `demoExample` 是演示语料里该命令的**真实调用行**（参数顺序、是否带引号、处理器写法均可直接照抄），仅在按 `query` 收窄结果集时附带；某条命令没有 `demoExample` 只表示语料未覆盖（`demoProject.stale` 为 `true` 时更要注意），不代表命令不可用。③ `uiExamples` 是按场景组织的可复制界面配方索引（模块 `contributes.examples` 与工作区 `examples/ui-recipes/<moduleId>/recipes.json`），传 `example`（标题、路径或序号）直接取回一条配方正文。④ 每个控件还有一张**组件卡**：概览与详情都带 `documentation`（LingBuilder 中文契约卡路径）与 `nativeDocumentation`（上游 `EU_*` API 文档路径）；`control` **唯一命中**时响应直接内联 `componentGuide.content`（代码创建命令、标记查找、属性表与枚举取值、事件与处理器命名、可粘贴骨架、人工红线段），命中多个时只给路径，需要上游 API 细节时用 `lingbuilder.file.read` 按路径读。卡片内容由 `npm run module:new-emoji-cards` 从模块清单确定性生成，人工只补「惯用要点与红线」段——所以卡片与清单不一致时以清单/卡片为准，不要按记忆纠正。
...
- CEF3 无头浏览器（官方 OSR，2026-09-20 落地，`MCP_INSTRUCTIONS` 第 3 条第四形态同步）：用户要「CEF3 无头抓取/无界面浏览器」时用 CEF 官方无窗口渲染（OSR），全程不创建任何窗口。创建两条入口：`.lcpp` 直接调 `CEF3_创建无头浏览器(实例编号, 地址, 独立缓存目录, 代理地址, 视口宽, 视口高)`（控制台项目首选；地址空=about:blank，缓存目录空=默认 `cef3-headless-<实例编号>`，代理非空=该实例独立出口 IP，视口默认 1280×720 且宽高必须正整数），或窗口项目在设计器放「CEF3无头浏览器」非可视组件（属性：实例编号/打开地址/独立缓存目录/独立代理地址/视口宽/视口高/窗口创建期自动启动，与 EdgeView 无头组件同口径）。实例编号是 **CEF3 引擎内项目级唯一**（运行时键 2000000+编号，不与 EdgeView/FBro 共号空间）；无头实例没有宿主窗口，控件版事件派发不生效——控制台与窗口项目都只能用 `CEF3无头_取事件JSON` 轮询（实例缓冲上限 200 条、超限丢最旧），给无头实例绑定中文事件处理器无效。取内容两条路：`CEF3无头_取主框架` 取框架句柄后接 `CEF3框架_*`/`CEF3填表_*`/`CEF3DOM_*`（句柄版命令零改动可用），或 `CEF3无头_执行JS`/`CEF3无头_取页面文本`/`CEF3无头_取页面源码`；只要纯文本判断优先 `CEF3无头_取页面文本`。浏览器级控制（重绘/帧率/回调订阅位）用 `CEF3无头_取浏览器句柄` 的浏览器句柄接 `lingbuilder.cef3.osr` 模块（`lingbuilder.cef3.browser` 的子模块，同 `windows-msvc-x64` target）的 `CEF3OSR_请求重绘`/`CEF3OSR_设置窗口外帧率`/`CEF3OSR_取窗口外帧率`/`CEF3离屏_订阅像素帧`/`CEF3离屏_订阅视图矩形`。三条红线：① 受管句柄（框架句柄/浏览器句柄）判有效**只用 `!= 0`**——64 位受管值高位恒置 1、按 int64 恒为负数，写 `> 0` 会把每个合法句柄判成无效，`CEF3OSR_*`/`CEF3离屏_*` 全部收句柄入参同理；② `CEF3无头_等待加载完成` 只证明「可以取内容」，判断「真的在渲染」必须用 `CEF3无头_等待出帧`（加载完成≠已出帧，真机首帧晚约半秒；该命令等待期间周期性请求重绘唤醒渲染管线），创建后立刻 `CEF3无头_导航` 是安全的（浏览器未建好时自动排队补发，不静默丢失）；③ **截图未开放**——`CEF3离屏_订阅像素帧` 只点亮订阅位、不外发帧内容（出帧证据用 `CEF3无头_取渲染帧数`），不得向用户承诺截图/取画面，更不得创建任何窗口再隐藏来冒充无头。另：`CEF3无头_执行JS` 返回 DevTools 信封 JSON（`{"result":{"type":…,"value":…}}`）不是裸值，按信封取 `value` 字段。范本：模块 snippet「CEF3 无头抓取（控制台项目）」与 `examples/cef3-headless-demo/`（窗口/设计器形态）；回归 `electron/tests/cefHeadlessCommands.test.ts`、`electron/tests/cefHeadlessRuntime.test.ts`、`npm run smoke:cef3-headless-native`。
...
- 外部 AI 需要操作「自己这个窗口」时（改标题、移动、置顶、拖拽），按第「当前窗口自身句柄与标题」节口径生成：改标题用免句柄的 `窗口_设置自身标题(标题)`，需要 HWND 时先 `窗口_取自身句柄()` 再传给 `lingbuilder.win32.window-utils` 命令。禁止生成 `窗口_按标题查找("设计器标题")` 反查自身句柄，也禁止生成 `控件_设置文本(当前窗口, ...)` 或 `当前窗口.标题 = ...`。这条同时写进 `MCP_INSTRUCTIONS` 第 17 条（回归在 `tests/aiBridge.test.ts` 的 instructions 关键词断言）。
- 外部 AI 生成 DLL（动态库）项目时（2026-09-23 起，`MCP_INSTRUCTIONS` 第 18 条同步锚）：两条路径按需求选——A) 通用路径 `project.create` 窗口模板 + `outputType="dll"`（「公开」子程序确定性导出为 DLL 接口，extern "C" + __cdecl，事件处理器/私有成员不导出，跨 DLL 边界只允许 POD 与文本签名；build.run 产出 <产物名>.dll + 同名导入库 .lib，不生成 wWinMain 与消息循环）；B) `templateId="windows-dll"` 创建 C ABI 示例工程（DllApi.lcpp 目前仅 获取接口版本 的整数返回值映射到 LingBuilder_GetApiVersion，不得生成「任意中文过程自动成为导出函数」的代码）。共用红线：DLL 项目没有运行入口，build.run 传 run=true 会被拒绝并引导改用生成，禁止再调 run.wait/run.log；DLL 构建必须 MSVC（缺失 MSBuild 时按中文指引转告用户安装或配置 LINGBUILDER_MSBUILD_PATH，不要反复重试）。跨 DLL ABI 细则见「动态库（DLL）输出与模块调用规则」节；回归 `tests/aiBridge.test.ts` 的 instructions 关键词断言与 outputType 创建用例。
...
项目启用 `lingbuilder.http.server` 时，AI 必须优先生成 2.1 受管 API：`HTTP_创建服务`、停止状态下的监听/资源配置、`HTTP_添加路由` / `HTTP_添加静态路由` / `HTTP_添加静态文件路由` 或 `HTTP_绑定请求处理器`、`HTTP_启动` 以及明确的停止/销毁。动态请求处理器必须是返回空的无参数事件或方法，并以 `&处理器名` 引用；处理器内先用 `HTTP_取当前请求()` 取得只在本次处理期间有效的 `HTTP请求`，再读取请求并发送一次响应或中止请求。固定内容的 JSON/文本/页面/文件响应必须优先登记静态路由（工作线程直回、不占 UI 线程），只有需要读控件、查数据库或按请求计算时才用动态路由。
...
HTTP 2.1 支持 HTTP/1.0/1.1、keep-alive、Content-Length/chunked 请求体、精确/末尾 `/*` 路由、HEAD 未显式登记时自动回落匹配 GET 路由（返回真实 Content-Length、无正文）、静态路由（`HTTP_添加静态路由(服务端, 方法, 路径, 响应内容, 内容类型)` / `HTTP_添加静态文件路由(服务端, 方法, 路径, 文件路径, 下载名称, 内容类型)`，工作线程直回不占 UI 线程）、`HTTP_设置连接轮转(服务端, 请求数)`（单连接强制关闭前最大请求数，默认 100，1–1000000，运行中可改）、查询/头/正文/客户端信息读取、文本/JSON/二进制/文件/Cookie/重定向响应、资源限制和统计，普通 Win32 与 New_Emoji 使用同一运行时。默认按监听目标解析后的实际 IPv4/IPv6 地址只允许回环绑定，并严格拒绝非法百分号编码或非法 UTF-8 解码结果；生成外部监听前必须显式调用 `HTTP_允许外部监听` 并提醒用户配置鉴权、限流和日志脱敏。模块不内置 TLS、HTTP/2 或身份认证，公网 HTTPS 必须由反向代理或网关提供。`HTTP_启动服务/等待请求/回复文本/关闭服务` 等 1.x 命令只用于迁移，不得作为新项目默认方案。
...
项目启用 `lingbuilder.sunnynet`（网络中间件模块 1.0，底层 SunnyNet v1.5.1，MIT）时，AI 按「创建 → 绑定事件处理器 → 设置端口 → 启动」生成，捕获/改写 HTTP(S)/WebSocket/TCP/UDP 收发数据。事件处理器必须是返回空的无参数事件，以 `&处理器名` 引用；处理器在窗口 UI 线程执行（底层回调线程投递，30 秒未处理按原样放行，不得在处理器内做长阻塞操作）。HTTP 请求与响应各触发一次，处理器内用 `网络中间件_取当前事件类型()` 区分 `#HTTP事件_请求`（1）与 `#HTTP事件_响应`（2）；请求头/体/URL/Cookie 修改只在请求事件生效，响应头/体/状态码修改只在响应事件生效。

三条系统级红线，AI 生成与解释时必须如实转述，不得省略：① `网络中间件_安装根证书` 会向系统受信任根证书库安装根证书 `LingBuilder.Sunny.Root`（首次安装程序会弹中文确认框，本地计算机库需管理员权限），配套提供 `网络中间件_卸载根证书`（清当前用户与本地计算机两处）；② `网络中间件_加载进程代理驱动` 加载内核级驱动捕获指定进程流量（需管理员、可能被安全软件提示、弹确认框）；③ **`网络中间件_重启计算机并完成驱动卸载` 在用户点击确认弹窗的「是」后会立即重启计算机**（底层设计即「驱动卸载必须重启完成」），点击「否」不执行任何操作；不重启的清理路径是 `网络中间件_卸载进程代理文件`。已加载驱动的中间件调用 `网络中间件_销毁` 会自动跳过底层 Close 并清理驱动文件。依赖：模块需要 `lingbuilder.sunnynet.sdk`（按需下载，约 81MB，缺失时构建给中文阻断诊断）。文本读体按 UTF-8，二进制用字节集命令族；局部变量带初始化会被提升到块首（生成器既有语义），创建句柄不要写成「逻辑型 结果 = 创建()」这类提升陷阱形态。
...
- `表格_` 与全部模块命令的宽字符串参数允许传入局部文本变量、文本型函数/功能库调用与拼接表达式；生成器统一通过受控 `LingCppWideArg` 适配为调用期间有效的 `const wchar_t*`，不得要求用户在 `.lcpp` 中书写 `.c_str()`。完整示例位于 `src/datagrid-api-demo/MainWindow.lcpp`。
...
- 命令族要点：请求/提交数据/进程消息为受管句柄对象（`FBro请求_*`/`FBro提交数据_*`/`FBro消息_*`）；填表 Set 族同步发后即忘，取值/取坐标内联等待并返回 JSON；`FBro框架_取源码/取文本` 返回 UTF-8 受管缓冲（配合 `FBro缓冲_转文本/保存文件`），避免大页面截断；启动开关（禁GPU/GPU缓存/GPU黑名单、媒体流、语音输入、自动播放、headless 无头、**跨域 enableCrossFrame**、**禁用代理 disableProxy** 共 9 项；headless 由「FBro无头浏览器」资源或 FBro_启用无头模式() 字面调用触发，跨域/禁用代理可由 FBroBrowser 属性「启用跨域模式/禁用代理」或 .lcpp 字面 `FBro_设置启动开关JSON("{\"enableCrossFrame\":true}")` 声明）由生成器烘焙 `LB_FBro_SetStartupSwitches` 在 CEF 初始化前应用，`FBro_取启动命令行` 查询生效命令行为正反对照证据；键白名单在桥与生成器各存一份，`tests/fbroHeadless.test.ts` 有双向漂移门禁。
...
- 无头浏览器（2026-09-19 落地）：无头模式是 CEF **进程级**启动命令行开关（官方 `FBroHsCommandLine_EnableHeadless`，桥 `LB_FBro_SetStartupSwitches` 的 `"headless":true`），只能在生成期烘焙、初始化前生效，运行期无任何命令能开关它。两条声明入口：① 设计器非可视资源「FBro无头浏览器」（属性：打开地址/缓存目录/附加信息JSON，窗口创建期自动后台实例化，按组件名用全部 FBro_* 命令操作、事件用 FBro_绑定事件）；② `.lcpp` 字面调用 `FBro_启用无头模式()`（控制台/无窗口场景，配合 `FBro_后台创建` 取句柄，句柄用 `FBro_实例导航/实例等待加载超时/实例执行JS/实例是否存活/实例关闭` 同步驱动；控制台无消息泵，事件处理器不会触发，取数必须走同步族）。红线：同一项目内无头声明与**进程内**可见 FBroBrowser 控件共存时生成前中文阻断（独立进程模式不受影响）；new_emoji 后端不支持无头；控制台项目不得放「FBro无头浏览器」资源（会被阻断，改用字面调用通道）。AI 不得生成「运行期切换无头」代码，也不得把 `FBro_启用无头模式()` 放进条件分支询问是否生效——它是全局声明，字面出现即烘焙。真机验收：`cd electron && npm run smoke:fbro-headless`。
...
- 示例红线：文本拼接优先使用 `格式化文本("K={}", 值)`；「命令调用 + 字面量」的直接 `+` 拼接自 2026-09-19 起由生成器把两侧的宽字符指针统一经 `LingCppWideArg` 归一后可正常编译（历史 C2110 指针加法已修复），但可读性仍以 `格式化文本` 为优先。
