...
- 设计器与 Win32 运行时的基础控件外观必须保持一致：普通按钮使用原生 owner-draw 圆角绘制；确定进度条根据最小值、最大值和当前位置在原生控件中央显示百分比文字。不允许只在 React 设计器预览中模拟这些效果。
...
- 窗口设计器的窗口自身事件保存在窗口模型 `events` 中；当前确定性支持 `Loaded`（创建完毕）。
- `events.Loaded` 是窗口创建完成后调用的中文处理器名；未显式配置时使用 `_<窗口类名>_创建完毕`。
- AI 修改窗口事件时必须同时保证 `.lcpp` 中存在同名事件处理器，不能只改设计器绑定或生成不存在的关闭、大小改变等未支持事件。
