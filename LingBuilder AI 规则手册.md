...
- 结构化编辑器（项目 DLL 命令声明卡片编辑器）支持「复制全部声明」「复制此命令」（单条命令带 DLL命令库 包裹与架构路径的独立片段）与「粘贴声明」（解析剪贴板、按命令名去重合并，重名跳过并在反馈区提示）。跨项目迁移 DLL 命令声明时应使用复制/粘贴而不是手工重抄。
