...
- 分享或导入 `.lcpppkg` 时必须尊重清单中的 `minimumGeneratorVersion` 与 `requiredCapabilities`；v1 旧包由新版 IDE 先从 `.lcpp` 补齐能力清单。ListView 高级接口使用 `win32.listview.advanced-api.v1`；类型化 `列表视图_创建行/创建行集合` 使用 `win32.listview.structured-rows.v1` 并要求生成器 0.2.8。缺少任一能力时不得继续生成调用，必须先提示升级。
