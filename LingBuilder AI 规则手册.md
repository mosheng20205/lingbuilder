...
- 已启用 `CDP 客户端模块`（模块 ID：`lingbuilder.cdp.client`，版本 `2.0.0`）时，新代码必须使用 `CDP_` 前缀命令驱动 Chrome/Edge 调试端口：`CDP_连接(调试地址, &就绪处理器)` 建立受管连接（支持多连接多端口并存），`CDP_新建页面`/`CDP_附加页面` 获取 `CDP页面` 会话，再用 `CDP_执行脚本`、`CDP_查询元素`+`CDP_点击元素`/`CDP_输入文本`、`CDP_截图`/`CDP_截图元素`/`CDP_打印PDF`、网络/Cookie/仿真命令执行自动化。所有异步命令必须使用 `&处理器名`，在 UI 线程通过 `CDP_取当前事件*` 快照判断结果。需要请求拦截时用 `CDP_拦截开始(页面, 模式, &处理器)`；“请求被拦截/需要认证”处理器必须立即取 `CDP_取当前拦截()` 并且只裁决一次（继续/改写/mock/终止/认证），不能保存后复用或让网络永久挂起。对话框处理器同样必须立即 `CDP_应答对话框`。新项目应调用 `CDP_设置命令超时`，下载前设置真实存在的目录，上传文件必须真实存在；Cookie、请求体、认证凭据不得写日志/分享包。元素输入优先 `CDP_输入文本`（逐字符键事件，headless 可靠），不要默认 `CDP_插入文本`。`CDP_连接` 只允许回环；远程地址必须显式 `CDP_连接远程` 并提示风险。`CDP连接`/`CDP页面`/`CDP元素`/`CDP拦截` 是受管句柄，不是文本或 controlRef，禁止加引号。阶段 3 的 Debugger、OOPIF/Worker、性能/堆快照、screencast、录制回放尚未实现，AI 不得生成对应 `CDP_` 假命令或宣称已支持。
