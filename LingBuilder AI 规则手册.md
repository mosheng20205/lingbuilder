...
- 子程序局部变量使用 `局部 类型 变量名`，可写初始化值和数组标记，例如 `局部 文本型 url = "https://example.com"`、`局部 字节集 ret`、`局部 整数型 结果[]`。局部变量必须位于方法、事件或构造块内，通常集中写在子程序开头，并且仅在当前子程序内可见；不要把临时值误建为程序集变量。
- 对变量赋值前必须先声明，赋值表达式的类型必须兼容变量类型。模块命令的 `contributes.commands[].returnType` 与模块类型的 `cppType` 是诊断和 C++ 生成的权威来源；例如 `网页_访问_对象` 返回 `字节集`，应保存到 `字节集` 变量，不能保存到 `整数型`。
...
- 结构：`包`、`使用`、`类`、`公开`、`私有`、`保护`、`局部`、`构造`、`析构`、`事件`、`结束`、`结束类`
...
- 已启用 `CEF3浏览器模块`（模块 ID：`lingbuilder.cef3.browser`）时，设计器工具箱会新增 `CEF3浏览器 (CefBrowser)` 控件；可在任意窗口添加多个实例，属性面板可设置打开地址 `url`、缓存目录 `cacheDir`、User-Agent、JavaScript/图片/WebGL 开关和代理。运行时命令第一个参数为设计器中文控件名：`CEF3_创建`、`CEF3_导航`、`CEF3_执行JS`、`CEF3_后退`、`CEF3_前进`、`CEF3_刷新`、`CEF3_停止`、`CEF3_关闭`、`CEF3_取标题`、`CEF3_取地址`、`CEF3_取最近事件`、`CEF3_取事件数据`、`CEF3_设置缓存目录`、`CEF3_设置代理`、`CEF3_绑定事件`、`CEF3_是否可后退/前进/加载中`。
...
- CEF3 事件名为 `开始加载`、`加载完成`、`加载失败`、`标题被改变`、`地址被改变`，通过 `CEF3_绑定事件` 回调到当前窗口无参数中文事件/方法。AI 不得编造 CEF3 的 DOM 同步回调语法，也不得把 CEF3 降级为旧版 IE WebBrowser 控件。
