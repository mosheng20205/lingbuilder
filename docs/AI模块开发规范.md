...
3. 把 AI 的回复完整复制，回到 LingBuilder 的“模块 → AI 生成模块”，粘贴到“导入 AI 生成的文件”输入框，点击“导入到 module-build”——IDE 会自动解析文件，在同盘临时目录完成清单和模块内容校验，通过后原子替换到 `.lingbuilder/module-build/<模块ID>/`。（也可以手动在文件管理器中把每个文件按路径保存到该目录。）
...
如果导入或校验失败，错误结果区的“复制错误详情”会复制完整错误标题和全部逐行诊断；把这段完整文本粘贴回 AI，让 AI 按每条诊断修正模块清单或源码后再重新导入。不要只复制其中一行，也不要把校验失败的模块当作可用模块继续打包。

解析区出现“复制 AI 模块解析诊断”时，说明回复格式本身存在重复文件、路径不安全或无法识别的文件；必须先修正这些诊断，导入按钮会保持禁用。文件标题允许使用 `./` 前缀，但同一模块中不能出现只差大小写的重复路径。

“③ 校验模块”和“导出 .lbmod”也使用同一套严格门禁：命令必须有 binding，文档和示例必须真实存在且非空，模块至少要有一项可用贡献；任一环节失败都不会生成可分发模块包。
