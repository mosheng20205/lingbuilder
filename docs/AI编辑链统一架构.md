...
### 面板修复批（2026-09-23）：会话映射、状态机自愈与项目上下文注入

- **面板会话 ↔ dsh 会话一一映射**：`agentSessionsRef` 按面板会话 ID 记录 dsh 会话；新建/切换/清除上下文、停止、重启、运行时崩溃都必须作废对应映射，服务层也不再复用上一轮 sessionId。否则「清除上下文」只是清 UI，模型仍记得全部历史。
- **运行时状态机自愈**：dsh 进程退出即时把快照置 `failed` 并透出退出原因；`runHarnessTurn` 把进程退出作为本轮失败分支（ack 之后的带轮死亡不会拒绝 pending 请求）；单轮失败即停机，下一条需求自动重启，杜绝两轮交错。主动停止导致的 in-flight 拒绝在面板侧静默。
- **当前项目上下文注入**：面板提交需求时经 `src/services/ai/agentTurnContext.ts` 注入项目 ID/源码目录/活动文件/是否窗口项目——Agent 经 MCP 只看得见工作区，多项目解决方案里没有这条事实会猜错项目（真机实测出在了默认占位项目上）。聊天区仍显示用户原始指令；意图判定仍由 Agent 完成，注入是事实陈述不是分类。
- **提案门检**：只有本轮真的调用过 `edit.propose` 才取回交接提案；过期（30 分钟）提案横幅预警并禁用应用；启动前凭据门检（自定义通道缺 Key 硬拦截）与零响应诊断文案。
- **AI 生成设计器模型归一化**：`normalizeWindowDesignerState` 按 win32 注册表补齐缺失外观键（背景/前景，与 createControl 同口径），并把 `enabled`/`visible` 映射为规范 `isEnabled`/`visibility`（画布有效状态只认规范字段）。
- **常规命名事件双口径**：源码 `控件名_被单击` 与 `_控件名_被单击` 都接进控件派发表，处理器名以源码实际定义为准。
