# FBro 与 CEF3 SDK 按需下载资源（维护者向）

本文档是**维护者向**参考：记录资源清单、直链验证口径、下载与安装的实现约束和实现位置。维护者向的云端清单发布与启用流程见 `docs/SDK下载清单发布流程.md`；**用户向**文档（下载失败排查 / 代理设置 / 离线手动部署）已迁至官网文档中心「[SDK 按需下载与离线安装](https://lingbuilder.com/guide/user/advanced/sdk-download)」（源文件 `cloud/admin/docs/guide/user/advanced/sdk-download.md`）。
...
- 平台：`Windows x64`
- 压缩包：`lingbuilder-cef3-sdk-150.0.14-chromium-150.0.7871.129-bridge4-windows-x64.zip`
- 下载地址：<https://msimgimg.xyz/uploads/lingbuilder-cef3-sdk-150.0.14-chromium-150.0.7871.129-bridge4-windows-x64.zip>
- 文件大小：`194123697` 字节（约 `185.19 MiB`）
- SHA-256：`ef841c19811ace53e30502ebc6d146aa90305ae969f327ba1199121822d6919b`
- 解压后大小：`473594068` 字节；文件数：`534`
...
## 代理与失败诊断（2026-08-30 起，实现约束）

> 用户向说明（代理怎么配、失败怎么办）已迁至官网文档中心「SDK 按需下载与离线安装」。本节保留实现口径供维护者核对。
...
完整的用户向离线安装指引（三种方式、安装标记字段、资源核对表）已迁至官网文档中心「[SDK 按需下载与离线安装](https://lingbuilder.com/guide/user/advanced/sdk-download)」，源文件为 `cloud/admin/docs/guide/user/advanced/sdk-download.md`。维护者排查安装问题时以该页为口径，本文件不再重复维护操作细节。
