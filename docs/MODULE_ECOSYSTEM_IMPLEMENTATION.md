...
> 2026-09-20 补充（随包模块升级自愈 + 修复重装，`ensureBundledModules` 语义升级）：上面的「只补缺」语义有个结构性漏洞——随包模块是 IDE 托管的版本化内容，老工作区里残留的**旧版清单永远不会被升级**；实例：2026-08-09 之前生成的 new_emoji 清单把 88 个 `cb` 回调参数声明成 `raw`，新版清单校验（`manifest.ts` 的 handler 规则）直接整卡拒绝，模块面板只显示「0.0.0 · 能力 0 项 · 清单无法读取或校验未通过」，且用户无自愈通道。现两路修复：① `ensureBundledModules` 对每个随包模块目录按清单判定（主进程不依赖 src 校验器，用「清单逐字节一致 + 点分版本比较」代替）：工作区清单**损坏（非法 JSON/缺版本号）**、**同版本但内容漂移**、或**版本低于随包副本** → 整体以随包副本重建（`rm` + 全量复制，陈旧文件一并清除）；工作区清单**缺失（半成品目录）**或**版本高于随包**（用户手动装过更新版）→ 保持旧「只补缺」语义不覆盖；随包清单自身损坏 → 不替换。② 模块面板对「清单无法读取或校验未通过」且随包源存在同 ID 可用副本的模块（`InstalledModule.bundledRepairable`，`ModuleService` 经 `LINGBUILDER_RESOURCE_ROOT` 推导 `default-workspace`）提供「修复重装」按钮 → `POST /api/modules/repair-bundled` → `ModuleService.repairBundledModule`（校验随包清单必须通过、ID 一致，内置/已链接开发源/无副本一律中文拒绝，替换前留快照、写 `repair` 历史，返回刷新后扫描结果）。开发源链接模块两路都跳过。修改判定逻辑必须同步 `tests/workspaceService.test.ts` 的「bundled modules replace stale or broken workspace copies but never downgrade newer ones」与 `tests/modules.test.ts` 的两条修复重装用例及本节。
...
- 2026-09-24 起 `lingbuilder.edgeview` 升级到 `1.6.0`（区域浏览器窗口缩放修复批次）：新增 `EdgeView_置区域位置(实例编号, 左, 顶, 宽, 高)`（窗口内逻辑坐标按 DPI 缩放，`SetWindowPos` 移动/缩放 创建区域 的承载 host 并即时对齐控制器；弹窗实例返回 0 并给中文诊断，应改用 `EdgeView_置实例大小`）。行为修复：`EdgeView_调整全部大小()` 原先对全部实例无条件 `put_Bounds(host 客户区)`，而承载 host 创建后不可改几何，「顶部留导航条」的区域布局一缩放页面就被拉成 host 尺寸排版成白屏；现 `EdgeViewInstance` 带 `autoStretch`（创建区域按 x/y=0 且宽高铺满窗口判定），窗口 `WM_SIZE` 只自动拉伸铺满实例、弹窗实例照旧走自身 WM_SIZE（与设计器 EdgeBrowser 控件「铺满才跟随」同一条规则）。配套生成器修复：窗口级事件（`SizeChanged` 等）在 Win32 后端只自动接线 `Loaded`，源码声明的 `_类名_大小被改变` 静默不生效；新增 `resolveWindowEventBindings` 统一出口按 `_窗口类名_事件中文后缀` 常规命名自动接线（方法真实存在才接线），同时喂 WindowSpec 事件串与窗口类派发表两处——「大小被改变」里 `EdgeView_置区域位置(1, 0, 54, 窗口_取事件宽度(), 窗口_取事件高度() - 54)` 即为留边距区域浏览器的标准缩放同步写法。总命令 320→321（基础/实例 57→58）；`generate-edgeview-api-doc.ts` 阈值 320→321 并重生成 API.md；`tests/modules.test.ts` 指纹回填 commands 3940 / parameters 6914 / commandDigest 3241e3bd / parameterDigest 1128aad0（内置）与 7957 / 19035 / 2f3d22d4 / 5a01e9ec（合并），controlReferences 1345 不变；新增「窗口级事件按 _类名_事件 常规命名自动接线」用例。MCP 第 3 条同步缩放红线（关键词断言 + `dist/cli.cjs` 重建 + stdio 握手验收）。
- 2026-09-19 三内核多店铺能力对齐（`lingbuilder.cef3.browser` 3.0.0-alpha.4、`lingbuilder.new_emoji.fbro-shell` 1.3.0）：CEF3 新增设计器无关的 `CEF3_创建弹窗浏览器(实例编号, 地址, 独立缓存目录, 代理地址)`（`LB_CEF3_BrowserCreateChrome` Chrome Runtime 顶层窗 + 独立 profile + 每实例代理）、`CEF3_创建区域(实例编号, 左,顶,宽,高, 地址, 独立缓存目录, 代理地址)`（运行时自建 WS_CHILD 承载内嵌多实例）、公开 `CEF3_枚举实例JSON()` 与 `CEF3_关闭全部实例()`（包装原私有 `CEF3_关闭全部()`）；弹窗/区域实例登记进 `cefBrowsers_`（合成 controlId=1000000+实例编号，与设计器控件空间隔离，事件按 user_token 路由），故枚举/关闭全部自动覆盖。**CEF3 per-browser UA（实例级 UA）已落地，纯运行时、无需重编桥**：新增 `CEF3_设置用户代理(控件名, 用户代理)` / `CEF3_设置实例用户代理(实例编号, 用户代理)` / `CEF3_取用户代理` / `CEF3_取实例用户代理`——置非空 UA 点亮 `LB_CEF3_ResourceRequestHandlerSubscribeBeforeResourceLoad`，运行时在「资源加载前」事件里对桥 `RegisterRequest` 出的活请求句柄 `packet->subject` 调 `LB_CEF3_RequestSetHeaderByName(..,"User-Agent",..,1)` 逐实例改写请求头（CEF 无 per-browser settings，请求头改写即官方范式）。另公开 `CEF3会话_取上下文实例(实例编号)` 让弹窗/区域复用全部句柄版 `CEF3会话_*` Cookie/会话命令。FBro 侧 `浏览器外壳_设置实例Cookie` 透传 `HttpOnly/Secure/Domain/Path`（host `LingBuilderFbroProcessRuntime.hpp:1342` 已支持，无属性向后兼容），新增 `浏览器外壳_新建独立实例代理(稳定ID,地址,标题,缓存目录,代理地址,用户代理)`（内部抽 `…新建独立实例内部`，start 时写 `config.userAgent/proxyServer`）。fbro-shell 1.4.0 再补内嵌区域动态（与 EdgeView/CEF3 创建区域同口径、不改桥）：`浏览器外壳_新建内嵌实例区域(稳定ID,左,顶,宽,高,地址,缓存目录,代理地址,用户代理)` 复用 `LING_FBRO_PROCESS_EMBEDDED` 把独立进程浏览器作为 WS_CHILD 挂到宿主窗口指定矩形新建的承载子窗，可同屏内嵌多个各自独立 profile/代理/UA、随主窗口 WM_SIZE/WM_DPICHANGED 由 `LB_NE_UpdateBrowserShellRegionBounds` 按各自矩形跟随；`浏览器外壳_取内嵌区域实例JSON()` 枚举；区域实例带 `regionEmbedded`、从多标签视口状态机跳过但仍 shellManaged 可被 关闭实例/销毁 覆盖。附带修复：`smoke-new-emoji-fbro-browser-shell.ts` MSBuild 路径改 vswhere 解析；修正 `FBro框架_遍历DOM/遍历_按路径设属性/按路径赋值/会话_创建上下文` 5 处 `调试输出(...).c_str()` 括号错位（C2228/C2672，曾使任何 new_emoji FBro shell 工程编不过）。全局清单指纹 commands 3694 / parameters 6445 / controlReferences 1307 / commandDigest 73ff26f9 / parameterDigest b8780e80；`MODULE_ENCAPSULATION_CHECKLIST.md` 计数与 CEF3 文档用例 publicCommands 411 同步。CEF3/FBro/EdgeView 清单+绑定+生成 C++ 回归用例绿；**CEF3 真机原生构建 + 运行冒烟已 PASS**（真实 MSVC + `lingbuilder.cef3.sdk` CEF 150 SDK + LingBuilderCefBridge.lib，编译调用 弹窗/区域/实例UA/取上下文实例/枚举/关闭全部 的工程 `ok:true`、exe 启动后 CEF 进程树存活 >8s）——该真机编译暴露并修复了 `CEF3_枚举实例JSON` 手写转义/拼接把 `L'\n'`/`\"` 写进 TS 模板字面量被吞成真实换行/引号的生成器缺陷（改 `wchar_t(0x5C)`/`0x22` 无斜杠写法）。FBro 原生 exe 早前卡在 `LNK1181`——已查明并非环境而是 `lingbuilder.fbro.browser` target.libs 自带 `modules/<id>/` 前缀、被 VS 导出器 `getModuleLibFiles` 再前置一次成双段路径（模块 target.libs 约定应为模块根相对，如 `lib/x64/...lib`，与 `new_emoji.ui`/`runtimeFiles` 同）；改正后 FBro 浏览器外壳工程编译零 C++ 错误 + 成功链接 + exe 启动真实创建伴随宿主窗口与 Chromium 子窗口（其外壳顶栏 DPI 几何冒烟断言是 new_emoji 布局另一议题，不属多店铺能力）。

## 分类内置模块库（2026-07）

- 参考精易模块的程序、窗口句柄、键盘鼠标、进程线程、配置、图片、网页、文本字节、文件目录、系统、杂类和组件分类，新增 51 个内置 v2 模块、340 条中文命令。
- 当前 `BUILTIN_MODULES` 合计 101 个模块、3928 条命令；正式清单位于根目录 `MODULE_ENCAPSULATION_CHECKLIST.md`。
...
- 全量门禁命令 `npm run module:control-ref-audit` 当前审计 109 个模块、7945 个方法、19007 个参数和 5103 个 controlRef 参数（含本机已装磁盘模块，换机需重装后重跑），并额外扫描 63 个模块 TypeScript 源文件中的原始 `insertText`、`example` 和 snippet 字面量；方法/参数摘要防止未来只抽样覆盖或只靠运行时归一化掩盖旧写法。
...
## CEF3 无头浏览器与 OSR 子模块（2026-09-20）

CEF3 无头/OSR 批次（提交 44e34d5..53f0591，Task 1-11）落地的模块与登记口径：

- **新模块 `lingbuilder.cef3.osr`**（CEF3无头渲染模块）：`lingbuilder.cef3.browser` 的子模块，与其它 CEF3 子模块同源版本（`CEF3_ALPHA_VERSION`），`dependencies` 引用核心，target `windows-msvc-x64`，manifest v2 `contributes.commands` + `bindings.commands` 同批登记 5 条 advanced 命令（`CEF3OSR_请求重绘` / `CEF3OSR_设置窗口外帧率` / `CEF3OSR_取窗口外帧率` / `CEF3离屏_订阅像素帧` / `CEF3离屏_订阅视图矩形`），全部按浏览器句柄寻址。命令名与 `scripts/generate-cef3-api-coverage.cjs` 权威映射表逐字一致（`coverage:check` / `catalog:check` 门禁）。定义在 `electron/src/services/modules/cef3Modules.ts`。
- **设计器非可视资源「CEF3无头浏览器」**（`CefHeadlessBrowser`，模块 `lingbuilder.cef3.browser`）：属性 实例编号/打开地址/独立缓存目录/独立代理地址/视口宽/视口高/窗口创建期自动启动，字段命名与 EdgeView 无头组件同口径（`instanceId/url/cacheDir/proxyServer/viewWidth/viewHeight/autoStart`）；`events` 为空（一期不派发事件），`requiredLibraries` 只带 `LingBuilderCefBridge.lib`。
