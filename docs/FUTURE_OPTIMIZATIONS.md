...
- 已完成（2026-09-24）：**模块详情「示例」页签接入随包例程（module-demos），一键在新 IDE 窗口打开**。此前 `examples/module-demos/` 的 89 个演示工作区只在开发仓里（`examples/` 不在 electron-builder 分发清单，见 2026-09-19 条待办①），且示例页签只显示模块清单 `contributes.examples[]` 声明的文档示例，多数模块为空。落地：① 打包——extraResources 新增 `../examples/module-demos → resources/module-demos`，filter 剔除 `模块命令清单.json`（MCP demoExample 语料，约 8MB）与 `README.md`，实际随包约 3.3MB 纯文本（NSIS 压缩后安装包增量约 1MB 内）；② 主进程 `electron/electron/moduleDemoService.ts`（纯 Node 可测）：枚举随包例程（只认 `src/MainWindow.lcpp` 工作区结构）、`prepareModuleDemoWorkspace` 复制到 `文档\LingBuilder 例程\<模块ID>`（copyMissingFiles 只补缺不覆盖=复用口径，保留用户改动、自愈半份拷贝，与 `ensureBundledToolchains` 同语义）、`buildModuleDemoSpawnPlan` 以 `--workspace` + `LINGBUILDER_REC_USER_DATA`（`<userData>\module-demo-runtime\<模块ID>`）拉起新 IDE 进程——独立 userData 绕开单实例锁，是「新开一个 IDE」而不是现有实例切工作区，例程实例的最近工作区/设置不污染主实例；IPC `modules:list-demos`/`modules:open-demo` 注册在 main.ts modules 组。③ 渲染层——`modulePublicInfo.collectPublicInfoItems` 新增可选 `bundledDemoModuleIds`，命中模块在「示例」分组补一条 `demoModuleId` 条目（清单 examples 与例程共存，一个=引用写法、一个=可运行工程）；`ModuleDetailPage` 示例页签渲染例程卡（「可运行」徽标 + 「在新 IDE 窗口打开」按钮，opening/ok/error 就地反馈），嵌入模式（主区页签）与 standalone 独立信息窗口同一实现都可用；demo 条目从 `ModuleDocumentPreview` 的 `declaredDocuments` 剔除（防止文档阅读器把模块 ID 当文档路径去读 404）。执行路径说明：打开例程是「新开工作区」类动作、无模块启停/购买门禁，故嵌入模式也由详情页经 `moduleDemoService` 直连 IPC 并就地反馈，不走 `MODULE_DETAIL_ACTION_EVENT`→ModuleInspector 模块动作出口（该单一出口红线覆盖的是收费门禁类动作；已在代码注释与本条记录依据）。回归：`tests/moduleDemoService.test.ts`（已接入 `test:lingcpp`：ID 校验拒路径穿越、枚举只认工作区目录、全新复制/复用保留用户改动/半份拷贝自愈、spawn 计划开发态显式 app 路径 + 独立 userData、公开信息注入与共存、extraResources 配置断言）；`tests/modules.test.ts` 详情页断言补例程正则，`src/services/modules` 源文件计数门禁 61→64（含并行会话在途 2 文件，全量字面量扫描 0 违规）按当前工作区实算写回。遗留：例程语料停在 2026-08（`npm run module:demos` 可收敛）；new_emoji 例程是 1.8MB 穷举生成代码，对新手不友好，后续应换人工提炼的惯用写法小例程；收费模块（FBro VIP 等）例程打开后仍受权益门禁，属预期；开发态 `LINGBUILDER_REC_USER_DATA` 拉起的例程实例与录制隔离共用机制，若父实例本就是录制实例会被覆盖为例程路径（预期）。

- 已完成（2026-09-24）：**字符串字面量区间扫描统一化 + 拼接表达式 `\"` 缺陷根治 + 无法翻译表达式生成端加固**（用户实测缺陷报告《灵码缺陷_拼接表达式转义》，基线 0.7.8）。根因链：`eplToCppRules.ts` 的 `splitEplBinaryExpression` 从后往前扫描时引号状态机只判 `char === quote`，被转义的 `\"` 被误判为字符串结束、引号奇偶翻转、顶层 `+` 找不到，`translateLingCppExpression` 各分支全部落空后走到 `return trimmed;` 兜底——整句中文源码被当 C++ 吐出（MSVC C2065，且存在「恰好能编译」就静默错行为的升级风险）。修复三层：① 新建 `electron/src/services/lingCpp/stringLiteralRegions.ts`（`collectLingCppStringLiteralRegions`/`buildLingCppStringLiteralMask`，正向扫描、`\"` 与连续反斜杠奇偶正确、中文引号 `“”` 不解释转义、`closed` 标记未闭合），`splitEplBinaryExpression`/`splitCallArguments`/parser 的 `splitParameterParts`/`splitParameterDefault` 全部改为复用掩码，`splitCallArguments` 行为逐字符等价（空实参丢弃、深度跟踪、引号字符保留均不变），**四个解析器不得再各写引号状态机**；② 生成端 `return trimmed;` 兜底加 `isPlausibleCppPassthroughExpression`（`eplToCppRules.ts` 新导出：无任何字符串区间、字符全部为标识符字符或 C++ 表达式安全符号才放行，`甲[0]`/`-1` 等既有透传形态不受影响）拦截，不放行的降级为 `L"" /* 无法翻译的表达式：<原文> */`，并由新增 `collectUntranslatableExpressionDiagnostics`（main.cpp 标记扫描 + sourceMap 取「生成行之前起点最近」条目）产出 `<源文件> 第 N 行：表达式「…」无法翻译成 C++，已按空文本降级…` 的**阻断诊断**，`build.run`/F5 在 MSVC 之前给出中文原因；③ 语言服务新增 `getUnterminatedStringLiteralDiagnostics`（`lingcpp-string-unterminated-*`，error 级、带 `range` 行列，跳过多行文本块/`@` 行/注释），未闭合字符串在编辑器与 `lingcpp.diagnostics` 可见。回归：`tests/stringLiteralRegions.test.ts`（8 条：区间扫描含 `"a\\"`/`"a\\\\"`/`"a\\\""` 奇偶、缺陷原形拆分、透传判定、形参默认值含 `\"` 与字符串内 `=`、端到端 5 用例生成 `LBFL_` 调用且零阻断、兜底降级 + 第 6 行阻断、未闭合字符串双通道诊断），已接入 `test:lingcpp`。真机：转义拼接复现.lcpp 原样放进真实工作区（createProject → buildRun 走 AiBridgeService 全链）MSVC 编译链接通过、产物 5 用例全为 `报告+LBFL_转义拼接复现_收(L"...")` 且零阻断；负例（`键文本 = 甲["键"]`）构建在编译前被中文阻断；LB-PDD 克隆工作区「拼多多店铺客户端」（6 个代码 .lcpp、零内嵌 C++）补齐 solution.json/模块清单后 `buildRun` 通过、**MSVC 0 警告 0 错误**。既有失败基线不变：`test:lingcpp` 235 项 231 过 4 失败（aiSecurityAccessibility×2、designerProfessionalUi×1、sdkDependencyService×1，均不引用本次改动模块，属既有源码漂移与本机缺 CEF3 SDK 发布包）。剩余风险：① `\"` 在生成 C++ 里落为 `\\\"`（`interpretLingCppStringEscapes` 不解释 `\"`、`escapeWideString` 再转义反斜杠）——用户想要「内容含引号」会得到「反斜杠+引号」，属**既有转义解释器口径**，本批不动，后续可评估把 `\"` 加进解释表；② 中文引号区间按「第一个 `”` 闭合」（与既有 `splitCallArguments` 语义一致），与生成端 `chineseQuoted` 贪婪到最后一个 `”` 存在口径差，含 `”` 的中文串在实参拆分位会拆错，后续可统一为「最后配对」；③ sourceMap 的 `findGeneratedMethodBoundary` 对部分 ASCII 事件名会匹配偏移（语句条目缺失），阻断诊断已用「起点最近条目」兜底到事件/类行，根治需修边界匹配；④ `:30249`/`:30372` 的注释兜底（暂不支持的控制流/语句）保持注释形态——不会生成坏代码但也没有中文诊断，后续可挂 `lingcpp-*` 提示。

- 已完成（2026-09-23）：**易语言类型转换初级命令批次 1——`到长整数`/`到小数`/`到单精度小数`/`到逻辑` 入账**。背景：`lingbuilder.win32.basic` 只有 `到整数` 一条文本转数值命令，超过 32 位的数值（时间戳、文件大小、地址）一转换就被截断，也没有文本转小数/逻辑的官方路径。四条命令一律单个 `文本`（`wideString`）参数、与 `到整数` 同款「空文本或无法转换返回 0/假」降级语义，本期**不做数值重载**（传数值实参需先显式 `到文本(数值)`，与 `到整数` 行为一致，已实测两者发射形状相同）：`到长整数` 返回 `long long` 且不做 32 位截断、`到小数`/`到单精度小数` 返回 `double`/`float` 并忽略首尾空白、`到逻辑` 按「空/“0”/“假”为假，非零数值与“真”为真，其余非空文本也为真」判定。落点四处齐全：`builtinModules.ts` contribution + binding + `BUILTIN_PARAM_DOCS` 命令级参数说明、普通 Win32 类成员与 new_emoji 自由函数**两套** C++ 运行时（`lingCppWin32Project.ts`，缺一处另一条生成路径就是假成功）、`NEW_EMOJI_WIN32_BASIC_COMMANDS` 后端契约白名单、`eplTokenizer`/`MonacoCodeEditor` 关键字表补 `到长整数`/`到单精度小数`（`到小数`/`到逻辑` 早在表里，属既有「假高亮」转正）。计数门禁写回：内置 controlRef 审计 101/3924/6882 → **101/3928/6886**（controlReferences 1345 不变），摘要 `bfedd0db`/`d4925a9e` → **`dc1e2241`/`9581befc`**；合并本机磁盘模块全量审计 7941/19003 → **7945/19007**（摘要 `d8d4982c`/`6da26058`）；`MODULE_ENCAPSULATION_CHECKLIST.md` 计数行 3924→3928（win32.basic 行 216→220）；本次核账修正一处我先前写错的值：该行「参考精易模块分类新增 51 个模块、N 条命令」**不含 win32.basic**（它在清单的「已有模块」表内），故该组数字保持 336，不随本批变化；另把总计数按当前注册表实算写为 **3938**（含并行会话 std.text/fs.core 批次的 +10），该组的行数 24→29、36→41 与组内合计 336→346 及 `tests/modules.test.ts` 内对应字面量须由该批次同步（属其改动范围，本批不动其文件）。`module:control-ref-audit` 复跑 109 模块/7945 方法/19007 参数/5103 controlRef、源字面量 63 文件 0 旧写法。验收：新增 2 条自动化用例（清单/binding/返回类型推断/补全 + 两后端生成真实 C++ 的逐行运行时断言），`test:lingcpp` 1234 项 1204 过 30 失败——**失败数与「本批改动回退后」的基线完全一致（均 30 条）**，唯一差异是一条 `ListView 完整数据接口…` 用例：它断言生成 C++ 含 `LingCppTextValue operator+(const wchar_t* value) const`，而并行会话在途的 `struct LingCppTextValue : std::wstring {…}` → `using LingCppTextValue = std::wstring;` 重构（修 C2666 重载二义）已把该字符串从生成器彻底移除——**该字符串当前源码树出现 0 次、HEAD 出现 8 次**，故该断言在重构在途期间对任何生成结果都不可能成立，与本批无关（本批为纯插入）；其余 29 条为既有基线；lint 与 `npm run build` 全绿。**归因教训**：用 `git checkout --`/`git show HEAD:` 做对照基线会把并行会话在同一文件里的在途改动一并回退，导致错误归因；多会话共用仓库时应以「当前工作区内容 − 本批次改动」为基线，并用可判定的静态证据（如被断言字符串存在性）逐条归因。真机全链（AI Bridge CLI stdio MCP，独立 Downloads 工作区）：`project.create` → `edit.propose` → `edit.apply` → `build.run` → `run.wait` → `run.log`，控制台程序逐条打印转换真值并全部符合预期——`到长整数("9223372036854775807")`=9223372036854775807、`到长整数("4294967296")`=4294967296（超 int32 未截断）、`到长整数("")`=0、`到长整数("abc")`=0、`到逻辑("")`/`("0")`/`("假")`/`(" 0.0 ")`=假、`到逻辑("真")`/`("1")`/`("yes")`=真、`到小数(" 3.25 ")`=3.25、`到单精度小数("0.1")`=0.1；窗口项目写 `信息框(格式化文本(...))` 构建后 exe 启动 3 秒仍在运行（模态阻塞符合预期）后正常停止。**批次 2 冻结未动**：`到字节集`/`取字节集数据`/`到文本` 字节集重载有两个待拍板决策点（文本入参走编码还是拒绝、越界返回语义），`到时间`/`到时刻` 同批冻结（现有 `时间_从文本` 已覆盖）；`字节集_从文本` 语义是「编码文本→字节」而非「内存原始字节」，**禁止改名或别名成 `到字节集`**。**未推送外部 AI 消费层**：本批纯补命令清单、不改工具形状，`MCP_INSTRUCTIONS` 措辞层按判断暂不做（`dist/cli.cjs` 已为真机验收重建，只含新命令不含新条款），下次推外部 AI 指引时再评估四条命令是否需要独立条款。遗留：`examples/module-demos` 逐命令演示语料停在 2026-08-07（206 条命令 vs 当前 220），`demoProject.stale` 已能自证新鲜度，重跑 `npm run module:demos` 可收敛（会成批改 `exports/` 源码包，属预期）。
- 已完成（2026-09-23）：**EdgeView 构建缺 WebView2 SDK 从「静默编桩 + 误导报错」改为「随包分发 + 构建阻断 + 环境检测明示」三件套（外部用户 aruile-toolsbox 实测反馈）**。根因：生成的 main.cpp 以 `__has_include(<WebView2.h>)` 决定 `LINGBUILDER_EDGEVIEW_AVAILABLE`，SDK 只从 NuGet 全局缓存（`%USERPROFILE%\.nuget\packages\microsoft.web.webview2\1.0.4078.44`）解析且缺失时仅记一条非阻断诊断继续编译——新用户机器没有 NuGet 缓存，全部 EdgeView 命令被编译成空实现桩，运行期却统一报「当前 WebView2 Runtime 版本不足；v1 需 141，v2 需 150」，误导用户去装 WebView2 Runtime（装多少遍都无效）；环境检测又只查 Runtime 注册表给 ✓，双重掩盖。修复：① **随包分发**——`scripts/prepare-webview2-bootstrapper.cjs` 打包前从本机 NuGet 缓存物化 4 个实际消费文件（WebView2.h + WebView2EnvironmentOptions.h + x64/x86 WebView2LoaderStatic.lib，23.2MB 原始 / NSIS 安装包增量约 4~6MB）到 `electron/third_party/webview2/microsoft.web.webview2/<版本>/`（保持 NuGet 目录布局，`electron/third_party/` 整目录 gitignore 外、打包机预置，与 aria2/sqlite/excel 同口径），经 extraResources 落 `resources/third_party/webview2/`；`package:dir` 链补挂 `prepare:webview2`，缓存缺失或 WebView2.h SHA-256 不符直接打包失败。② **SDK 定位唯一出口** `src/services/modules/webView2SdkModule.ts`（`locateWebView2SdkPackage`：`LINGBUILDER_WEBVIEW2_SDK_ROOT`（兼容包仓库根/版本父目录/包根本身三种指向）→ 随包副本 → NuGet 缓存），`materializeEdgeViewSdk` 消费之，缺失/哈希不符升级为 `addBlockingDiagnostic`（对齐 CEF3 SDK 缺失口径），不再产出空实现 exe。③ **桩文案改口**——`#else` 世界新增 `EdgeView_报告未编译`：「EdgeView 命令未编译进本程序…构建时缺少 WebView2 SDK…与已安装的 WebView2 Runtime 版本无关，安装 Runtime 无法解决」，与真实实现里 `EdgeView_报告接口缺失`（真 Runtime 接口缺失，保留 141/150 文案与下载地址）区分；`cl -Zs` 对桩世界做过语法编译验收。④ **环境检测**新增 `webView2Sdk` 检查项（`/api/environment/check` 展示为「WebView2 SDK（EdgeView 构建）」，缺失进警告区）。用户侧应急：从任意装有机拷 `microsoft.web.webview2\` 缓存目录到 `%USERPROFILE%\.nuget\packages\` 后重新 F5。回归：`tests/modules.test.ts`（缺失阻断断言、定位器三候选用例、桩文案与 hint 计数断言）、`tests/environmentCheck.test.ts`（SDK 检查项/警告/非 Windows 不发命令）。遗留：SDK 尚无自动下载通道（nuget.org flatcontainer + SHA 钉扎可作后续项）；新打包机必须先备钉扎版本 NuGet 缓存或设 `LINGBUILDER_WEBVIEW2_SDK_ROOT`，否则打包链在 `prepare:webview2` 处失败（预期行为）。
- 已完成（2026-09-23）：**外部 AI「认错项目」消歧三件套（workspace.list projects[] / MCP 第 19 条 / projectHint）**。外部 AI 经 MCP 修改多项目工作区时只能按目录名猜目标项目（默认项目 `src/` 占位与各项目 `src/<id>/` 嵌套并存、同名 MainWindow.lcpp 常见），猜错后 propose/apply/build 全链路静默成功（`resolveWindowProjectForApply` 的 sourceRoot 前缀匹配只选门禁上下文、不质疑文件该不该改）。三层修复：① `workspace.list` 首项新增 `projects[]`（id/name/sourceRoot/type/isWindowProject，只读注册表视图）与 `startupProjectId`，数据经 solutionService 新增的 `peekSolution()` 只读读取（`getSolution()` 在空工作区会物化默认项目，读接口禁用，回归断言钉住）；② `MCP_INSTRUCTIONS` 新增第 19 条（动手前先用 projects[]/solution.json 确定目标项目、多项目必须显式传 projectId、禁止按目录名猜），workspace.list / lingcpp.diagnostics / edit.propose 工具描述同步；③ 单项目工作区的 `getLingCppDiagnostics` / `proposeEdit` 响应自带 `projectHint`（projectId/name/sourceRoot + 中文说明），多项目不出（没有唯一答案不给假答案）。遗留（待评估）：propose/apply 仍不校验「文件属于哪个项目」——外部 AI 若拿到错误路径依旧静默写入，未来可在 apply 侧按 sourceRoot 归属做警告级提示；`lingcpp.diagnostics` 裸文件名报错文案仍指工作区根（见下条第 8 行缺口的后半段）。回归：`tests/aiBridge.test.ts`（projects 视图/不物化/projectHint/第 19 条关键词/工具描述断言），重建 `dist/cli.cjs` 后真机 stdio 握手 7/7 PASS。
- 已完成（2026-09-23）：**MCP/AI Bridge 对外暴露 IDE 版本号**。历史上 MCP 握手 `serverInfo.version` 是硬编码 `0.6.6`（与 `electron/package.json` 已脱节），`health()` 与 `workspace.list` 均不带版本，外部 AI 无法做兼容性判断或在问题报告里自证环境。现在解析唯一出口是 `src/services/aiBridge/ideVersion.ts`：`LINGBUILDER_IDE_VERSION` 环境变量（主进程在 `aiBridgeManagerService` 与 `agentRuntime` 两条托管启动链注入 `app.getVersion()`；内嵌 Agent 经 overlay `env` 块透传给 MCP 宿主）→ 就近 package.json 逐级向上回退（源码树 / dist bundle / 打包 asar 均命中）→ `0.0.0` 显式未知兜底；消费点三处：`health().ideVersion`、`workspace.list` 合成根条目 `ideVersion`、`serverInfo.version` 动态取值（`MCP_INSTRUCTIONS` 第 15 条与 workspace.list 工具描述同步提及）。`electron/electron/agentRuntime` 因 tsconfig `rootDir` 不能 import `../src`，只镜像环境变量名，由 `tests/agentRuntime.test.ts` 常量逐字一致用例把关。回归：`tests/aiBridge.test.ts`（health/serverInfo/根条目/解析单测）。
- 已完成（2026-09-22）：**AI 面板引擎收敛为「本机 Agent」单通道**。面板不再有「系统 AI」「自定义 API」两个入口（连同 `cloudAi` 事件订阅、BYOK 聊天/编辑表单、面板侧意图判定谓词与 `aiConnectionSessionService.ts` 一并物理删除），模块页「AI 生成模块」改经 `src/services/ai/agentRequestBus.ts` 把需求交给同一引擎。**保留清单与后续可做的口子**：① 云端系统 AI 与点数计费、`/api/ai/chat|connect|models`、`/api/lingcpp/edit/propose|from-system-draft`、`panelAiBridgeService`、`aiProviderService` 四协议实现全部原样留着（CLI、外部宿主、打包冒烟与面板「模型通道」探测仍依赖），因此**若将来要在 IDE 内恢复「云端系统 AI 对话」，需要重新接回的只是渲染层那一段订阅与表单**，服务侧无需重建；② `deriveAiNewFileAllowance`（从指令推导新建文件白名单）与 `/api/ai/chat` 现在没有面板调用方，属「端点存活、入口下线」，清理前必须先决定云端系统 AI 是否长期只服务 CLI；③ 模块一键生成的三阶段提示词链（`aiModuleGeneration.ts` + `POST /api/modules/ai-generate`）已删除，若实测本机 Agent 封装模块的成功率不及原链路，恢复方向不是重做双通道，而是把《AI 模块开发规范》做成 Agent 可读资源（例如经 `module.info` 或随包规范文件下发），让它在一轮会话里按规范自查；④ 面板「停止」当前语义是回收内嵌运行时（dsh 没有单轮取消协议），下一条需求会自动重启——真正的中途打断需要 dsh 侧提供 cancel 语义。回归口径见 `tests/aiAgentPanel.test.ts` 与 `docs/AI编辑链统一架构.md`「AI 面板引擎收敛」节。

- 已完成（2026-09-22）：**`lingbuilder.win32.basic` 新增「当前窗口自身」三条免句柄命令，根治纯 Win32 窗口项目取不到自身 HWND**。背景是外部 AI（DeepSeek Harness 经 AI Bridge MCP）实测「给按钮加点击计数并改窗口标题」时暴露的结构性洞：`窗口_设置标题` 只在默认未启用的 `lingbuilder.win32.window-utils` 且首参必须是 HWND，而全生态没有任何命令能产出「自己这个窗口」的 HWND，AI 只能退化成 `窗口_按标题查找("设计器标题")` 反查——用户在画布改一次标题功能就静默失效。新增：`窗口_取自身句柄()`（长整数型，唯一自身 HWND 入口）、`窗口_取自身标题()`（文本型）、`窗口_设置自身标题(标题)`（逻辑型，改标题免句柄直调）。两个 UI 后端都有真实运行时实现（Win32 走 `LingWindowBase` 成员用 `hwnd_`，new_emoji 走 `g_newEmojiWindow` 自由函数），并同步登记进 `NEW_EMOJI_WIN32_BASIC_COMMANDS` 契约白名单。**外部 AI 四层消费面已同步**：`MCP_INSTRUCTIONS` 第 17 条（含「禁止用 窗口_按标题查找 反查自身句柄」红线，`tests/aiBridge.test.ts` 三条关键词断言 + 重建 `dist/cli.cjs` 后真机 stdio 握手验收到发，instructions 17493 字节 < dsh 的 32768 上限）、`module.info` 命令 description、`LingBuilder AI 规则手册.md` 新增「当前窗口自身句柄与标题」节 + 外部客户端节、模块随包文档 `docs/modules/win32-basic/README.md` 新增同名小节。计数门禁写回：内置 controlRef 审计 101/3921/6881 → **101/3924/6882**（controlReferences 1345 不变），摘要 `ea0277d6`/`b13a9f02` → **`bfedd0db`/`d4925a9e`**；`MODULE_ENCAPSULATION_CHECKLIST.md` 计数行 3921→3924、win32.basic 行 213→216；合并全量审计按本机实算写回 7941/19003（含并行会话在途磁盘模块漂移，归因已注在测试注释）。验收：`tests/modules.test.ts` 生成断言（三条真实运行时符号 + `窗口_设置自身标题(LingCppWideArg(格式化文本(...)))` 调用形状）+ 真机 MCP 全链（`edit.propose` 纯源码 → `edit.apply` → `build.run` 编译成功 → 启动 exe → ctypes 投 `BM_CLICK` 三次，标题 `已点击 1/2/3 次`，`run.log` 打出 `自身句柄 = 7078342` 与窗口枚举 HWND 逐位一致）。**待办**：官网 `/commands` 页需重跑 `npm run module:web-sync`（要管理员凭据，未执行）。**后续可做（A2 方案，本批未做）**：把 `当前窗口` 关键字扩展到 `handle` 参数位（`lingCppWin32Project.ts` 已有只对 `NE_/EU_` 生效的 `当前窗口`→`g_newEmojiWindow` 特例可作起点），使 `窗口_设置标题(当前窗口, ...)` 对全部模块命令成立；代价是 binding 值类型、补全、诊断、生成器与 controlRef 门禁口径全链路，且必须重新划清 `handle` 与 `controlRef` 的语义边界（AGENTS 明令 controlRef 不得伪装成 handle），属独立批次。
- 已修复（2026-09-22）：**系统 AI 编辑提案按指令关键词误拒纯行为需求（「AI 返回的设计器模型与当前模型完全相同」）**。`proposeLingCppEdit` 的 strict 分支有两处 `throw`（`:65` 指令含布局词但 AI 未回设计器模型、`:77` 回了但与当前模型等价），判据都是 `isDesignerEditInstruction` 的关键词表（`窗口|按钮|显示|移动|文本框|…`）——「给按钮1加上点击计数…把窗口标题改成已点击 N 次」必然命中，于是纯 `.lcpp` 行为改动被结构性挡死（用户在 AI 面板实测到的红字即此）。修复改为**唯一事实判据**：把提案落盘后的源码（按 apply 的同一 `replaceRange` 顺序回算）跑 `controlRef` 语义诊断，引用了模型中不存在的控件才中文阻断并指向 `updatedDesignerProject`，否则按纯源码提案受理并丢弃 AI 原样回传的等价模型。收口为单一实现 `src/services/lingCpp/controlReferenceAdmission.ts`（`collectControlReferenceAdmissionProblems` + `formatControlReferenceAdmissionBlock`），propose / apply / build 三入口共用，`AiBridgeService.assertControlReferencesInDesigner` 改为委托它，不再有两套控件引用校验。「美化界面」类宽泛视觉请求的本地产出可预览布局降级保持不变（既有两条用例仍绿）。回归：`tests/lingcpp.test.ts` 新增「纯行为需求不再误拒（等价模型与缺模型两种形态）」「引用不存在控件仍按同一门禁中文阻断」两条；边界细则同步 `docs/AI编辑链统一架构.md`、`LingBuilder AI 规则手册.md` 第 7 节。**遗留**：`caller-draft`（外部 AI 自带草稿）路径仍完全宽松，缺控件由 apply/build 门禁兜住，这是 AGENTS 既有口径，不改。
- 待办（2026-09-22，P1/P2 与随包分发均已落地，仅剩 Harness home 隔离）：**AI 面板内嵌 Agent 运行时（DeepSeek Harness）**。已完成：`electron/electron/agentRuntime/` 三文件（profile 解析与 overlay 生成 / 自写换行分隔 JSON-RPC 客户端 / 生命周期服务）+ 主进程 IPC `agent-runtime:*` + 三层安全边界（dsh 15 个本地工具行整行禁用、MCP `--mcp-toolset agent` 摘掉写盘与执行类工具、面板代执行 apply/build）+ `tests/agentRuntime.test.ts` 与 `tests/aiBridge.test.ts` 回归 + 真机端到端（真实 dsh 起停、只调 `mcp__lingbuilder__*`、子进程无残留）。**剩余四项**：① ~~P2 面板 UI~~ **已落地（同日 P2）**：`AiConnectionMode` 增 `'agent'`、面板第三页签「本机 Agent」、`runAgentTurn` 提交需求并经 `GET /api/lingcpp/edit/agent-proposal` 取回提案交给现有预览/应用/撤销 UI；跨进程提案经 `.lingbuilder/agent-proposals/` 交接（提案 ID 正则校验、30 分钟过期、最多 20 条、apply 成功即删）。真机验收：agent 工具序列 `workspace_list → file_read → edit_propose`、提案前源文件不变、另一进程 `applyEdit` 按 ID 成功、交接目录清空、被遮蔽工具零调用。**P2 剩余三项已同日收尾**：事件流渲染为聊天区顶部折叠的「本轮 LingBuilder 工具调用」逐条卡片（步骤状态不入会话 store，卡片显式标注写盘与构建不在此列）、运行时状态常驻指示行（消费 `agent-runtime:status`，含未启动/启动中/待命/执行中/停止中/启动失败与 Node 版本、模型、PID，启停按钮走同一 IPC，失败直显 `problem`）、「拒绝提案」即时删除交接目录对应文件（`/reject` 路由 `deleteAgentProposal`，30 分钟过期降级为兜底）。② **随包 Node 分发已落地**：`scripts/prepare-agent-runtime.cjs` 默认随包（`=0` 关闭），钉 Node 24.20.0 win-x64（按 SHASUMS256 校验、官方站优先镜像回落），落 `resources/node/`；取不到运行时只降级为占位目录 + manifest `problem`，不阻断出包。③ **dsh 发行包分发与版本 pin 已落地**：钉 `@deepseek-ai/dsh@0.1.6-alpha.2` 以 `--ignore-scripts` 装进 `resources/dsh/`，版本 pin 在脚本常量并记入 `agent-runtime.json`；装后裁剪 `@deepseek-ai/libreoffice-kit-win32-x64`（325MB，`dsh-office-to-pdf` 的 LibreOffice 原生包，内嵌场景永不使用、只在真正转换时惰性解析），随包增量收敛为 Node 101.8MB + dsh 225.6MB。门禁 `npm run verify:agent-runtime`（source）与 `verify:agent-runtime:unpacked`（打包后）除核对文件/manifest/裁剪一致外，必须**用随包 node.exe 真跑一次 `dsh --version`**——只查文件在不在不足以证明可用（Electron 当 Node 会整树导入失败）。真机复验：清空 PATH、不设任何 `LINGBUILDER_DSH_*` 覆盖，随包副本跑通「dsh 起 → 仅 MCP 工具 → edit.propose 交接 → 另一进程 apply → 交接目录清空」。dsh 官方仍标注 developer preview、会有破坏兼容，升级 pin 版本必须重跑该真机链。剩余：**④ Harness home 隔离**：当前复用用户本机 `~/.dsh` home（沿用其 provider 与凭据，与外部客户端同权），`LINGBUILDER_DSH_HOME` 预留未启用；隔离需同时解决 provider 凭据来源（BYOK 写入独立 home 的凭据存储或 `llm-pi-ai` provider 行）。另两条实测暴露的产品缺口待评估：内置中文语法范本（`如果/否则`）没有只读可查入口，外部 AI 会绕道用 `project.create(approved=false)` 当语法探针；`lingcpp.diagnostics` 传裸文件名报「工作区中不存在」且工作区里存在同名占位 `src/MainWindow.lcpp`，容易误取。
- 已修复（2026-09-22）：**CEF3 填表 12 条命令（CEF3填表_赋值/置选择框/置选择项/置内文本/置外文本/置内代码/置外代码/置属性/点击元素/聚焦元素/触发事件/滚动到元素）全部空转仍返回成功的桥级 bug**。根因在生成器 `lingCppWin32Project.ts` 的 `CEF3_填表_执行`（不在 C++ 桥 DLL）：拼装的 JS 把助手脚本声明和 `(0,…)` 落成两条语句——`(0,setValue,#username,…)` 是逗号表达式根本不会调用 `lbTianBiao`，且 `#` 选择器裸插成非法 JS 令整段脚本 SyntaxError，而 `LB_CEF3_FrameExecuteJavaScript` 只证明提交成功，12 条命令恒返回 1（ep15 填表演练真机暴露，file:// 与 https 页面同样失效；`CEF3框架_执行JS` 直传脚本不受影响）。修复照抄 EdgeView `EdgeView_填表执行` 的成熟形态（ep16 同病先修）：`(function(){return 助手})('op','selector',index,'a','b')` 单条调用表达式 + 字符串实参包单引号（拼接字面量转义单引号/反斜杠/换行）。回归：`tests/modules.test.ts` 填表用例新增调用形状断言（IIFE 存在 / 禁止 `L"(0,"` 伪调用 / 实参单引号），旧用例只钉了助手声明没钉调用串所以放过了本 bug。真机验收：CLI 重打 `dist/cli.cjs` 后无头重建 ep15 示例，stdout 断言 7 项全过（提交汇总 `林uilder / 杭州 / 已同意`、#notice 内文本、keydown 计数、页面标题翻 `-已提交`、`填表链路全部通过`）。**注意**：改生成器后必须 esbuild 重打 CLI bundle 并删示例 `.lingbuilder-build/<id>/` 整目录再重建；桥 DLL 无改动不需要 `module:cef3-bridge`。已知边界不变：填表命令是 fire-and-forget（返回 1 只代表脚本已提交，执行失败不回报），读取族仍未开放（`CEF3自动化_执行JS异步` 承担读取）。

- 已修复（2026-09-21）：**「控件_设置文本在创建完毕/列表框事件里不生效」双根因根治（外部用户反馈，Win32 基础+高级控件项目）**。根因一：自绘 BUTTON 家族（Button/CheckBox/RadioButton 按 `BS_OWNERDRAW` 自绘创建）的可见标题由 `PaintOwnerButton` 画 `ControlSpec.text`（生成期固化的设计器文本），`控件_设置文本` 只 `SetWindowTextW` 改 HWND 文本，画面永远画旧文本且命令返回 true（`控件_取文本` 读 GetWindowText 还能取到新值，进一步掩盖）。修复：`ControlSubclassProc` 对 BUTTON 家族拦截 `WM_SETTEXT`，镜像进 `RuntimeControl.hasOverrideText/overrideText`（复用 GroupBox 标题的成熟覆盖机制）并 `InvalidateRect`，`PaintOwnerButton` 两个标题绘制点改为优先消费覆盖文本。根因二：源码里手写 `_控件名_事件中文后缀`（如 `_列表框1_选择项被改变`）但设计器模型未绑定时，控件事件解析只读设计器绑定字符串 → 事件静默不派发（原防呆警告只覆盖 Button+被单击）。修复：`win32ControlRegistry.ts` 新增 `getConventionalControlEventBindings`（唯一命名出口），`generateControlArray` 把「设计器未绑定但源码已定义」的常规命名事件自动并入控件 events 派发串（设计器显式绑定优先、不重复），语言服务「尚未在设计器中绑定」提示与补全「（尚未绑定）」措辞同步豁免/改写；旧「未绑定 Click」启动警告已删除（被自动接线取代）。另把裸 `事件 创建完毕()` 的窗口 Loaded 解析改为按窗口自身源码类精确判定（`findWindowCreatedHandler`/`generateWindowSpec`，防多窗口同名误绑；注意 `findLingCppMethod` 的 `类名_方法` 前缀等价规则本就把裸创建完毕解析到 `_类名_创建完毕` 别名上，此为防御性补充而非主修）。验收：4 条生成级回归（自动接线/列表框复现/裸创建完毕/自绘镜像）+ CLI 无头项目真机验收——创建完毕改按钮文本立即显示「Ver 20260921A01」，真实鼠标点击列表「API管理」触发未绑定的事件、分组框标题变「API管理」。**遗留**：`模块源目录中的 controlRef 补全…` 用例的 62≠61 计数写回归新增模块文件的并行会话处理（协议见 modules.test.ts 注释）。

- 已完成（2026-09-21）：**聊天面板模块生成路由 + 系统 AI 编辑链新建文件白名单 + 云端 EDIT_DRAFT_INVALID 留痕（根治「生成模块被报 AI 未返回有效的完整文件修改结果」）**。故障链：`isLikelyCodeEditInstruction` 的英文关键词无词边界，`demo_add` 里的 "add" 子串把「生成 demo.math2 模块」误判成代码修改，请求进了云端编辑链；编辑链文件白名单只含已有工作区文件（最多 5 个），模块新文件必然被过滤成空，云端报 `EDIT_DRAFT_INVALID`。四层修复：① 英文关键词加 `\b` 词边界 + 新增 `isLikelyModuleGenerationInstruction`，模块生成需求路由到专用链；② 模块生成编排（清单→其余文件→补全三阶段 + 契约解析重试 + 导入 module-build）收口为 `src/services/modules/aiModuleGenerationFlow.ts` 唯一实现，聊天面板与模块面板共用，二进制产物不计入待补全文件；③ `AiEditRequest.newFiles`（`AiNewFileAllowance`：paths / directories+extensions+maxCount≤10）云端 `parseEditDraft` 与本地 `from-system-draft` 双层放行声明内的新路径，渲染层 `deriveAiNewFileAllowance` 从指令推导，本地以空基准进提案、写盘安全仍由 AiBridgeService 路径策略在 apply 强制；④ 云端保留真实 `EDIT_DRAFT_INVALID` 错误码（DB+事件）、解析失败把模型原始输出前 1200 字符写容器日志、错误信息区分「没给文件草稿」与「路径全被白名单过滤」。**边界与注意**：electron 根 tsconfig 未开 strictNullChecks，非 strict 下布尔判别式联合不收窄，结果类型必须用单一接口（`ok: boolean`+可选字段）；newFiles 校验在 `cloud/api/src/ai/ai.service.ts` 与 `src/services/ai/aiEditFileScope.ts` 各有一份同语义副本（两包不共享代码），改语义必须双侧同步。**追记（同日验收实测）**：用户在切换工作区后的 IDE 里验收时暴露第二层根因——switchWorkspaceRuntime 重建 25 个服务却漏掉 panelAiBridgeService（构造期快照 workspaceRoot），导致 AI 提案/应用读写旧工作区（写验收目录、读 electron 目录的「写读分裂」），已改为工厂 + 切换时重建。内嵌 AI Bridge 路由仍绑定启动实例，切换后同样存在旧根问题（开发环境默认关闭、桌面版切换走独立进程，暂记债务）。另：设计器画布模型仍是 localStorage 原型回退、不随服务端落盘自动持久化（apply 客户端守卫已改与「提案基线」比对绕开该缺口），画布→工作区文件的正规持久化迁移仍按原计划推进。**后续可做**：① 模块生成聊天链成功后未联动打开模块页/自动填充「包制作」路径（ModuleInspector 状态属面板内部，可经事件总线接）；② BYOK 模块生成仍受服务端 8192 maxTokens 上限约束（DeepSeek 限制），大模块建议系统 AI 通道；③ 聊天里「新建文件」目录许可当前只对 `.lcpp` 开放（`NEW_FILE_DIRECTORY_EXTENSIONS`），后续按需扩 `.h/.cpp`。

- 已修复（2026-09-21）：**分组框 `控件_设置文本` 后边框消失**。根因（消息探针实证）：分组框是子类化 `STATIC`，外观全部由 `PaintGroupBox` 自绘（背景 + 边框 + 断口标题，标题取自运行期不可变的设计器规格 `ControlSpec.text`）；GroupBox 子类分支只拦截 `WM_ERASEBKGND/WM_PAINT/WM_PRINTCLIENT`，`控件_设置文本` 发出的 `WM_SETTEXT` 落到 `DefSubclassProc` 后，user32 的 STATIC 会在 `WM_SETTEXT` 处理内部立即按裸静态样式自绘（文本顶格、无边框，且不经过任何 `WM_PAINT`/`WM_PRINTCLIENT` 往返），把自绘边框整面覆盖，随后更新区已验证，`PaintGroupBox` 永不再触发；即使触发，画的也是旧规格文本（标题不同步是同一根因的第二面）。修复收敛在 `ControlSubclassProc` 的 GroupBox 分支单点：拦截 `WM_SETTEXT`/`WM_ENABLE`，从 `WM_SETTEXT` 的 `lParam` 同步标题进 `RuntimeControl` 新增的 `hasOverrideText`/`overrideText`（`RuntimeControl` 尾部默认初始化，不影响既有聚合初始化），`DefSubclassProc` 后强制 `InvalidateRect` 让 `PaintGroupBox` 立即整面重绘（标题、置灰、边框、断口全部恢复）；`PaintGroupBox` 标题来源改为 override 优先。`控件_设置文本` 本身零特判——任何路径改文本（含动态创建的 `控件_创建分组框`）都走同一拦截；`控件_取文本` 仍读 HWND 文本，与画面一致。真机验证：最小设计器工程 F5 后依次驱动设置文本/清空标题/禁用/启用/回读五条路径，边框全程保留、标题即时更新、禁用置灰正确、回读与画面一致。遗留：同机制下其它「子类 STATIC + 规格文本自绘」的组合当前只有分组框一种，后续新增此类控件时必须同法拦截 `WM_SETTEXT`/`WM_ENABLE`，不能复用本次仅挂 GroupBox 分支的拦截。

- 已完成（2026-09-20）：**AI 模型调用唯一适配层 + BYOK 聊天链收口（根治聊天 prompt 被原文回显）**。背景：自定义 API 模式的聊天历史借道 `/api/translate` 汉化通道——把整段聊天 prompt（人格 + 对话转录 + 提问）当「一条待翻译字符串」发出，Gemini 会响应内嵌提问碰巧可用，换 DeepSeek 等严格遵循翻译指令的模型后「中译中」= 原文回显，用户提问被整段复读（2026-09-20 真机 GIF 实锤）。三层根治：① `electron/src/services/ai/aiProviderService.ts` 成为四协议（gemini/openai 兼容/deepseek/anthropic）唯一适配层，收编 server.ts 原先三份重复的 provider 分支（`generateAiText`/`testAiConnection`/`listAiModels`/`getGeminiClient`/`resolveAiConnectionConfig` 全部迁入），并新增 `generateAiChat`（多轮 messages、可选 SSE 流式 `onDelta`/`onReasoning`、`AbortSignal` 贯通）；② 新增 `POST /api/ai/chat`（非流式 `{ok,reply}` + SSE `delta`/`reasoning` 事件流），system = LingBuilder 助手人格 + 当前文件节选（8KB 上限）+ 规则手册（与编辑 planner/翻译链同源注入），限幅 64 条消息/单条 32K 字符；③ `AiAssistant.tsx` 自定义 API 分支改发真实多轮 messages（与系统 AI 分支同构，最近 18 条），流式增量直接更新聊天气泡，停止按钮走 AbortController 中止并把流式中消息定格为已取消。`/api/translate` 回归纯汉化职责（批量汉化、一键智能汉化两处调用方不变）。`maxTokens` 8192、`temperature` 0.6、deepseek 保持 `thinking:{type:"disabled"}`；OpenAI 兼容端点的 `reasoning_content` 增量单独进 `reasoningText` 折叠展示。测试：`tests/aiProviderChat.test.ts` 4 用例（校验与缺 Key、deepseek 兼容端点消息形状+规则手册注入+thinking 参数、SSE 逐段透出+完整文本收尾、anthropic system 外置+activeFile 节选）接入 `test:lingcpp`；直接读 `AiAssistant.tsx` 源码断言的 `aiConnectionSession`/`aiSecurityAccessibility`/`cloudAccount*` 24 项全绿。同步：`docs/AI编辑链统一架构.md` 新增「聊天链」节、AGENTS「AI 模型调用与聊天链收口」条、`electron/README.md`「BYOK 聊天链路」节。**待办（本批刻意未做）**：① 系统 AI 模式聊天仍走云端流式链路，两模式的前端渲染已同构但尚未抽出共享组件；② 聊天 `maxTokens` 未按模型上下文自适应（如 Claude 8K 上限端点会被 8192 顶满，需按 provider/model 分档）；③ 旧版被打包用户若停留在复读版会话里，历史已污染的复读消息仍在会话存储中，需用户手动新建会话（不做自动清洗，避免误删真实历史）。

- 已完成（2026-09-19）：**大能力模块的界面开发视图接入 MCP + new_emoji 界面配方语料**。`lingbuilder.module.info` 新增三个只读视图（工具数仍 23，实现集中在 `electron/src/services/aiBridge/moduleUiViews.ts`）：`designerControls`/`control` 详情（模块 `contributes.designerControls[]` 的控件清单、属性表含枚举 options、事件 `handlerPattern`、`defaultProps`、容器 `layout` 与代码创建契约，此前对 AI 完全不可见）、`demoExample`（逐命令附带 `examples/module-demos/` 语料里的真实调用行，缓存 + 结果集 ≤400 才附带，未命中不伪造）与 `demoProject`（演示项目定位与新鲜度）、`uiExamples`/`example`（可复制界面配方索引与正文）。首批 7 条 new_emoji 配方落 `examples/ui-recipes/lingbuilder.new_emoji.ui/`，从已实机验收源码提炼，固化「模型零控件 + `控件_创建NE*`」与「模型控件 + `.内容` 成员语法」两条合法路径。测试：`tests/uiRecipes.test.ts`（齐备/行数上限/红线扫描/`commands` 与正文一致/逐条 `.lcpp` 语义诊断零 error）接入 `test:lingcpp`，`tests/aiBridge.test.ts` 补视图回归。**待办（本轮刻意未做）**：① 配方与演示语料都在仓库 `examples/` 下，而 `examples/` 不在 electron-builder 的 `files`/`extraResources` 清单内，**打包版工作区取不到**，外部 AI 在打包版仍只能拿到模块 `contributes.examples[]` 声明的示例——二期应把 new_emoji 配方改由模块包携带（`generate-new-emoji-module.cjs` 复制进 `docs/examples/` 并登记 `contributes.examples[]`，随 default-workspace 与 `ensureBundledModules` 补铺），这与「模块封装要简单、示例随模块走」一致，优先选它。**【2026-09-24 部分】`module-demos` 已随 extraResources 分发并在模块详情「示例」页签提供「在新 IDE 窗口打开」（见 2026-09-24 首条）；ui-recipes 配方与 `模块命令清单.json` AI 语料仍在工作区侧，MCP `demoExample`/`uiExamples` 在打包版依旧取不到，二期模块包携带方案继续有效。**② `examples/module-demos/` 全量停在 2026-08-29（new_emoji 3784 命令 vs 当前清单 4011），`NE表格_设置列` 等后续命令因此无 `demoExample`，已用 `demoProject.stale` 显式暴露而不是伪造，重跑 `npm run module:demos -w lingbuilder-electron` 可让 `stale` 归假（会成批改 `exports/` 源码包与根解决方案，属预期）；③ 只有 new_emoji 有人工配方，CEF3/EdgeView/FBro/SQLite/HTTP 服务端等模块的写法仍散在 `AI 视频自主生产/` 教程工程，应按同一约定补 `examples/ui-recipes/<moduleId>/` 并把「已验收源码 → 配方」写进模块交付清单；④ `lingbuilder.win32.basic`/`common-controls` 的 `contributes.designerControls` 属性表与 `win32ControlRegistry` 尚无对齐门禁，两侧漂移时 AI 拿到的属性表会与画布属性面板不一致（可复用 `tests/windowControlCompleteness.test.ts` 的断言面）。⑤ **整模块默认概览响应仍 ~356KB**（实测 MCP Streamable HTTP，2026-09-19）：带 `control`/`example` 的界面查询已改为不回命令表（控件详情 377KB→46KB），但只给 `moduleId` 时仍会返回 500 条命令文档（约 350KB），外部 AI 第一次探模块就可能吃掉大半个上下文。后续应按视图分档：默认只回元信息 + 控件概览 + 配方索引 + 命令计数，命令表必须显式 `query`（或新增 `includeCommands`）才展开，并对单条命令的参数文档做字段白名单裁剪；改动需同步 `module.info` 工具描述、`MCP_INSTRUCTIONS` 第 3 条与 `tests/aiBridge.test.ts` 体量断言。详见 `AGENTS.md`「大能力模块界面开发视图与配方语料」、`docs/MODULE_ECOSYSTEM_IMPLEMENTATION.md` 同名节、`examples/ui-recipes/README.md`。
...
- 已完成（2026-09-20）：列表框新增「文本对齐」属性（`textAlign`：左对齐默认/居中/右对齐），经通用 `CF_ALIGN_CENTER`/`CF_ALIGN_RIGHT` 标志位透传到 `PaintOwnerListBox` 自绘 `DrawTextW`，设计器画布预览同步按属性渲染，旧项目无该属性时保持左对齐不变。已知边界：`控件_创建列表框` 代码创建路径的运行时 `flags` 目前硬编码 `CF_SHOW_BORDER`，不带对齐参数，代码创建的列表框暂不支持文字对齐；后续如需支持，应为创建命令扩展可选对齐参数并在该路径解析进 `flags`。
...
- 写法口径（2026-09-20）：`返回(表达式)` 括号形态定为示例、教程、演示项目与外部 AI 生成的统一写法（`MCP_INSTRUCTIONS` 第 12 条、规则手册三处同步）；不带括号的 `返回 表达式` 与光杆 `返回` 保持合法等价（`parseReturnValue` 同译 `return`），属存量兼容契约，不得移除支持。已知边界：括号不配对（`返回(合计` 少右括号）时语句静默降级为 C++ 注释、返回值丢失——是否为该形态补一条中文诊断待定。
...
- 已完成（2026-09-24）：区域实例窗口缩放白屏根治——`EdgeView_调整全部大小()` 原先对全部实例无条件把 WebView2 对齐到承载 host 客户区，而 host 创建后没有任何命令能改其几何，「顶部留 54 像素导航条」类布局一缩放就被拉成 host 尺寸、页面按超大宽度排版成白屏（用户侧只能靠 700ms 定时器防抖绕开）。修复三层：① `EdgeViewInstance` 新增 `autoStretch`（创建区域按「x/y=0 且宽高铺满窗口」判定），`EdgeView_调整全部大小()` 只自动拉伸铺满实例、跳过弹窗（与 09-13 设计器控件同一条规则）；② 新命令 `EdgeView_置区域位置(实例编号, 左, 顶, 宽, 高)`（1.6.0，320→321 条）用 `SetWindowPos` 移动/缩放承载 host 并即时对齐控制器，「大小被改变」里一行同步、拖拽跟手；③ Win32 后端窗口级事件接线缺口（源码声明 `_类名_大小被改变` 不生效、必须手工登记设计器 window.events）——`resolveWindowEventBindings` 统一出口按常规命名自动接线并同步喂 WindowSpec 事件串与窗口类派发表两处（裸名仍不接线，多窗口裸名互相覆盖是已知独立问题）。回归：`tests/modules.test.ts` EdgeView 多店铺用例扩展 + 新增「窗口级事件按 _类名_事件 常规命名自动接线」用例；MCP 第 3 条已带缩放红线（`tests/aiBridge.test.ts` 关键词断言 + `dist/cli.cjs` 重建 + stdio 握手验收）。真机 MSVC 冒烟（192 DPI）另暴露并修复第四处：`窗口_取事件宽度/高度` 原样返回 WM_SIZE 的物理像素，喂给按逻辑坐标的几何命令会二次缩放——现已按 `MulDiv(值,96,dpi)` 返回设计器逻辑坐标（DIP），与 控件_设置位置大小/置区域位置 同口径；new_emoji 后端同名命令仍返回物理像素（EU 引擎坐标系，未动，`electron/docs/modules/fbro-shell/examples/MainWindow.lcpp` 里的手工换算仍正确），清单描述已写明后端差异。
...
- [x] 全部内置、官方和已安装模块进入逐方法/逐参数审计；当前覆盖 109 个模块、7945 个方法、19007 个参数、5103 个 controlRef（含本机已装磁盘模块，换机需重装后重跑），并用摘要锁定目录变化；63 个模块 TypeScript 源文件也进入原始补全/示例/snippet 门禁，不再只验证加载后的归一化结果。
...
## 6（已完成 2026-09-23 零内嵌 C++ 能力批次）. `到文本(...)` 参与拼接时容易生成有歧义的 C++

- 已完成（2026-09-23）：`LingCppTextValue` 收敛为 `using LingCppTextValue = std::wstring;`，`到文本` 的七个重载直接返回
  `std::wstring`（普通 Win32 后端的文件级实现与另一个后端的类成员实现同步改），隐式 `operator const wchar_t*` 与四个自定义
  `operator+` 一并消失：`到文本(值) + 文本变量`、`到文本(a) + 到文本(b)`、`到文本(a) + 功能库.取文本()` 都不再触发 `C2666`。
  数组文本成员的返回类型从 `const wchar_t*` 改为 `std::wstring`（同时消除 `C2110` 指针加法与 16 槽轮转缓冲区被同一表达式
  取用多次的别名风险），宽字符实参归一仍统一走 `LingCppWideArg(...)`，并补齐「括号包裹的调用」与「拼接结果」两类实参形态
  （`(功能库.取文本())`、`库.取文本() + " 后缀"` 此前会裸传给 `const wchar_t*` 形参触发 `C2664`）。
  验证：`electron/scripts/smoke-zero-inline-cpp-native.ts`（MSVC 真编译验收项目）与 `tests/lingcpp.test.ts` 的「零内嵌 C++ 能力批次」用例。

- 原始现象（保留）：一个表达式里出现两次 `到文本(...)`，或 `到文本(...)` 与 `std::wstring` 相加，MSVC 报
...
- **遗留与注意**：① `.lingbuilder-build/` 与 `generated/cpp/` 的旧导出物不会自动继承修复，必须重新导出/重新生成；② 已修复（2026-09-22，DLL 项目 spawn msbuild ENOENT 真机暴露）：`resolveMsBuildCommandAsync()` 已接入 vswhere 探测（先限 C++ 工作负载、无则放开；内部版本号不限，VS2026 目录名是 18）+ 固定路径扫描（18/2022/2019）+ ENOENT 中文修复指引，扫描结果按进程缓存；③ CLI/无头环境导出含 SQLite 模块的工程需设 `LINGBUILDER_SQLITE_RUNTIME_ROOT` 与 `LINGBUILDER_RESOURCE_ROOT`（图标），否则随附 sqlite3.dll/默认图标缺失会导致导出不完整或阻断；④ 冒烟门禁 `smoke:fbro-process-native` 内部自带的工具集探测可改用统一探测器消重。
...
1. **窗口项目「无头资源自动启动」GUI 真机验收**：生成产物与编译已验证（控制台 exe 编译了同一套 LingWindowBase 钩子），但设计器放置组件 → 运行 → `EdgeView_枚举实例JSON` 自动出现实例的 GUI 链路未实机点验；IDE 打开演示项目时顺手确认并截图。
...
1. **像素帧→PNG 截图**：`CEF3离屏_订阅像素帧` 现只点亮订阅位、不外发内容。要出图需桥侧为 on_paint 帧补缓冲生命周期管理（拷贝时机在 `LingBuilderCefBridge.cpp`），并把订阅回调降形接进 `lingCppWin32Project.ts` 的订阅事件段（V4 subject），生成期拿到帧数据后再编码 PNG；在此之前对外口径保持「截图未开放」。
2. **OSR 输入命令扩句柄形态**：`CEF3_发送鼠标单击事件`/`CEF3_发送鼠标移动事件`/`CEF3_发送鼠标滚轮事件`/`CEF3_发送触摸事件`/`CEF3_发送按键事件` 等目前只收控件名（controlRef），无头实例没有控件名、无法注入输入；应按浏览器句柄扩一组变体（与 `CEF3OSR_*` 同一寻址）。
3. **无头事件处理器派发**：无头实例的 CEF 事件现在只进实例内有界环形缓冲（上限 200 条、超限丢最旧），只能 `CEF3无头_取事件JSON` 轮询；后续可评估经无头泵窗口派发到中文事件处理器（需定义与控件版事件的兼容口径）。
4. **存活实例登记表与 `FromMessageWindow` 防护一致性**：Task 5 为无头 teardown 新增了 `LingWindowBase` 存活实例登记表（构造注册/析构注销，带 `LingBuilder_取存活实例` 校验），但 wWinMain 消息循环路径仍有一处未防护的 `FromMessageWindow` 裸 `GWLP_USERDATA` 指针转换——要么把该处同样接入登记表校验，要么删掉登记表回到轻量方案，不得维持两套半成品。
5. **桥 stderr 换行**：`LingBuilderCefBridge.cpp` 多处 `fprintf` 把换行写成字面 `\n` 文本，真机日志整段挤成一行，影响无头冒烟与排障可读性；统一改真实换行。
6. **控制台 + new_emoji 设计后端形态**：`lingCppWin32Project.ts` 在 `usesNewEmojiDesigner` 时先分派，该组合会同时丢掉 CEF 子进程守卫与 `LingBuilder_CEF3_退出回收()` 挂接（Task 4 记录的 M-4 存量缺陷）；需要在分派前补齐守卫与回收。
7. **旧 `.lcpppkg` 教程包按新桥重导**：CEF3/EdgeView 教程包导出于旧桥（cefQuery 多通道、V4 视口、无头命令族之前），按新桥重导后随教程发布（发布动作待单独授权，与 SDK 归档/直链同批）。

## EdgeView 填表族运行期缺陷修复与级联崩溃排查（2026-09-21，第 16 集示例真机暴露）

第 16 集（填表族网页自动化）示例首次对 `EdgeView填表_*` 22 条命令做运行期验证，暴露并修复：

- **已修复（`edgeViewRuntime.ts` 的 `EdgeView_填表执行`）**：包装串 `(function(){return fn})();(args)` 有两处缺陷——`})()` 后多余分号使第二段成为逗号表达式，助手函数 `lbTianBiao` 从未被调用；实参未包引号（裸标识符 ReferenceError）。叠加效果是 22 条命令真机全部空转返回 null（此前只有编译期验证）。修复为 `})()` 直接连调 + 实参单引号包住（`EdgeView_填表拼接字面量` 负责转义）。**注意该函数族历史上只有编译期冒烟，"编译通过"不等于"运行生效"——后续新增 v2 命令族必须补一条运行期断言（如 hasElement+getValue 回读）到 smoke-edgeview-native。**
- **待根治（间歇性崩溃）**：在 EdgeView 任务完成处理器内部同步下发下一条异步命令（级联写法）存在间歇性崩溃（进程直接消失，无诊断输出）。第 16 集示例已改为「同一按钮一次性并发发出全部读命令、各处理器只写各自结果标签」规避。疑点方向：`EdgeView任务_完成` 派发处理器时处理器内再走 `EdgeView任务_新建`/`ExecuteScript` 的重入路径（任务表遍历与新建交叠、或 WebView2 回调栈上同步建任务）。复现：第 16 集示例以「存在检查 → 读回校验 → 内容读写」快速连点可复现（改并发形态后不复现）。
- **测试版本号债务**：`tests/modules.test.ts` 有断言钉住 edgeview 版本 `1.5.0`，模块已是 `1.5.1`（无头批次），该测试未随版本升级更新；另仓库根 `.lingbuilder` 缺物化演示工程导致的数条 ENOENT 环境失败为既有债务（见 2026-09-14 记录）。

## 延时/定时命令批次遗留（2026-09-21）

`lingbuilder.win32.basic` 新增 `延时`、`延迟调用` 与「时钟」非可视组件后仍有两处后续优化空间：

- **控制台/动态库项目暂不支持时钟组件**：当前生成器对带时钟资源的非窗口输出直接中文阻断（引导改用 `延迟调用`/多线程命令）。后续可让时钟计时器挂到控制台入口的「无头泵窗口」上（`LingBuilder_确保无头泵窗口` 复用同一 WndProc，WM_TIMER 分发已就位），配合「启动」子程序里的泵循环即可让时钟在控制台生效；需要同步解除阻断诊断并补真机验证。
- **延迟调用无参数传递形态**：当前 `延迟调用(毫秒, &处理器)` 仅支持无参处理器（与 `线程_提交` 的变参捕获不同，延迟场景最常见的是零参回调）。如需带参形态，可在 `ModuleDelayedCallInvocation` 上扩展 `variadicParameterIndex`，生成器捕获列表沿用 `translateManagedModuleInvocation` 的 `lbArg` 模式。
- **测试版本号与环境红（既有债务延续）**：本次批次期间 `tests/modules.test.ts` / `tests/windowDesigner.test.ts` 中由并行会话在途改动引起的红（cron/sunnyNet 运行时、wWinMain 前台激活重构、workspace demo 物化、`lingbuilder.cron` 计数写回）不属于本批改动；本批自身的新增基线（内置 101/3915/6866、工作区 109/7932/18987、win32.basic=213）已实算写回并验证通过。

## 外部 AI 接入链路批次遗留（2026-09-21，P1~P7 落地后）

- **CLI `project build/run` 与 MCP `project.create` 的收费门禁口径不一致**：MCP 侧经 `AiBridgeService` 挂 `assertModuleAccess`（含 2026-09-21 的 moduleAccessGate 三分类与惰性重试），而 `cli.ts` 的 `project` 子命令构造 `AiBridgeService` 时未传该依赖（默认空实现），`lingbuilder project build/run` 不做收费权益校验。已在 `skill-kit/SKILL.md` 第 0.5/3 步向外部 AI 写明这是入口差异而非配置错误；是否给 CLI 补挂同一门禁属产品决策，补挂时需同步 `tests/moduleAccessGate.test.ts` 与 SKILL.md 措辞。
- **P1 场景「授权有效→直接放行」的真机端到端依赖云端签发的真 Permit**：Permit 信任锚按设计禁止注入/覆盖（`MODULE_PERMIT_TRUST_ANCHORS` 硬边界），单测用生成的 Ed25519 密钥覆盖热更新放行路径，真机只验证了「代理在线→宿主启动换取成功→install 收费模块直接成功」（开发机恰有有效 Permit）。如需在无授权环境复现完整放行链路，需要云端测试环境签发专用测试 Permit。

## 窗口圆角 Win10 回退与 CLI cornerStyle 缺口（2026-09-23，Aruile工具箱真机验收暴露）

- **已修复（`lingCppWin32Project.ts` 运行时模板）**：`cornerStyle: rounded/small-rounded` 原先只走 `DWMWA_WINDOW_CORNER_PREFERENCE`（属性 33），该属性仅 Windows 11 生效，Win10 上 `DwmSetWindowAttribute` 返回 E_INVALIDARG 后静默直角。现当 DWM 调用失败时，对**无边框窗口**（borderStyle==0）回退 `SetWindowRgn(CreateRoundRectRgn)` 区域裁剪（rounded=16 DIP / small-rounded=8 DIP，随 DPI 缩放；WM_SIZE 时重算，最大化清区域、还原重算，与 Win11 DWM 行为一致）；Win11 仍走 DWM 平滑圆角，带边框窗口不回退（保持系统边框原样）。
- **CLI 通道口径缺口**：`build-request.json` 此前普遍不带 `cornerStyle`（AI智能助手等参照工程的 build-request 也没有），CLI 无头构建出的窗口 `cornerPreference=-1`，连 DWM 属性都不会设置。给窗口要圆角的工程必须在 build-request 的 window 对象上显式补 `"cornerStyle": "rounded"`；IDE 通道（`.lingbuilder/window-designer.json`）已带该字段不受影响。
- **已知边界与后续项**：Win10 区域裁剪圆角是硬边（无抗锯齿），要平滑圆角需 Windows 11，或后续评估「layered 窗口 + WebView2 透明背景 + 页面自绘圆角」方案（涉及 EdgeView 透明背景能力，未实现）；`examples/aruile-toolsbox` 已含 `GetWindowRgn`/`PtInRegion` 程序化验收配方（角点在区域外、中点在区域内、最大化清区域往返）。
- **双内嵌机制说明（用户问过）**：解决方案树「内嵌资源」节点 = `embeddedFiles`（RCDATA + 运行期释放 `%TEMP%\lingbuilder-embedded\<工程ID>\`）；窗口模型 `embeddedSite`（内嵌站点）= RCDATA（lbsite-N.bin）+ WebResourceRequested 内存应答，**零磁盘释放**，两者互不相干。内嵌站点的编辑入口在设计器属性面板「当前窗口 / 内嵌站点」组（主机/入口/扫描目录回填清单），不在树节点里。
- **源码包导出器已同步（2026-09-23 修复）**：`lcppSourcePackageService.ts` 原先只复制 sourceRoot/configRoot/designerPath/assets，`embeddedSite` 引用的 www 等网页目录会被丢掉，收包方构建报「内嵌站点文件不存在」。现导出时按设计器模型 `embeddedSite.files`（含 entry）逐个随包复制并计入完整性清单；新增回归 `tests/lcppSourcePackage.test.ts`「LCPP 源码包会随包携带内嵌站点（embeddedSite）网页文件」。安装版 IDE（0921 包）内置导出器未含此修复，需随下次打包更新。

## 模块命令实参类型门禁的已知边界（2026-09-23，任务：修「字节集传文本形参」级联报错）

- **已落地**：`build.run` / `native.preview`（export 经 preview 继承）在生成前对全部 `.lcpp` 跑 `collectArgumentTypeAdmissionProblems`（`src/services/lingCpp/argumentTypeAdmission.ts` 唯一实现，语言服务同源诊断 `lingcpp-argument-type-*`），实参推导类型与 binding 形参类型已知且不相容即中文阻断（含行列、形参名、转换建议）；生成器对已知非文本返回值不再套 `LingCppWideArg`。
- **后续项 ①**：`edit.apply` 目前不跑实参类型门禁（只跑控件引用门禁）——外部 AI 可应用带类型错误的源码，到 build 才被拦。是否前移到 apply 待用户拍板（前移会提高提案拒绝率，需配合「诊断驱动自动修复」体验）。
- **后续项 ②**：类型推断对二元算术表达式（`a + 1`、`b * 2`）返回 unknown 而跳过校验；整数/小数运算参与文本拼接（`"x" + 整数变量`）依赖 `到文本` 范式，生成器拼接分支已能处理，但门禁无法发现「直接把数值表达式传文本形参」的写法。补二元表达式类型推导后可收敛。
- **后续项 ③**：项目全局变量 / 项目常量初始化表达式中的模块命令调用不参与本门禁（与数组命令校验同口径，只覆盖方法体与局部初始化）；`utf8String` 返回值（ABI 为窄字符）作为文本实参的处理沿用旧行为，未验证是否存在真实消费方。
- **取证配套**：LB-PDD 进程消失问题维持 `[未定位]`；LocalDumps（DumpType=2 → `%TEMP%\lbdumps`）与分析脚本 `electron/scripts/analyze-lbdump.ps1` 已布防，判读口径写在脚本头（转储为空 + 无 WER = 指向外部 TerminateProcess，不硬凑灵码缺陷）。
