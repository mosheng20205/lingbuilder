...
G03 完成证据：Git 更改视图补齐仓库初始化、工作区/暂存区统一 Diff、放弃跟踪文件修改、明确确认后删除未跟踪文件、远程仓库增改删、显式 PR 源分支和窗口聚焦/重新可见自动刷新；分支、远程、Blame、PR、合并/变基及危险确认全部使用 IDE 内置中文对话框，不依赖 Electron 不支持的浏览器 `prompt()`。所有 Git 写操作统一通过 `CommandService -> SourceControlService -> /api/source-control/* -> GitService` 执行。服务端修复首次提交前 `No commits yet on main` 分支头解析，并将父级 Git 仓库状态严格裁剪为当前工作区相对路径，暂存、Diff、恢复和冲突操作复用同一路径模型。真实临时仓库专项测试覆盖初始化、中文 Diff、暂存 Diff、两类恢复、远程生命周期和嵌套工作区隔离；Git 专项与 UI 契约测试 **11/11 通过**，`npm run lint`、`npm run build` 和 `git diff --check` 通过。
