...
- **系统 DLL**（`系统 = 真`）：user32、gdi32 等随 Windows 分发的库，免分发——不复制文件、直接链接系统导入库。系统 DLL 的导出名固定为英文，**中文命令名必须用 `= 真实导出名` 指定别名**（如 `提示音 = MessageBeep`）；构建时生成指向该真实导出名的内联转发函数完成绑定，声明的导出名在系统 DLL 中不存在时会得到中文阻断诊断。这个英文导出名还会自动登记为中文命令的**补全别名**：在代码里敲 `MessageBeep` 能筛出「提示音」，上屏仍是中文命令名。
- **命令备注**（命令行下一行 `备注: 文本`）与**参数备注**（参数后 `// 文本`）会同步进入补全提示。
- **传址参数**（参数后加 `传址`）：按指针传入，DLL 回填结果（仅整数/小数/逻辑/字节等 POD 类型支持，文本型不支持）。
- **命令名不能为空**：空名声明无法写回声明文件（会被解析器当作无法识别的行丢弃）。「添加命令」会先给出 `新命令N` 占位名，你可以直接改；清空命令名会被拒绝，删除整条命令请用卡片右下的「删除此命令」（折叠状态下同样可用）。
- 声明表格编辑器支持每条命令的折叠/展开、右键搜索与批量展开折叠，并按备注与参数提供补全提示。跨项目迁移用两个按钮分工完成：「复制此命令」只复制该命令自身的声明行，粘贴时并入目标项目的当前库；「复制全部声明」复制整份声明（含各库与架构路径），粘贴时按库名合并。同名命令一律跳过并在反馈区提示。

构建时框架自动完成：按实际导出表生成导入库并链接（系统 DLL 的中文别名命令生成指向真实导出名的内联转发函数）、把 DLL 复制到 exe 同目录、把声明的函数注册为中文命令（补全与新手提示同步可用，英文导出名同时作为检索别名）。缺少对应架构 DLL、系统 DLL 未导出声明的函数、DLL 未导出声明的函数时，构建会给出中文阻断诊断。
