...
- CDP 模块是**通用协议客户端**：既可驱动外部 Chrome/Edge（`--remote-debugging-port` 启动），也可通过 `FBro_取调试端口` / `浏览器外壳_取当前调试端口` 取得地址后连接 LingBuilder 内嵌 FBro 浏览器。`FBro_取调试端口` 对进程内与独立进程宿主模式都返回真实端口；进程内模式的端口在生成程序初始化 FBro 运行时时按 `enableDevTools` 属性（缺省启用）自动预留，把项目内全部进程内控件的开发者工具关闭即可不开放端口。
