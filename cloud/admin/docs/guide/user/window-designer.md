...
- **对齐与等分**：使用画布顶部布局工具栏的 **左齐**、**顶齐**、**右齐**、**底齐**、**水平居中**、**垂直居中**、**横向分布**、**纵向分布**、**等宽**、**等高**。选区少于 2 个控件时按钮置灰。
- **撤销与重做**：工具栏 **撤销**（`Ctrl+Z`）/ **重做**（`Ctrl+Y`），布局调整可随时回退。
...
未选中控件时展示窗口级属性，分组为 **当前窗口 / 布局**（设计后端、宽度、高度）、**当前窗口 / 打开位置**、**当前窗口 / 外观**（窗口标题、背景颜色、标题栏颜色、标题文字颜色、窗口边框、窗口图标）、**当前窗口 / 内嵌站点**（把网页静态文件编入 EXE 并从内存服务，配合 EdgeView 浏览器模块可实现单 exe 分发，详见 [EdgeView 浏览器模块](/guide/user/modules/edgeview)）、**当前窗口 / 原生框架**、**当前窗口 / 状态**。面板底部另有 **删除此控件**。

> [!IMPORTANT]
> **设计后端** 决定这个窗口用哪套原生 UI 库来画和运行，可选 **Win32 原生控件** 与 **new_emoji 模块**，每个窗口独立选择：
>
> - 要添加 **New_Emoji 控件** 分组的控件，必须先把当前窗口的设计后端切换为 **new_emoji 模块**；后端仍是 Win32 时，该分组会提示「当前窗口使用 Win32 后端，请在窗口属性将「设计后端」切换为 new_emoji。」分组显示（未启用）则是另一回事——项目还没在 **配置项目所使用模块** 中启用 new_emoji 模块。
> - **窗口已有控件时后端锁定，不能切换**（属性面板会提示「窗口已有控件，后端已锁定；新建窗口可选择另一后端」）。要换后端请新建窗口，或先删除该窗口全部控件。
> - 同一项目可以有 Win32 窗口和 new_emoji 窗口并存，各自按自己的后端生成 C++ 代码和运行行为。new_emoji 后端的控件一览与用法详见 [new_emoji 原生界面库模块](/guide/user/modules/new-emoji)。
