...
1. 在主窗口右侧面板中找到 **AI 聊天** 标签页（若未显示，点击右侧工具栏的 **AI 智能编程助手** 按钮即可切换显示）。
2. 聊天面板包含两部分：
   - **对话区**：展示用户与 AI 的问答记录
...
> [!WARNING]
> 应用 AI 生成代码前，请先阅读代码逻辑，确认其符合项目需求。AI 可能对特定库接口或模块版本的细节理解不准确，建议应用后立即按 `F5` **编译并热运行** 验证。

## 4. 成本配额管理

AI 对话服务按请求消耗 **点数**（credits）。在 AI 助手面板底部可查看当前剩余点数。

| 操作 | 路径 |
|---|---|
| 查看剩余点数 | **AI 助手面板** 底部额度显示 |
| 充值 / 购买套餐 | 面板底部点击 **充值** 按钮，按套餐购买 |
| 查看用量 | 每次对话结束后，回复中会附带本次输入/输出用量 |

> [!NOTE]
> 点数仅在使用内置云端模型服务时消耗。配置自己的 API Key（BYOK）后，请求费用由模型服务商另行结算，与 LingBuilder 内置点数无关。
