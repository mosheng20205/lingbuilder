...
- 在 LingBuilder 的 **AI 聊天**、**AI 补全**、**AI 审查** 等功能中一键切换模型服务
...
1. 在 LingBuilder 主窗口顶部菜单栏，点击 **工具** > **AI Bridge**。
...
| 模式 | 说明 | 适合场景 |
|---|---|---|
| **只读模式** | AI 仅可读取项目文件，不可修改 | 代码审查、问答 |
| **预览模式** | AI 可修改文件，但写入、构建和导出需要显式确认 | 日常辅助编程、安全检查 |
| **工具模式** | AI 可读写文件、执行构建命令等，但仍在受控工具范围内 | 高级自动化、批量重构 |

在 AI Bridge 连接中心底部的 **权限模式** 下拉框中选择所需模式，点击 **保存** 生效。

> [!WARNING]
> **工具模式** 下 AI 操作会直接作用于项目文件与系统命令。请仅在可信项目中开启工具模式，并关注 AI 的每次文件修改提示。
