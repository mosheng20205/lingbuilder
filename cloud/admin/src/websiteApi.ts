...
export async function fetchWebsiteUpdates(signal?: AbortSignal): Promise<WebsiteUpdateEntry[]> {
  const response = await fetch(`${CLOUD_API}/v1/site/updates`, { signal });
  if (!response.ok) throw new Error('更新记录暂时无法加载。');
  const value = await response.json() as { updates?: WebsiteUpdateEntry[] };
