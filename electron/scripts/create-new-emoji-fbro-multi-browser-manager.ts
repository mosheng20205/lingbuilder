...
每个 RequestContext 都在创建浏览器前加载项目 assets 下的 \`assets/${projectId}/doubao-downloader\` 目录。路径由 GetModuleFileNameW 得到真实 exe 位置并兼容项目资源分层，不依赖工作目录或开发机绝对路径；插件不是模块，不写入 \`.lingbuilder/modules\`。插件缺失、清单损坏、版本不支持或 FBro 拒绝加载只会更新中文状态，不会导致整个工作台退出。
