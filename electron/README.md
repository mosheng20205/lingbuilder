...
> 2026-09-20：**CEF3 官方无头/OSR 能力对外部 AI 全量可见（第四形态）+ 文档同步**。无头浏览器选型升为四形态：EdgeView 隐窗伪无头 / CDP 独立进程真无头 / FBro 进程级 headless / **CEF3 官方 OSR**（`CEF3_创建无头浏览器(实例编号, 地址, 独立缓存目录, 代理地址, 视口宽, 视口高)` 或设计器「CEF3无头浏览器」非可视组件；运行时键 2000000+编号、CEF3 引擎内项目级唯一，不与 EdgeView/FBro 共号）。① `MCP_INSTRUCTIONS` 第 3 条补第四形态全口径（取内容两条路：`CEF3无头_取主框架`→`CEF3框架_*`/`CEF3填表_*`/`CEF3DOM_*` 或 `CEF3无头_执行JS`/`取页面文本`/`取页面源码`；浏览器级控制 `CEF3无头_取浏览器句柄` + `lingbuilder.cef3.osr` 的 `CEF3OSR_*`/`CEF3离屏_*`；三条红线=句柄判有效只用 `!= 0`、先 `CEF3无头_等待加载完成` 再取内容且判渲染用 `CEF3无头_等待出帧`、截图未开放；`CEF3无头_执行JS` 返回 DevTools 信封 JSON；控制台只能轮询 `CEF3无头_取事件JSON`），`tests/aiBridge.test.ts` 补 5 条断言（含 `隐藏窗口|移出桌面|后台窗口` doesNotMatch 门禁），重建 `dist/cli.cjs` 并真机 stdio 握手验收。② `lingbuilder.cef3.browser` snippets 新增「CEF3 无头抓取（控制台项目）」范本（创建→排队导航→等待加载→标题/JS/源码/正文→等待出帧→视口→两把句柄→事件→关闭，正文照抄已实机验收的 `smoke-cef3-headless-native.ts` 链路）；窗口/设计器形态范本在 `examples/cef3-headless-demo/`。③ 文档：`LingBuilder AI 规则手册.md` 外部 AI 节新增「CEF3 无头浏览器（OSR）」条（第 5 节「无头浏览器三形态选型」同步改四形态并加 ④ 指向）、`docs/模块开发手册.md` 新增 11.4 节登记 `lingbuilder.cef3.osr`（manifest v2/targets/bindings/随包文档口径）、`docs/MODULE_ECOSYSTEM_IMPLEMENTATION.md` 写实例登记表编号口径（1000000=弹窗/区域、2000000=无头）、本条与 `AGENTS.md`「模块能力更新必须同步 MCP 通道」条款四形态化、`docs/FUTURE_OPTIMIZATIONS.md` 登记 7 条后续项（像素帧出图/OSR 输入句柄化/无头事件派发/存活登记表加固/stderr 换行/控制台+new_emoji 守卫/旧教程包重导）。④ `module:cef3-docs` 重生成 CEF3 模块 README；行为门禁回归：无头命令不进 controlRef 门禁、`build.run` 控件门禁对「只有无头资源、画布无控件」项目不误伤（`cefHeadlessCommands`/`cefHeadlessRuntime`/`windowDesigner` 等显式回归全绿）。官网命令 web-sync 与 SDK 归档/直链发布属对外动作，另行授权后执行。
> 2026-09-20：**CEF3 代理能力补齐（全局代理 + 实例代理回读，与 EdgeView 同口径）**。`lingbuilder.cef3.browser` 3.0.0-alpha.4→**alpha.5**，新增 `CEF3_设置全局代理(代理地址)` / `CEF3_清除全局代理()` / `CEF3_取全局代理()` / `CEF3_取实例代理(实例编号)` 四条公开命令（contributes+bindings+参数文档+新 snippet「CEF3 多店铺独立出口 IP」，模块 README 重生成）。运行时收口于 `lingCppWin32Project.ts`：`cef3GlobalProxy_` 类成员 + `CEF3_应用代理配置` 统一解析，五个创建点（控件/派生弹窗/弹窗/区域/无头）与非桥 legacy 的 `CEF3_准备实例上下文` 同语义回落——实例自带代理优先、显式直连（`CEF3_设置代理` 传空）不参与回落、已建实例不受影响；`CEF3_取实例代理` 按编号覆盖弹窗/区域（1000000+）与无头（2000000+）段。**代理认证（用户名/密码）经真机二分实验证伪 CEF 150 两条官方通道后按用户裁定本轮不登记命令**：① 代理 407 不投递 `CefRequestHandler::GetAuthCredentials`（桥入口探针 0 命中、桩侧确有挑战）；② `OnBeforeResourceLoad` 注入 `Proxy-Authorization` 头被 network service 静默吞请求（启用即桩侧零流量）。唯一可靠路线＝桥内置 127.0.0.1 认证中继（明文体转发 + CONNECT 隧道），登记 `docs/FUTURE_OPTIMIZATIONS.md`；需要认证现改用 FBro（`FBro会话_设置代理认证`，独立进程天然隔离）或 EdgeView（`EdgeView_设置全局代理` + 各创建代理变体，独立出口须「不同缓存目录+不同代理」成对）。验收：本机局域网双转发代理桩（源站拒绝无中继标记直连、排除 Chromium 对 127.0.0.1 的绕代理假象），无头回落/实例自带/弹窗回落/回读/清除全绿、exe 干净退出；`npm run lint` 绿，`tests/modules.test.ts` 指纹写回 3810/6754/1340（摘要 `68b6e115`/`ca97482c`）、CEF3 公开命令 472、清单计数同步；期间为探针/注入实验两次重编 CEF3 桥 DLL 并 `--install` 进已装 SDK，**最终已从回退后的源码重建**（桥内仅留结论注释），CEF3 SDK 归档发布前须按磁盘重算 SHA；官网命令 web-sync 待凭据。详见 `更新记录/2026-09-20.md`。

> 2026-09-20：**FBro 启动开关批次4——跨域走官方 `FBroHsCommandLine_EnableCrossFrame`**（bridgeVersion 与 `lingbuilder.fbro.browser` 同步 2.8.0→**2.9.0**）。① 桥：`ApplyStartupSwitchesTo` 增 `enableCrossFrame`→官方 `FBroHsCommandLine_EnableCrossFrame`、`disableProxy`→官方 `FBroHsCommandLine_DisableProxy`，应用函数只准调官方语义化包装（裸 `AppendSwitch` 有测试拦截，否则白名单形同虚设），`EnableSingleProcess` 刻意不开放（官方注释「仅调试模式有效」，会破坏独立进程宿主契约）；新增 `kStartupSwitchKeys`（9 键）+ `ValidateStartupSwitchJson`（受控形态 `{"键":true|false}` 单层对象，逐字符扫描不用 CEF 值 API——初始化前包装层未就绪会 FATAL）；`LB_FBro_SetStartupSwitches` 补齐三件事：非白名单键/非法形态→`LB_FBRO_ERROR_INVALID_ARGUMENT`、**已初始化→`LB_FBRO_ERROR_OPERATION_FAILED`**（开关只在 `OnBeforeCommandLineProcessing` 读一次，此前是静默无效）、写入 `g_startup_switches_json` 挪进 `g_mutex`（旧代码无锁写、读侧持锁，真实数据竞争）。② 语言层：新命令 `FBro_设置启动开关JSON(开关JSON)` 与 `FBro_启用无头模式` 同范式——**字面量在生成期烘焙、运行期调用点只查询烘焙结果**（返回 1=已进初始化前登记；0=未烘焙并打中文诊断，参数写成变量必然落到 0）；FBroBrowser 增创建期属性「启用跨域模式 / 禁用代理」；同键以代码字面声明覆盖属性勾选（可显式关成 false）；非法 JSON 或白名单外键→生成前中文阻断，禁止「写了没生效还静默通过」。扫描器自写单趟跳过字符串定位器：先复用等长 `maskLingCppStringLiterals` 被实测打脸——遮蔽状态机在嵌套引号/转义下与真实字符串边界脱节，会把被字符串包住的命令名当真调用去烘焙。③ 真机功能验收 `npm run smoke:fbro-startup-switches`（三组独立构建逐键归因）：两个本地端口=两个源，父页 iframe 引另一端口子页并在 **iframe 自身 onload** 探测 `contentDocument.title`（挂父页 load 与 CEF OnLoadEnd 先后不稳，实测读到原始标题 `PARENT`）；只声明 `enableCrossFrame` ⇒ 命令行含 `--disable-web-security --disable-site-isolation-trials`、无 `--no-proxy-server`，跨源子页标题**真读回** `CROSS_OK:FBRO-CROSS-CHILD-TITLE`；只声明 `disableProxy` ⇒ 只有 `--no-proxy-server` 且跨源读回 `CROSS_NULL`；对照组 ⇒ 命令行按契约为空、读回 `CROSS_NULL`。官方开关实际写入文本由此钉死，不再猜拼法。④ 边界：9 个开关**仅进程内模式生效**（生成期只有进程内初始化片段调登记，`LingBuilderFbroProcessRuntime::Launch()` 起 Host 时命令行零参数、只传 `LINGBUILDER_FBRO_*` 环境变量），属性与命令描述写死该口径；全模式统一需 Host 侧 `LINGBUILDER_FBRO_STARTUP_SWITCHES` 管道并重定义互斥，已入 `docs/FUTURE_OPTIMIZATIONS.md` 后续项。⑤ 回归与门禁：`tests/fbroHeadless.test.ts` +5 用例（属性烘焙、字面并入与优先级、四类非法形态、注释/字符串不触发、桥↔生成器白名单双向漂移与官方包装/初始化后拒绝/锁内写入）12/12 绿；FBro 族命令数基线 691→692、公开命令 682→683、controlRef 审计 3789/6723（摘要 `114b1e96`/`ce4f9172`）、封装清单计数 3789；`electron/docs/modules/fbro/README.md` 重生成并 `:check` 绿；FBro snippet 新增「跨域前置开关与命令行回读」；`LingBuilder AI 规则手册.md` 新增「跨域/同源策略放开三内核选型」（CEF3 逐条白名单 / EdgeView 创建期属性+环境变量双通道 / FBro 官方跨域模式，共同红线=进程级创建期、运行期切换一律无效）。⑥ 外部 AI 消费层同步（按「模块能力更新必须同步 MCP 通道」常设规则补漏）：`MCP_INSTRUCTIONS` 第 3 条此前对**外部 AI 完全不可见**的新口径已补齐——跨域/同源放开按内核分流（FBro 走官方 `FBroHsCommandLine_EnableCrossFrame`、9 键白名单、仅进程内生效、`FBro_取启动命令行` 回读对照；CEF3 只有逐条 `CEF3平台_添加/删除/清空跨域白名单`，明确「不存在全局关闭安全策略命令、不得伪造」；EdgeView 只有创建期属性或 `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 环境变量通道）、CEF3 cefQuery 多通道（分号多条 + 桥全局递增查询ID + 事件 `channelIndex`）、三内核共同红线（进程级/创建期，运行期切换一律无效）；`FBro_设置启动开关JSON` 命令描述补「仅进程内模式生效」与跨内核指向（此前该边界只在设计器属性与 snippet 里）；规则手册「9. AI Bridge 外部客户端规则」节新增「外部 AI 浏览器跨域与进程级启动开关规则」条（细则原先只在生成规则节，外部 AI 节缺失）；`tests/aiBridge.test.ts` 补 7 条 instructions 关键词断言；`electron/docs/modules/fbro/README.md` 重生成并 `:check` 绿；`dist/cli.cjs` 已重建（外部 stdio 宿主只从 bundle 取 instructions）。遗留：SDK 归档重打与后台直链需按 2.9.0 发布（`sdkDependencyCatalog` 仍钉 2.7.0，即 2.8.0 起未发布过），属对外发布动作待单独授权；6 个旧开关的逐键回读组合待补。详见 `更新记录/2026-09-20.md`。
> 2026-09-19：**CEF3 桥 cefQuery 通道改多通道（CEF3 填表读取的前置底座）**。`LB_CEF3_EnableJsQuery` 从「每程序只支持一条」改为**追加式向量登记**（下标即通道号，上限 16 条）：`OnBeforeChildProcessLaunch` 把全部通道按 `查询名,取消名;查询名,取消名` 编进 `--lingbuilder-js-query`，渲染进程每通道一个 `CefMessageRouterRendererSide`、浏览器进程每通道一个 `CefMessageRouterBrowserSide` + `BridgeJsQueryHandler(state, channel)`，`OnContextCreated/Released`、`OnBeforeBrowse/OnBeforeClose/OnRenderProcessTerminated/OnProcessMessageReceived` 全量扇出。**对外查询ID 改由桥 `g_js_query_sequence` 全局递增分配**——多条通道的 CEF `query_id` 都从 1 起计数，直接透传必然跨通道串台；CEF 原生 id 只留在反查表给 `OnQueryCanceled`，「查询请求/查询已取消」事件新增 `channelIndex`（序号，不是通道名），`LB_CEF3_JsQueryRespond` 签名与 ABI 不变（老生成代码继续可用），取消通知改为**仅在查询仍挂起时派发**。函数名做合法 JS 标识符校验（禁 `;` `,`、≤128），同名重复注册幂等成功、查询名复用与取消名跨通道占用给中文报错，两名留空回落 `cefQuery`/`cefQueryCancel`。IDE 侧：`CEF3_初始化` 生成期改为遍历**全部** CefBrowser 控件、按 `;` 拆条、跨控件按查询名去重后逐条注册（原来只取第一条通道的第一个名字）；`FbroJsQueryEditorDialog` 的 cef3 形态解除「固定单通道」限制，与 FBro 共用多行增删/排序；控件属性说明、`CEF3_启用JS扩展` 命令描述、OnQuery/OnQueryCanceled 事件字段与 `LingBuilder AI 规则手册.md` CEF3 节同步改写。顺带修掉只在真机 MSVC 才暴露的生成期缺陷：`LB_CEF3_HANDLE` 是整型 typedef，11 处 `nullptr` 赋值报 C2446/C2440；`CEF3_框架浏览器句柄` 误声明 `static` 却调用非静态成员 `CEF3_查找实例`（C2352）；CEF3 填表 12 条命令共用同一官方别名 `CefFrame::ExecuteJavaScript` 触发清单「命令名称或别名重复」——`api()` 现允许传空别名（映射到生成期成员函数的命令本就没有官方导出名）。构建门禁结构修：`scripts/build-cef3-bridge.cjs` 先跑的媒体路由模式用 `TerminateProcess` 硬退出，会留下 2 个重入同一 exe 的 CEF 子进程，紧接着的全量运行被残留进程抢占缓存目录后**静默早退**（日志只到 `execute-subprocess=-1`，实测 100% 复现）；新增 `waitForOrphanTestProcesses()` 在两次运行之间按 `tasklist` 轮询等其退出（只等待不 `taskkill`，避免误杀并行会话的同名进程）。验收：桥自测新增真回环用例（同一浏览器 `window.cefQuery` 与 `window.secondQuery` 各一次，断言 `channelIndex` 分别 0/1、页面 `onSuccess` 各收到 `jsquery-pong-0`/`jsquery-pong-1`）+ 初始化前四类注册语义负例，`npm run module:cef3-bridge` 跑绿；把 CEF3 JS 交互示例工程模型改成两条通道后 CLI `project build` 真机 MSVC 编译链接通过（确认逐条注册与去重循环落在 `LINGBUILDER_CEF3_BRIDGE_AVAILABLE 1` 分支）；`npm run lint`/`build`、cef3 catalog/coverage、edgeview api-docs/coverage 四个 `:check` 全绿；审计基线写回 3788 命令 / 6722 参数 / 1340 控件引用（摘要 `7d6e90c9`/`84dcb921`）并新增 EdgeView `创建选项_置/取附加参数` 文本参数白名单两项。遗留：新桥产物只落在 `.lingbuilder/module-build/lingbuilder.cef3.sdk/sdk/bridge/x64`，未覆盖进已装 SDK 模块目录（需 `--install`），SDK 归档重打与后台直链更换待授权后执行；CEF3 填表读取 9 条待走内部通道 + 请求ID 信封。详见 `更新记录/2026-09-19.md` 追记十二。
> 2026-09-19：**FBro 无头浏览器全链落地（设计器资源 + 控制台代码通道）**。① 桥层：`ApplyStartupCommandLineSwitches` 新增 `"headless"` 分支调官方 `FBroHsCommandLine_EnableHeadless`（CEF 进程级开关，初始化前烘焙、运行期不可切换，与调试端口同范式），bridgeVersion 2.7.0→2.8.0（`module:fbro-sdk -- --install` 重编并 SHA 核对落 SDK 模块）。② 可视化：设计器「非可视」新增 **FBro无头浏览器** 资源（`FBroHeadlessBrowser`，地址/缓存目录/附加信息 JSON），生成器烘焙 headless 并在属主窗口创建期 `LB_FBro_CreateBackground` 建后台实例、按组件名复用全部 FBro_* 命令与事件派发（`FBro_查找实例` 支持资源名回退）。③ 代码：`FBro_启用无头模式()` 字面扫描烘焙（字符串/注释/@行不触发，运行期调用点查询烘焙常量）；`FBro_实例导航/等待加载超时/执行JS/是否存活/关闭/绑定事件/取最近事件/取事件数据` 8 条句柄命令登记进 manifest（控制台无消息泵，文档明确同步族取数）。④ 门禁：无头声明与进程内可见 FBroBrowser 共存、new_emoji 后端、控制台携带无头资源均生成前中文阻断（独立进程控件不误伤）。⑤ 回归：`tests/fbroHeadless.test.ts`（7 用例）+ `npm run smoke:fbro-headless` 真机（窗口资源全量 MSVC 编译 + 控制台 exe 实跑断言 HEADLESS=1/命令行含 --headless/后台实例标题回传）。FBro 命令清单同步 docs 重生成（browser 模块 119 接口）。遗留：SDK 归档与线上下单清单需按 2.8.0 重打包发布（未发布前旧 IDE 下载源仍是 2.7.0 桥，构建新无头工程会被 runtime manifest 门禁拦住）。
> 2026-09-19：**外部审计清单五项优先修复**（38 条核实：约 2/3 属实、D-4/D-6/G-2 已在仓、F-1/H-1 不属实）。① `lingbuilder.net.cookie` 生成运行时 `LB_CookieExpiry` 显式 `session:true` 分支缺 `session = true;`，会话 Cookie 被导出成 `expirationDate:0/session:false`（导入即丢弃）——一行修复 + `smoke-cookie-native` 补显式 session 两用例，真机双架构 OK。② `lingbuilder.net.http-client` 2.0.0→2.1.0（74→78）新增 Cookie 注入命令族：`HTTP客户端_请求置Cookie`（按请求整体注入，注入请求含重定向关闭自动罐回送防双 Cookie 头）、`HTTP客户端_置Cookie(客户端,名称,值,域,路径)`（RFC 6265 风格域/路径匹配手工罐）、`HTTP客户端_取CookieJSON`、`HTTP客户端_删除全部Cookie`；手工项只驻内存、销毁即清；`/whoami` 回显真机断言双后端全过；内置审计基线 3701/6463、摘要 `1da7f207`/`d4439de8`。③ EdgeView 1.5.1：`创建弹窗浏览器(代理)`/`创建区域(代理)`/`置实例大小` 宽高统一按逻辑坐标（DIP）随窗口 DPI 缩放（原裸物理像素，150% 屏 1180×820 实测只有 786×546），参数说明与 API.md 补「逻辑坐标/DIP」口径，`smoke-edgeview-native` 真机断言 400×300 弹窗客户区 600×450@144dpi。④ 生成器 wideString 形参（`const wchar_t*` ABI）实参统一归一：调用形态（功能库函数/模块命令/拼接）与控件成员表达式一律包 `LingCppWideArg`，二元 `+` 两侧同为宽字符指针时改走 `std::wstring(LingCppWideArg(l)) + LingCppWideArg(r)`（堵 C2110/C2664 两类「函数返回值直传实参」失败）；规则手册三处「先落变量再传参」旧红线同步放宽。⑤ `edit.propose` 的 `edits` 重叠检测改为**先按 startLine 排序再比较**（原按调用方顺序只比相邻对，`[1-8,20-25,3-5]` 交错区间漏检后降序应用会静默错位替换），错误信息带冲突区间对。验证：aiBridge/lingcpp/httpClientRuntime 套件全绿；`test:lingcpp` 全量仅存 2 红（TabControl 预览、SDK ZIP 清单）均为并行会话在途文件。另按「模块能力更新必须同步 MCP 通道」常设规则完成外部 AI 消费层四层同步：`MCP_INSTRUCTIONS` 第 9 条 Cookie 注入口径（原运行期观测顺延第 10 条）、`edit.propose` edits schema 补排序检测说明、http-client「Cookie 注入（2.1）」snippet、重建 `dist/cli.cjs` 后真机 stdio initialize 握手验收 PASS。详见 `更新记录/2026-09-19.md` 追记十一。
> 2026-09-19：**new_emoji 组件卡与随包组件文档（MCP 界面视图第三层）**。修掉一个长期悬空引用：包内 `docs/lingbuilder-designer-catalog.json` 给 93 个组件都声明了 `documentation: docs/components/<slug>.md`，但**93 个文件一个都没随包分发**（清单门禁只查 `contributes.docs[]`/`contributes.examples[]`，查不到目录内部引用，所以一直没人发现）。两条来源分别补齐：① 上游 `T:\github\new_emoji\docs\components\*.md`（`EU_*` 导出、状态读回、Python 示例）由 `generate-new-emoji-module.cjs` 新增的 `copyComponentDocs` 整目录拷进包；② 新增 `scripts/generate-new-emoji-component-cards.ts`（npm `module:new-emoji-cards`，支持 `--module-root`/`--out`/`--check`）从 `contributes.designerControls` + `bindings` **确定性生成**中文契约卡到 `docs/lingbuilder-components/<slug>.md` + `index.json`：代码创建命令与参数顺序、`lingCppType`、标记查找与 `控件_是否有效`、`designerType`、完整属性表（含枚举取值与默认值）、事件与 `handlerPattern`（处理器参数中文类型）、容器 `layout`、本控件命令族（≤40 条）、一段可直接粘贴的 `lcpp` 骨架；打包脚本经 `generateComponentCards` 复用同一入口，重新打包即随包生成。人工只维护 overlay `docs/modules/new-emoji/component-cards/<Type>.md`（只写「惯用要点与红线」），**首批 20 张已写**（Text/Button/Input/EditBox/Checkbox/Radio/Switch/Select/InputNumber/Table/RichList/ListBox/Menu/Tabs/Panel/Container/Image/Progress/Badge/IconButton），其余 73 张显式标 `待补`（不静默留空），推进清单在 gitignore 跟进文档 `COMPONENT_CARDS_PLAN.md`。`moduleUiViews` 新增 `loadComponentGuides`/`readComponentGuide`：控件概览与详情都带 `documentation` + `nativeDocumentation`，`control` **唯一命中**时内联 `componentGuide`（8000 字符截断并给 `absolutePath`），命中多个只给路径；带 `control`/`example` 时同时跳过 `commands` 与整套概览，实测单控件响应 377KB→105KB→46KB。踩坑：slug 不能按 kebab 推导（`ListBox`→`listbox.md`、`SelectV2`→`selectv2.md` 与推导不一致），必须取 catalog 的 `documentation` 文件名。测试：新增 `tests/newEmojiComponentCards.test.ts`（93 张齐备、标题/创建命令/中文类型/红线段落与清单一致、目录引用零悬空、overlay 命令名必须存在于清单、`--module-root` 二次生成字节幂等）并接入 `test:lingcpp`；`tests/aiBridge.test.ts` 补「唯一命中回正文 / 多命中不内联 / 概览按过滤跳过」三处断言；真机 MCP（重建 `dist/cli.cjs` + Streamable HTTP）冒烟通过。同步 `AGENTS.md`、`LingBuilder AI 规则手册.md`、`docs/MODULE_ECOSYSTEM_IMPLEMENTATION.md`、`docs/模块开发手册.md`（新增「组件级文档不得悬空」节）、`examples/ui-recipes/README.md` 与 `更新记录/2026-09-19.md`。
> 2026-09-19：**重打包验证与两个缺陷修复**。按要求真跑 `generate-new-emoji-module.cjs`（含 `--install`/`--check`）抓出：① 本批接线自身——`execFileAsync(tsxCli, …)` 把 `node_modules/tsx/dist/cli.mjs` 当可执行文件传入，Windows 直接 `spawn EFTYPE`，改为 `execFileAsync(process.execPath, [tsxCli, cardScript, …])`；② 既存缺陷——`readModuleDocumentation` 只认 `contributes.docs[]`，而模块详情页把 `contributes.examples[]` 也列为可打开项，导致 aria2/CEF3/fbro-shell/OpenCV/Protobuf/两个 DLL 演示的「示例」全部报「未在公开文档中声明」，现 docs/examples 同为单一出口（dev 实测 CEF3、aria2 示例已能打开，`tests/modules.test.ts` 补用例）。结果：`--check` 通过（`module-build`、已安装目录与 `.lbmod` 207 项逐字节一致：93 契约卡 + `index.json` + `README.md` 索引 + 97 上游文档），索引登记进 `contributes.docs[]`（单卡不逐一登记以免淹没文档列表，单卡正文由 MCP `control` 内联）；用重打包后的模块跑真实 MSVC 构建（new_emoji 窗口圆角演示，CLI `project build`）退出 0、无 `error C`/`LNK`，DLL 与图标照常内嵌。另注：`tests/modules.test.ts` 在本工作区仍有 9 条红（controlRef 计数与封装清单基线、CEF3/FBro 目录、缺 `.lingbuilder/projects/*/window-designer.json`、已安装模块数 5<7），已用 `git worktree` 跑 HEAD 基线逐条比对，属存量/环境红，与本批无交集。
> 2026-09-19：**大能力模块的界面开发视图进入 MCP（`lingbuilder.module.info`）+ new_emoji 界面配方语料落地**。针对「new_emoji 4000+ 命令、93 个设计器控件，外部 AI 经 MCP 写不出界面」，本轮**不新增工具（工具数仍 23）**，把事实来源补进 `module.info`，实现集中在新文件 `src/services/aiBridge/moduleUiViews.ts`（只读视图，不参与门禁）：① `designerControls` 控件概览（模块 `contributes.designerControls[]` 此前对 AI 完全不可见，只能猜 type/属性/事件名并被控件门禁阻断）——默认给类型/中文名/是否容器/`lingCppType`/代码创建命令 `控件_创建NE*`/允许父级/属性与事件数（上限 120），传 `control`（`按钮` 或 `Button`）才展开 `designerControlDetails`：完整属性表含枚举 options 与默认值、事件 `handlerPattern`、`defaultProps`、容器 `layout`、代码创建参数与 `通过标记文本获取NE*`/`控件_是否有效`（上限 6），无匹配给中文报错并列出前 12 个控件名。② `demoExample`：逐命令附带 `examples/module-demos/<模块>/src/模块命令清单.json` 的真实调用行（参数顺序、引号、`&处理器` 可照抄，优于 `insertText` 占位符），按 `path+mtime+size` 缓存、限 24MB、仅在结果集 ≤400 条时附带，**未命中语料不下发该字段**；同时返回 `demoProject`（项目 ID/`sourceRoot`/`generatedAt`/分组数/源码包名/`stale`）——当前 new_emoji 语料停在 3784 命令而清单已 4011，`NE表格_设置列` 等新命令无示例，AI 据此判断新鲜度而非放弃命令，刷新方式 `npm run module:demos -w lingbuilder-electron`。③ `uiExamples`/`example`：可复制界面配方索引与正文，来源为模块 `contributes.examples[]` 与工作区 `examples/ui-recipes/<moduleId>/recipes.json`，超 24000 字符截断并给 `absolutePath`。首批 7 条 new_emoji 配方落 `examples/ui-recipes/lingbuilder.new_emoji.ui/`（窗口骨架与代码创建控件／录入表单与校验／表格增删改与双击编辑／富列表模板与虚拟数据源／弹出菜单与消息框回调／无边框外壳与自绘标题栏／设计器模型控件与成员语法），全部从已实机验收源码（`.lingbuilder/ne-examples-stage/01..13`、组件总览画廊）提炼，单窗口 ≤80 行，并固化两条合法路径：模型零控件 + `控件_创建NE*` 代码创建，或模型控件 + `控件名.内容` 成员语法与 `_控件名_被点击`；两者均由生成器负责显示与消息循环。新增 `tests/uiRecipes.test.ts`（齐备与行数上限、红线扫描、`commands` 声明与正文一致、逐条 `.lcpp` 语义诊断零 error）并接入 `test:lingcpp`；`tests/aiBridge.test.ts` 补三个视图的回归用例（含语料过期不伪造示例、无匹配中文报错）；`MCP_INSTRUCTIONS` 第 3 条与工具描述同步。已知边界：`examples/` 不随安装包分发，打包版工作区取不到两套语料（见 `docs/FUTURE_OPTIMIZATIONS.md`）。同步 `AGENTS.md`、`LingBuilder AI 规则手册.md`（新增「大能力模块的界面开发视图」「new_emoji 两条合法落地路径」）、`docs/MODULE_ECOSYSTEM_IMPLEMENTATION.md`、`docs/模块开发手册.md`（`contributes.examples[]` 章节）与 `更新记录/2026-09-19.md`。
> 2026-09-18：**进程内存扫描族、进程枚举、管理员自检与 UAC 提权**（全链路落地）。① `lingbuilder.advanced.process-memory` 升级 1.1.0（+7 命令）：`进程内存_读字节集`（单次 ≤64MB、用户态地址校验、部分可读返回前缀并报 299）、`进程内存_读到缓冲区`、`进程内存_枚举区域JSON`（VirtualQueryEx，可只列已提交可读区域）、`进程内存_扫描字节集`/`进程内存_扫描字节集JSON`（运行时 32MB 整读 + 8MB 分块 + 重叠补偿，命中升序去重，达上限提前结束）与 `进程内存_取错误码`/`进程内存_取错误`；模块改为**默认随新项目启用**（读取层只补缺不覆盖，禁用持久化到 `optOutDefaultModuleIds`，依赖 `lingbuilder.std.buffer` 自动带上）。② `lingbuilder.process` 新增 `进程_按名称取ID列表`/`进程_按名称取ID列表JSON`（Toolhelp 枚举，PID 文本数组/JSON）；`lingbuilder.system.info` 新增 `系统_是否管理员()`。③ 解决方案项目 `buildProperties.requireAdministrator: true`：直编链接追加 `/MANIFESTUAC:level='requireAdministrator'`，VS 导出工程写 `<UACExecutionLevel>RequireAdministrator</UACExecutionLevel>`，缺省 asInvoker 与历史一致；增量指纹已纳入该字段（翻转后正确重链接）；实测 `#pragma comment(linker,"/manifestuac")` 不生效，必须在链接调用点传参。演示工程 `examples/process-memory-scan-demo/`（控制台项目，requireAdministrator=true）：枚举 6 个 explorer 实例、枚举 5386 个已提交区域、自身进程特征扫描命中 3（升序去重）、csrss 打开失败报中文「需要管理员」、低权限令牌运行读取 0 字节，RT_MANIFEST 提取核验 `level='requireAdministrator'`。controlRef 审计基线写回 98 模块/3640 命令/6301 参数（`3a0ab8bb`/`1753ef17`）。模块文档 `electron/docs/modules/advanced/process-memory.md`；详见 `LingBuilder AI 规则手册.md` 2026-09-18 条目与 `更新记录/2026-09-18.md`。
> 2026-09-18：**安装器进程自愈**（`installer/installer.nsh`，随 21:28 起的 0.7.5 包生效）。针对「AI 客户端经 MCP stdio 不断复活 `LingBuilder.exe` 宿主并锁住 `app.asar`，导致 NSIS 半更新且静默 exit 0 假成功」事故（真机取证：复活源含 ChatGPT/Codex 桌面版与自建 node CLI 代理；electron-builder 复制失败框点「重试」会落到忽略错误的兜底直解压，被锁核心文件被静默跳过）：① 安装开始把 `$INSTDIR\lingbuilder.cmd` 临时改名 `.disabled`——通用兜底，任何复活源重拉宿主必然失败，复制阶段新启动器落盘、`customInstall` 清理残留；② `taskkill /F` 已知名单 ChatGPT.exe / Codex.exe / claude.exe / gemini.exe（不存在静默跳过；仅安装器执行，卸载器不动）；③ `LingBuilder.exe` 循环清杀最多 8 轮（含无窗口 stdio 宿主），仍失败弹「先退出 AI 客户端」中文提示；④ 尾部补杀一轮 + 半更新自检：比对已安装 `resources\app.asar` 与包内解出副本（`$PLUGINSDIR\7z-out`）的字节数（`CopyFiles` 保留打包期时间戳，mtime 不可作新鲜度判据），不一致即交互弹中文警告、静默模式以退出码 87 结束，杜绝「exit 0 假成功」；包内副本不存在（兜底路径）时记「跳过」不误报。
> 2026-09-17：修复 AI Bridge 无头构建的 **new_emoji 单文件内嵌回归**：`compileMsvcPreviewWithModules` 未把 `delayimp.lib /link /DELAYLOAD:new_emoji.dll` 真实拼入链接命令（且 `/MANIFEST:EMBED` 落在 `/link` 外被 cl 以 D9002 静默忽略），new_emoji.dll 退回硬导入、EXE 单独分发即 0xC0000135。现内嵌 RCDATA + 首次调用解压钩子恢复生效，单 exe 分发实测自动释放 DLL 正常运行；bin 目录旁置的 `lingbuilder-newemoji-window.ico` 仅为构建副本（图标本就编译进 EXE 资源），可删。
...
> 2026-09-17：内嵌资源收尾——**设计器「内嵌资源」面板 + 旧「窗口内嵌文件」迁移 + 官网文档与命令入账**。①窗口设计器属性面板新增「项目 / 内嵌资源（跨窗口共享）」组（`src/components/EmbeddedResourceEditor.tsx`）：逻辑名/源文件/启动释放三列可增删改，入口「选择文件…」（多选）、「选择文件夹…」（递归展开、逻辑名保留相对路径）、「扫描目录」（引用工作区内已有文件、不复制）与未启用模块时的**一键启用**；四个动作注册为设计器命令 `designer.embeddedResources.addFiles/addFolder/addDirectory/enableModule`（`designerCommandTargetService.ts`，`command()` 助手补上参数透传）。②受控文件通道：主进程新增 IPC `designer-assets:select-embedded-files`（多选）与 `designer-assets:select-embedded-folder`（选目录，两者都做 realpath + 类型校验），复制进工作区、扩展名/大小与逻辑名校验收敛到渲染层本地服务 `POST /api/window-designer/embedded-resources/import` 与只读扫描 `GET .../scan`（`DesignerAssetService.importEmbeddedResourceFiles/importEmbeddedResourceFolder/scanEmbeddedResourceDirectory`）：文件默认复制到 `<项目源码根>/resources/`（同名不同内容自动 `-2` 去冲突、内容相同复用），文件名非法字符归一化为连字符并回报改名说明，无扩展名/超 256MB 跳过并给中文原因。③旧 `embeddedFiles` 自动迁移为项目级 `extract: true` 内嵌资源（`embeddedResourceMigration.ts`，释放路径不变；接入设计器归一化与生成器入口），并修掉「窗口图标选不显示 → 内嵌文件被静默丢弃」的耦合缺陷（`windowsExecutableIconService` 的 rc 行与字节物化不再以 `iconEnabled` 为门）；旧释放函数 `LingBuilder_释放内嵌资源文件()` 下线，改由运行期 `LB_EmbeddedResourceReleaseExtracted()` 在 `wWinMain` 启动释放。构建侧三处 `materialize` 统一改用 `getEmbeddedResourceSpecsForBuild()`（先迁移再取规格，否则 rc 引用 2301 段而物化只写 2001 段会 rc.exe 失败）。④验证：`npm run smoke:embedded-resource-native`（真实 MSVC x64 构建 + 运行，覆盖图标关闭与旧字段迁移，一次全绿）、`tests/embeddedResource.test.ts` 6 项、CDP 驱动真机面板实测（添加/删除条目、一键启用模块均通过）；0.7.5 在线打包并解包验证通过。
> 2026-09-17：新增内置模块 `lingbuilder.advanced.memorydll`（内存加载DLL模块，7 条命令）：把 DLL 字节手工 PE 映射到内存（映射节区 → 基址重定位 → 导入表 → TLS 槽位 → x64 异常表 → 入口函数），全程不向磁盘释放 DLL。配套「项目 DLL 命令声明」新增 `加载方式 = 内存`——DLL 以 RCDATA 内嵌进 EXE（资源号 2201 起，≤8 个/单个 ≤32MB），生成惰性解析包装，不生成导入库也不复制 DLL 到 exe 目录；未启用模块、系统 DLL 混用、位数不符、加壳或超限一律构建期中文阻断。模块文档 `docs/modules/memorydll/README.md`，示例 `examples/memory-dll-demo/`，端到端冒烟 `scripts/smoke-memory-dll-native.ts`（三场景全过且 exe 目录零 DLL）。已知边界：映射模块内 MSVC C++ 异常不派发（SEH 仅 x64 可用）、卸载只注销不释放映像；命令查找页需重跑 `module:web-sync`。
...
> 2026-09-18：补齐 CSV/表格导入两处缺口。新增共享内核 `src/services/windowDesigner/textCodecsRuntime.ts`（编码名称解析、BOM 检测、代码页编解码、`AUTO` = BOM → 严格 UTF-8 → GB18030）与 `tabularSourceRuntime.ts`（RFC 4180 记录级解析：引号内换行、双写引号、CRLF/CR/LF、自定义分隔符、空行跳过；`表格数据` 快照句柄），由 `generateSharedTableRuntime` 在装配层按启用模块铺设一次，编码模块、`文件_读取文本`、CSV 模块与 SQLite 虚拟表共用同一份实现。`lingbuilder.data.csv` 1.0.0 → 1.1.0（5 → 18 条：`CSV_打开文件`、`CSV_解析文本`、`数据表_*` 11 条），`lingbuilder.std.encoding` 30 → 32 条（`编码_字节集转文本` / `编码_文本转字节集`），`lingbuilder.fs.core` 的 `文件_读取文本` 增可选 `编码名称`（缺省仍 UTF-8 严格解码），`lingbuilder.database.sqlite` 2.2.0 → 2.3.0（72 → 73 条）：内置 `csv` 虚拟表，`CREATE VIRTUAL TABLE t USING csv(filename=..., schema=..., header=1, encoding='AUTO')` 可直接 SELECT/JOIN/`INSERT ... SELECT`，随模块编译进 exe、零额外 DLL、不需要加载扩展；相关导出按可选解析，缺少时只关闭虚拟表能力且经 `SQLite_取虚拟表支持(连接)` 可见，参数错误给中文诊断。可选参数一律由运行时 C++ 默认实参承载（生成器不补省略实参）。验收：`npm run smoke:csv-sqlite-native`（Win32/x64 双架构编译 + exe 逐项自检，含 GBK、BOM、跨行字段、表头不计入数据行、列类型亲和）。说明位于 `docs/modules/csv/README.md` 与 `docs/modules/sqlite/README.md`。
> 2026-09-12：新增 `lingbuilder.data.excel@1.0.0`（Excel 表格模块），随附 `LingBuilderExcel.dll`（libxlsxwriter 1.2.3 + OpenXLSX 0.5.1 等静态链接，MSVC /MT，Win32/x64 双架构），45 条中文命令与 `Excel工作簿` 受管句柄：创建模式为内存文档模型（保存后可继续修改再保存），支持单元格坐标读写、公式、日期、整行分隔符批量读写、区域读取、列宽行高/合并/冻结/数字格式/字体背景色/插入删除行列等全部格式能力；打开模式经 OpenXLSX 保真修改既有 .xlsx（格式与结构命令给出中文阻断诊断）。无需安装 Microsoft Excel，仅支持 .xlsx。F5 构建、AI Bridge 构建和 VS 工程导出按目标架构自动复制运行桥到 exe 同目录并强校验 SHA-256（来源与构建口径见 `third_party/excel/NOTICE.md`）。`npm run smoke:excel-native` 已完成双架构 MSVC 编译与 x64 端到端验证（创建/保存/另存为/重开读回/公式/追加行/打开模式中文诊断）。正式说明位于 `docs/modules/excel/README.md`。
> 2026-09-11：新增 `lingbuilder.database.mysql@1.0.0`（MySQL 数据库模块），通过 MariaDB Connector/C 3.4.10（`libmariadb.dll`，Schannel TLS 构建，Win32/x64 双架构）原生协议直连 MySQL/MariaDB，提供 43 条中文命令与 `MySQL连接` / `MySQL语句` 受管类型：密码连接、utf8mb4 中文、`?` 参数化预编译语句、逐行强类型结果读取、事务/自动提交、线程级中文错误缓存。F5 构建、AI Bridge 构建和 VS 工程导出按目标架构自动复制运行库到 exe 同目录并强校验 SHA-256（来源与构建口径见 `third_party/mariadb/NOTICE.md`）；官方不再发布 Connector/C Windows 二进制，随附 DLL 由 v3.4.10 源码以 MSVC+Schannel 本地构建，导出表覆盖官方 3.4.10。安装包经 `extraResources` 携带运行库与 LGPL 许可文本。`npm run smoke:mysql-native` 已完成双架构 MSVC 编译与 x64 真库端到端验证（参数化绑定、NULL/BLOB/中文往返、事务回滚、错误密码拒绝、卸载重连）。正式说明位于 `docs/modules/mysql/README.md`。
...
> 2026-09-19：`lingbuilder.http.server@2.1.0` 新增 3 条命令（共 51 条）：`HTTP_添加静态路由` / `HTTP_添加静态文件路由`（固定内容与磁盘文件由工作线程直回，不进 UI 线程，HEAD 自动回落匹配 GET 路由并返回无正文响应）、`HTTP_设置连接轮转`（单 Keep-Alive 连接强制关闭前最大请求数，默认 100，1–1000000，运行中可改）。并发压测工具 `npm run bench:http-server-native`（`--stages/--duration/--skip-build --port/--report-only`）建立基线：动态路由 10/50/100/200 并发平均 2.6~3.0 万 req/s（瓶颈为 UI 线程逐请求派发），静态路由版平均 3.9~4.4 万、峰值 4.97 万 req/s、P99≤3ms、四阶段零错误（9900X 同机回环）。`smoke:http-server-native` 扩展静态路由/HEAD/轮转 6 项协议用例，并修复 standalone 构建缺图标物化（RC2135）。后台处理器路由（方案 B）与 UI 派发批量化（方案 C）见 `docs/FUTURE_OPTIMIZATIONS.md`。正式说明见 `docs/modules/http-server/README.md`。
...
> 2026-09-18：manifest v2 新增 `contributes.constants[]` 模块公开常量（对齐易语言模块常量表）：基础类型纯字面量 + 非空中文说明 + 可选 basic/advanced 分级，校验与收集统一在 `moduleConstantService.ts`。`.lcpp` 源码以 `#常量名` 引用（强制常量解析：未知常量与常量赋值给阻断级中文诊断；字符串/注释/多行文本块/`@` 内嵌 C++ 行内的 `#` 不受影响），新手模式敲 `#` 直接呼出常量补全，`#常量` 使用独立语义令牌着色（`LINGCPP_CONSTANT_TOKEN_COLORS`，深色取易语言常量紫）。生成期与项目常量一起物化为 `inline constexpr`/`inline const std::wstring` 编译期常量，项目常量同名遮蔽、跨模块同名构建前阻断；`lingbuilder.module.info` 随清单返回 `constants[]`。专项说明见 `docs/模块开发手册.md`「公开常量」与 `docs/MODULE_ECOSYSTEM_IMPLEMENTATION.md` 2026-09-18 节。
...
- 自动拉起的 stdio 宿主环境里没有模块授权与浏览器凭据：`configure()` 写托管配置前会用 `validateHostExecutable` 拒绝 `.cmd/.bat/.ps1/.vbs`，保证宿主 `command` 直连主程序 exe（`ELECTRON_RUN_AS_NODE=1`）而非 `lingbuilder.cmd`（后者复活 GUI 进程、会挡住安装器更新）。需要让外部 AI 用收费模块或 FBro 浏览器时，用户在「Bridge 启动设置」勾选“允许外部 AI 客户端使用本机授权”（`aiBridgeStartSettings.externalModuleAccess`，**默认关闭**、运行中不可改）；开启后主进程 `LocalAuthorizationService` 才在 `127.0.0.1` 随机端口监听，并把带轮换令牌的本机授权发现文件写到 `<userData>/ai-bridge-local-auth.json`，由 CLI 侧 `localAuthorizationClient.ts` 换取模块 Permit 与 FBro 授权码。凭据只驻内存、不落盘、不进日志；PID 已不存在的陈旧发现文件一律忽略；未开启或换取失败保持 fail-closed，只输出中文诊断，IDE 退出即撤监听并删文件。回归见 `tests/localAuthorization.test.ts`。
- 灵码 Skill 离线快照位于 `electron/skill-kit/`（`SKILL.md` + `manifest.json`），经 `build.extraResources` 落进 `resources/skill-kit/` 随安装包分发；它是一份给外部 AI 客户端读的自安装指引，AI 客户端读完后自行把 stdio MCP 写进自己的配置，IDE 不代写任何第三方客户端文件。改 `SKILL.md` 正文必须同步 `manifest.json` 的 `bytes`/`sha256` 并推进 `sequence`（远端更新的防回滚基准）；文中 `# lingbuilder:managed-block:begin/end` 区间必须与 `createManagedBlock` 逐要素同形，由 `tests/skillKit.test.ts` 把关。打包链校验：`npm run verify:skill-kit-release`（打包前）与 `npm run verify:skill-kit-installer`（解包后比对仓库内容与版本），两者已并入 `npm run package:win`。
- 联网更新由 `electron/electron/skillKit/` 消费官网签名清单：`GET /v1/site/skill-catalog` → 信任锚验签（`keyId` 口径与 SDK 清单共用同一把生产 Ed25519，签名域名独立为 `lingbuilder-skill-catalog-v1`）→ `sequence` 防回滚且不得低于包内快照 → 锚定字段（`id`/`entrypoint`）逐字比对 → 逐文件按清单 `downloadUrl` 下载并校验 SHA-256 与字节数，**全部成功后才整体落盘** `%APPDATA%\<userData>\skill-kit\`，同时写 `state.json` 记录 `acceptedSequence`。任一环节失败一律不抛错、只回退包内快照并带中文 `problem`（缓存也只有每个文件哈希都对得上才算可用）。正文直链必须挂持久静态资源桶 `/update-assets/skill-kit/`，**不得放 `/docs/` 下**——那是 VitePress 构建产物目录，文档重新发布会整目录替换把正文打成 404。线上门禁 `npm run skill-catalog:check`（`LINGBUILDER_CLOUD_RELEASE_MODE=online`）拉线上清单并与仓库正文逐字比对。注意：`skill-kit` 随包是在 0.7.6 之后接入的，`release/` 里 11:09 那份 `LingBuilder-0.7.6-x64.exe` 不含 `resources/skill-kit/`，下一个对外分发包重新打包后才会内置离线快照。
...
- 主进程把生命周期事件写入 `userData/logs/diagnostic-YYYY-MM-DD.log`（`electron/electron/diagnosticLogService.ts`）：启动/退出、本地服务意外退出码、渲染进程崩溃（`render-process-gone` 原因与退出码，崩溃后自动重载，60 秒内第 3 次停止重载）、页面无响应/恢复、页面加载失败、渲染层 error 级控制台输出（每分钟 30 条节流）、GPU 等子进程崩溃、系统挂起/恢复/解锁、每 60 秒各进程内存采样，以及渲染层 `window.onerror`/`unhandledrejection`/全局 `AppErrorBoundary` 转发的崩溃现场。日志只含诊断信息，不含用户代码内容。
- 渲染层崩溃由 `electron/src/components/AppErrorBoundary.tsx` 兜底显示中文错误卡片（不再整树卸载黑屏），卡片提供 重新载入界面/导出本地日志/打开日志目录/删除本地日志。
- 桌面入口：「帮助 → 打开本地日志目录 / 导出本地日志… / 删除本地日志」，同名命令也在命令面板（`workbench.action.help.*Logs`）。用户机器黑屏一次后，请其用「导出本地日志…」生成单文件回传分析；删除走原生确认框，日志另有 7 天自动清理与单文件 5MB 上限。
...
- 首次运行会在“文档/LingBuilder/起始工作区”创建干净的“未命名解决方案 / 新建项目”，只复制安装包内置模块等必要资源，不会携带开发仓库项目；以后从安装版专用的 `userData/workspace-state.packaged.json` 恢复最近工作区，开发版继续使用独立的 `workspace-state.json`（保留最近 200 条，`RECENT_WORKSPACES_LIMIT`）。欢迎页最近工作区支持按名称/路径实时搜索，「查看全部」打开全量选择器（方向键选择、Enter 打开、悬停可从列表移除），文件菜单在 5 条之外提供「更多最近工作区…」打开同一选择器。文件菜单、命令面板、解决方案根节点右键菜单和载入失败页均可“关闭当前解决方案”，该操作保留原磁盘文件并切换到新的空白工作区。工具栏“打开”使用原生选择器并由主进程自动切换受管本地服务，开发版与安装版行为一致。
...
- 系统 AI 编辑草稿的 renderer 事件桥接按请求 ID 暂存用户原始提示词；旧版云端 `edit_draft` 未携带 `instruction` 时仍按本地提示词执行布局/美化回退，完成或失败后立即清理，禁止把一次请求的布局语义带入下一次请求。
...
- 内置 `lingbuilder.edgeview` 当前版本 `1.5.0`、最低生成器 `0.2.7`。窗口、分组框或选项卡内每个控件都有独立 HWND、Environment、Controller、WebView、Profile 和默认 `.edgeview/<controlId>` UDF；属性面板包含稳定运行期属性和 v2 创建期选项，并明确提示修改创建期属性后重建。`designer.edgeview.previewControl` 继续使用独立原生窗口，不向 React 画布嵌入 HWND。
...
- EdgeView 共提供 289 条中文命令：237 条目录化安全 API 和 52 条基础与实例命令。1.4.0 起新增**实例编号寻址**能力族：`EdgeView_创建弹窗浏览器`（独立顶层窗口、任务栏可见、`WS_EX_APPWINDOW` 无 `WS_CHILD`、`WM_SIZE` 自适应、主窗销毁连带关弹窗）、实例版会话/Cookie（`EdgeView会话_批量置Cookie实例` 等，支持注入 `HttpOnly` Cookie）、实例级 UA（`EdgeView设置_置/取用户代理实例`）、实例生命周期（`EdgeView_关闭全部实例/枚举实例JSON/置实例可见/置实例大小/取实例大小JSON/置实例标题`），全部走 `实例编号` 整数寻址、不参与控件存在性门禁，面向多店铺独立弹窗 + Cookie/缓存隔离验证。1.5.0 起新增 `EdgeView_创建弹窗浏览器代理(…, 代理地址)`：每个弹窗可带专属 HTTP/HTTPS/SOCKS5 代理，配合不同 `独立缓存目录`（独立浏览器进程）实现**每店铺独立出口 IP**；基础 `创建弹窗浏览器` 传空代理回落 `EdgeView_设置全局代理`。v2 新增受管 Frame/Worker/Extension/Notification/Certificate/SharedBuffer/FileSystemHandle、完整 Options、资源响应正文、PDF 流、另存为和证书决策等。新处理器统一写 `&处理器名`；任务使用五态、`shared_ptr + generation` 和迟到回调拒绝，Loader 保持到进程退出。
- Cookie 注入三件套：`EdgeView会话_置Cookie带属性(控件名, 名称, 值, 域, 路径, 过期时间, 安全, 仅HTTP, 同源策略)` 单条写入含 WebView2 元数据（过期 UTC 秒、Secure、HttpOnly、SameSite 枚举 0 None、1 Lax、2 Strict）；`EdgeView会话_批量置Cookie(控件名, Cookie列表JSON)` 按 JSON 数组批量注入并返回成功条数，键名 `name/value/domain/path` + 可选 `expires/secure/httpOnly/sameSite`，与 `EdgeView会话_取Cookie异步` 输出（1.3.0 起附 `expires/secure/httpOnly/isSession/sameSite`）格式互相兼容，可完成「导出→换账号 Profile→回灌」闭环；旧 `EdgeView会话_置Cookie` 四参形态保持不变。
...
- `edgeViewApiCoverage.generated.json` 固定 SDK `1.0.3537.50` / Runtime 141 与 SDK `1.0.4078.44` / Runtime 150。新基线 995 个方法的结果为 public 338、internal 557、excluded 100、pending 0。运行 `npm run module:edgeview-coverage:complete` 检查双 SDK 哈希、目录、符号、测试和漂移；`npm run smoke:edgeview-native` 执行 Win32/x64 MSVC 原生冒烟。CompositionController、PointerInfo、AutomationProvider、实验 API、Host Object、裸 COM/指针继续排除。
- 2026-09-15 起 EdgeView 以 `WebView2LoaderStatic.lib` 静态链接，EXE 不再依赖同目录 `WebView2Loader.dll`，可单文件分发（目标机仍需系统 WebView2 Runtime）；F5/CLI 链接统一 `/MANIFEST:EMBED`，产物目录不再出现外置 `.exe.manifest`。窗口模型新增 `embeddedSite: { files, entry?, host? }`：网页静态文件编入 EXE RCDATA（ID 2101 起），生成运行时经 `WebResourceRequested` 对 `https://<host>/*` 内存应答，运行期零 HTML/JS/CSS 释放；`files` 为显式清单，dist 重建后需同步更新。端到端样例见 `AI 视频自主生产/进阶方案/AI智能助手-EdgeView/`、`Cat小助手-EdgeView/`、`抖音助手-EdgeView/`。
- 2026-09-19 无头浏览器（方案 C 全链）：① EdgeView 无头实例 `EdgeView_创建无头实例(代理)`——离屏 WS_POPUP 隐窗宿主（从不 ShowWindow），界面/任务栏零显示，实例编号命令与 71 项事件全可用；隐窗宿主不渲染像素，截图/PDF 一律改走 CDP；设计器非可视组件「EdgeView无头浏览器」按属性（实例编号/地址/缓存目录/UA/代理/自动启动）在窗口创建期自动建立实例，枚举 JSON 新增「是否无头」。② CDP 真无头 `CDP_启动浏览器(参数JSON,&就绪)` spawn 本机 msedge/chrome `--headless=new --remote-debugging-port=0` 独立进程、解析 DevToolsActivePort 自动连接，`CDP_停止浏览器/停止全部浏览器` 优雅退出+目录回收+进程退出自动清理，同步失败读新增的 `CDP_取最后错误`。③ 控制台底座：wmain 入口自动创建无头泵窗口（`LingBuilder_确保无头泵窗口`，WM_CREATE 泵分支只登记线程 owner），CDP/HTTP/WS/网页异步/线程完成处理器在控制台配合 `EdgeView_泵消息(毫秒)` 即可派发；弹窗闪窗治理 `EdgeView_创建弹窗浏览器初始隐藏(代理)`。真机验收：控制台演示 EdgeView 无头全链 + CDP Edg/152 真无头截图落盘，退出码 0；回归 `tests/edgeViewHeadless.test.ts`（已入 test:lingcpp）+ cdp 基线 144→148；EdgeView API.md 重生成 320 条。
...
> 2026-09-19：三内核（EdgeView/CEF3/FBro）多店铺能力对齐，CEF3 `3.0.0-alpha.4`、FBro shell `1.3.0`。**CEF3** 全为纯 TS 运行时改造：新增设计器无关的 `CEF3_创建弹窗浏览器(实例编号,地址,独立缓存目录,代理地址)`（`LB_CEF3_BrowserCreateChrome` 顶层窗 + 独立 profile + 每实例代理）、`CEF3_创建区域(实例编号,左,顶,宽,高,地址,独立缓存目录,代理地址)`（运行时 WS_CHILD 承载内嵌多实例）、公开 `CEF3_枚举实例JSON()`/`CEF3_关闭全部实例()`（包装原私有 `CEF3_关闭全部()`），实例登记进 `cefBrowsers_`（合成 controlId=1000000+实例编号）故枚举/关闭全部自动覆盖；`CEF3会话_取上下文实例(实例编号)` 让弹窗/区域复用全部句柄版 `CEF3会话_*` Cookie/会话命令；**实例级 UA** `CEF3_设置用户代理(控件名,UA)`/`CEF3_设置实例用户代理(实例编号,UA)`（+ `CEF3_取用户代理`/`CEF3_取实例用户代理` 回读）——CEF 无 per-browser settings，实现为点亮 `LB_CEF3_ResourceRequestHandlerSubscribeBeforeResourceLoad` 并在 `CEF3_Bridge事件回调V4` 的「资源加载前」事件里对桥 `RegisterRequest` 出的活请求句柄 `packet->subject` 调 `LB_CEF3_RequestSetHeaderByName(..,"User-Agent",..,1)` 逐实例改头，无需重编桥。**FBro** `浏览器外壳_设置实例Cookie` 透传 `HttpOnly/Secure/Domain/Path`、新增 `浏览器外壳_新建独立实例代理(…,代理地址,用户代理)`；fbro-shell 1.4.0 再补**内嵌区域动态** `浏览器外壳_新建内嵌实例区域(稳定ID,左,顶,宽,高,地址,缓存目录,代理地址,用户代理)` + `浏览器外壳_取内嵌区域实例JSON()`——复用已出货 `LING_FBRO_PROCESS_EMBEDDED` 跨进程内嵌（不改桥），把独立进程浏览器作为 WS_CHILD 挂到宿主窗口指定矩形承载子窗，可同屏内嵌多个各自独立 profile/代理/UA、随主窗口缩放/DPI 跟随。EdgeView 1.5.0 弹窗独立代理。全局清单指纹 commands 3694 / parameters 6445 / controlReferences 1307（`73ff26f9`/`b8780e80`），CEF3 文档用例 publicCommands 411。验证：`tsc`/双 tsc lint、CEF3/EdgeView/FBro docs/catalog/coverage 门禁、`npm run build` 全绿；**CEF3 真机原生编译+运行冒烟 PASS、FBro 外壳工程真机编译+链接+启动 PASS**（区域运行时代码一并编入；FBro 外壳顶栏 DPI 几何 smoke 是 new_emoji 布局另一议题，非本次引入）。详见 `更新记录/2026-09-19.md`、`LingBuilder AI 规则手册.md` 与 `docs/MODULE_ECOSYSTEM_IMPLEMENTATION.md`。
...
- `/api/environment/check` 执行真实只读环境探测，覆盖 Node.js、MSVC、Windows SDK、CMake、g++、clang++、WebView2 和平台信息；`ready` / `msvcBuildReady` 只在 MSVC 与 Windows SDK 满足默认 Win32 原生构建时为真，`cppCompilerAvailable` 单独表示存在任意 C++ 编译器，避免仅安装 g++/clang++ 时误报默认构建就绪。工具栏环境检查显示检测结果和中文修复提示，不再使用固定成功文本。工具菜单与命令面板可打开“环境修复中心”，通过固定微软下载地址和固定参数安装 C++ Build Tools 工作负载或 WebView2；API 不接受任意 URL、命令或参数。离线部署见 `docs/LINGBUILDER_OFFLINE_ENVIRONMENT_PACKAGE.md`。
...
- `lingbuilder.module.info`
- `lingbuilder.build.stop`
- `lingbuilder.run.wait`
- `lingbuilder.run.log`
...
外部 AI 项目质量结构性修复（2026-09-18 起）：① **项目文件归属隔离**——新建项目落 `src/<id>`、`config/<id>` 与默认项目 `src`/`config` 常态嵌套，`solutionService.readProjectFileSnapshots` 只剪「严格位于本项目根之下」的其他项目根，解决方案树/构建源码集合/项目全局变量与 DLL 命令解析/AI Bridge 上下文统一消费该出口（修复「MainWindow.lcpp、config.ini 在树里出现多份」「默认项目吸入别家 16 个 .lcpp 导致重复类定义」两类事故）；`project.create` 嵌套时 message 自动提示。② **控件引用门禁**——`edit.apply` 与 `build.run`/`native.preview`/`native.export` 对窗口项目最终源码按有效设计器模型校验 controlRef（含 `_控件名_被单击` 事件绑定），引用模型中不存在的控件即中文阻断并列出缺失控件与「updatedDesignerProject → apply → 再构建」修复路径；`edit.propose` 保持宽松，DLL/控制台/未注册项目不误伤，杜绝「源码有控件、设计器空、IDE 画布空白、exe 无控件」脱节。③ **MCP 代码组织引导**——`MCP_INSTRUCTIONS` 第 7 条要求外部 AI 将可复用逻辑与大批量 `@` 内嵌 C++ 下沉到功能库文件（一文件一库、限定名调用），禁止整个项目堆进单个 `MainWindow.lcpp`。

设计器上下文语义（2026-09-12 起）：

- **面板编辑链收口（2026-09-18 起）**：IDE AI 面板的 `/api/lingcpp/edit/propose|from-system-draft|apply` 与 AI Bridge 共用同一 `AiBridgeService` 实现（专用 yolo 实例——面板是本地受信 UI，提案预览 + 用户确认即批准环节），路径校验、设计器深比较、控件引用门禁、编码/EOL 保留与审计只维护一条链；`proposeLingCppEdit + applyWorkspaceEditToFiles` 不再作为第二套编辑事务入口，新增编辑类面板能力禁止绕回 `aiEditService` 裸调用。同时内嵌 `/api/ai-bridge` router **不再全局注入 planner**，外部 AI 的 `files[]` 草稿与独立 `lingbuilder ai-server` 行为完全一致（此前会被降级为本地假注释草稿）。系统 AI 的 Gemini planner 失败降级与 502 根因文案、apply 响应契约（`proposal/nextSourceCode/appliedFiles/designerProject`）保持不变。

- **组件表与代码组织视图（2026-09-18 起）**：`lingbuilder.lingcpp.diagnostics` 在窗口项目下额外返回两个只读视图（MCP 工具数仍为 23，未新增工具）。`designerInventory` 是设计器权威组件表：按「调用方 `designerProject` 优先 → 磁盘模型」解析，逐窗口返回 `controls[]`（`name`/`type`/`content`（超 80 字截断）/`x,y,width,height`/`visible`/`enabled`/`eventBindings`/`parentName`）、`controlCount`、`hiddenControlCount`、`sourceEventHandlers`（按窗口 `className` 匹配源码 `类` 的事件处理器）、`truncated`（单窗口超 160 控件截断）与项目 `nonVisualResources`；用户问「窗口里的组件都有哪些/为什么没显示」时外部 AI 直接读它（`hiddenControlCount>0` = 控件被设为 `Collapsed`），不再猜 `window-designer.json` 路径。`codeOrganization` 返回项目各 `.lcpp` 的行数与角色（`window-main`/`function-library`/`other`，固定文件不计）、现有功能库清单（含公开方法名）与中文处方，并在「窗口项目 + 零功能库 + 最大文件 ≥300 行」时追加 `lingcpp-code-organization-split-suggestion`（`info`），任一文件 ≥800 行升级为 `warning`——两条都不参与门禁、不阻断构建。设计器上下文为 `none`（未传模型且项目未注册）时 `designerInventory` 不返回、`codeOrganization.windowProject=false` 且不给建议，避免伪造组件表。阈值与截断上限是 `AiBridgeService` 的 `CODE_ORGANIZATION_*` / `DESIGNER_INVENTORY_MAX_CONTROLS_PER_WINDOW` 常量。两条面向中文口语的补强（同日）：① 模型有窗口但控件总数为 0（caller 传入与磁盘 persisted 都算）时给 `lingcpp-designer-controls-empty`（`warning`，不阻断），直说「画布空白、exe 里没有组件是因为控件从未进入设计器模型，只写 `.lcpp` 不产生界面」并指向 `updatedDesignerProject` → `edit.apply` → 再构建；② 拆分处方与 `MCP_INSTRUCTIONS`/`edit.propose` 描述显式声明「功能代码 / 功能性代码 / 公共代码」= 功能库（独立 `.lcpp` 里的 `功能库 名称 … 结束功能库` 块，`公开:` 段供 `库名.功能(...)` 调用），**不是**在原文件里多写本地函数，且 `suggestion` 内嵌 `createFunctionLibraryTemplate` 生成的可粘贴骨架、目标路径落在当前项目源码目录内。
...
`lingbuilder.build.run` 启动的 exe 同样由 `ManagedProcessService` 管理，不使用 detached/unref；同项目重跑会在编译前回收旧进程和日志流。AI Bridge HTTP/MCP 构建使用项目租约，CLI 收到 `SIGINT` / `SIGTERM` 时会拒绝新构建、等待在途租约结束，再停止全部受控运行进程；启动失败会以 `run-start` 阶段返回失败，不能报告假成功。运行期观测工具（2026-09-18 起）：`lingbuilder.build.stop` 只停止 Bridge 自己启动的受控进程（所有权限模式可用），`lingbuilder.run.wait` 等待进程退出并返回退出码（超时默认 30 秒、上限 600 秒），`lingbuilder.run.log` 读取最近一次受控运行的控制台输出（进程退出后仍可读，`tailLines` 取末尾 N 行）。
...
LingBuilder 模块标准为 `schemaVersion: 2`。旧 `.lbmod` v1 不再兼容，C++ 依赖写入 `targets[]`，中文命令到 C++ 的确定性映射写入 `bindings.commands[]`。`docs/模块开发手册.md` 是面向外部模块作者的正式说明。
...
欢迎页「新建模块」提供两个模块作者入口（2026-09-20 起）：「从模板新建」调 `POST /api/modules/developer/create` 在 `.lingbuilder/module-build/<id>` 生成 manifest v2 骨架，「打开 .lbmod 模块包」调 `POST /api/modules/developer/open-package` 把模块包解开为可编辑源码；两者成功后都自动登记开发源（module-links.json），编辑清单与源码后用模块页「模块包导出」发布 .lbmod，与 CLI `module init` / MCP `module.scaffold` 共用同一 `createModuleTemplate` 骨架。
...
> 2026-09-18：账号登录统一走工作台顶层登录弹窗（`src/services/workbench/cloudAccountLoginService.ts` + `CloudAccountLoginDialog`）：收费模块门禁发现未登录时自动弹出并在登录成功后继续启用；弹窗内「忘记密码？」为两步重置（先 `POST /v1/auth/password/forgot` 按邮箱发 30 分钟令牌邮件，再 `password/reset` 粘贴令牌+新密码，成功后回登录态并提示全设备会话已注销）。AI 助手面板登录区同入口复用该弹窗（`initialMode: 'reset'`）。云端 `password/forgot` 带 IP 限流（20 次/15 分钟）且防枚举恒返回 ok；本地链路为 `cloud-account:forgot-password|reset-password` IPC → `cloudAccountService`。

> 2026-09-19：账号入口全界面常驻并把登录/注册/充值收为单一实现（此前注册表单只藏在 AI 面板「AI 对接设置 → 系统 AI」二级折叠里，标题栏、菜单栏、欢迎页、命令面板零入口）。新增 `src/services/workbench/cloudAccountSessionStore.ts` 作为账号会话与点数的**唯一状态源**（`subscribeCloudAccountSession` / `refreshCloudAccountSession` / `applyCloudAccountSignedIn|Balance|SignedOut` / `signOutCloudAccount`），标题栏徽标、AI 面板、设置页、帮助菜单一律消费它，不得各自维护 `cloudSession` 副本；无 `cloudAccount` IPC 的宿主（Web 预览、node 测试）刷新时必须落回未登录并给中文提示，禁止沿用伪登录态。`cloudAccountLoginService` 的 `initialMode` 扩为 `login | register | reset`，`CloudAccountLoginDialog` 因此成为全 IDE 唯一账号表单（含注册形态，注册成功带着邮箱回登录态）；`App.tsx` 顶层按请求 ID 给对话框加 `key`，并在**欢迎页与工作台区两个渲染分支都挂载**账号对话框。充值新增 `cloudAccountRechargeService.ts` + `CloudAccountRechargeDialog.tsx`（套餐、下单、二维码/收银台、每 3 秒轮询到账并写回会话源），`AiAssistant.tsx` 不再自带登录表单与充值状态。常驻入口：`CloudAccountTitleBarEntry`（未登录显示「登录」，已登录显示点数徽标 + 邮箱/充值/退出下拉）、`WelcomePage` 底部链接区、帮助菜单账号分组、设置页新增「账号」分类 `AccountSettings`（只呈现状态，动作仍走共享对话框）。命令面板注册 `workbench.action.account.login|register|resetPassword|recharge|logout`。回归锚点：`tests/cloudAccountEntry.test.tsx`（入口齐备 + 表单与充值实现各只有一份 + 到账写回会话源）、`tests/cloudAccountLoginDialog.test.tsx`（注册形态渲染与服务透传）。本节改动不涉及 IPC 契约、云端接口与令牌存储位置。
...
`lingbuilder.data.protobuf` 使用受控 `lingbuilder.protobuf.protoc`，固定 SDK 版本 27.3.0。桌面版安装包随附该 SDK（`resources/default-workspace/.lingbuilder/toolchains/protobuf`），IDE 打开工作区时自动铺设到 `<工作区>/.lingbuilder/toolchains/protobuf`（只补缺失文件，不覆盖已有内容；新建工作区随模板自带）。SDK 布局按架构拆分：`bin/protoc.exe`、`bin/<arch>/libprotobuf.dll` + `abseil_dll.dll`、`lib/<arch>/libprotobuf.lib` + `abseil_dll.lib`、`include/**`；`runtime-manifest.json` 逐文件校验大小和 SHA-256，并在缺失、篡改、版本、架构不符时阻断 F5、AI Bridge、CLI 和 VS 导出。IDE 自身不联网下载，也不会调用 PATH 中的系统 protoc；SDK 二进制不入库，打包机/开发机用 `npm run protobuf:prepare` 重建、`npm run protobuf:manifest` 重新生成清单。完整 `.pb.h`、`.pb.cc`、descriptor set、`.proto` 输入和运行时依赖会进入可复制导出目录。

启用该模块的工程由构建管线自动注入消费条件：全部编译单元（含生成的 `.pb.cc`）定义 `PROTOBUF_USE_DLLS` + `ABSL_CONSUME_DLL`，链接 `libprotobuf.lib` + `abseil_dll.lib`，最低 `/std:c++17` 并全配置使用 `/MD`（与 SDK 二进制一致；缺这些定义会分别导致运行期虚表 AV 或 LNK2019）。项目里没有任何 `.proto` 文件时模块保持惰性，不要求 SDK 就绪。
...
AI 使用 `剪贴板_置文本`、`剪贴板_取文本` 等命令时，项目必须启用 `lingbuilder.system.clipboard` 模块；F5 会按项目模块清单生成对应 Windows 剪贴板运行时。

AI 生成的控件属性赋值也必须使用当前设计器中的裸控件名，例如 `编辑框1.内容 = "结果"`。语言服务会按当前窗口设计器符号解析这类赋值；不存在的控件、跨窗口控件或不兼容字段仍会阻止 F5 构建。

`.lcpp` 编辑助手会把当前窗口设计器的完整 `designerProject` 与工作区源码一起提供给模型。用户提出“增加进度条”“移动按钮”“调整窗口大小”等需求时，AI 必须返回完整源码文件和完整设计器模型；系统 AI、BYOK、AI Bridge、MCP 使用相同契约。

F5、AI Bridge 和源码包读取只把项目源码根目录中的真实 `.lcpp` 作为输入；`.lingbuilder-build`、`generated/cpp`、`dist`、`node_modules` 等生成/工具目录会被统一排除。生成目录中的源码快照不会与真实源码重复聚合，现有产物也不会因此被自动删除。

编辑结果先显示为可审查提案。提案确认后，Electron 同时更新源码编辑器、`window-designer.json` 和设计器画布；涉及进度条时会校验 `content` 与 `properties.value` 同步。应用阶段会重新读取磁盘模型，若模型已被其他操作修改、模型无效或调用方快照过期，则拒绝覆盖并要求重新生成提案。未确认或被拒绝的提案不会改动源码或布局。

设计器提案校验同时识别启用模块贡献的控件和旧项目兼容的 `Upload` / `DragUpload`。旧控件在 AI 仅调整布局时会原样保留，新增未知控件仍会被阻断。提案生成、系统 AI 草稿和应用接口遇到模型错误或并发冲突时返回中文 HTTP 错误，不能使本地 Electron 服务进程退出。

设计器控件 `type` 有规范标识清单（编辑框是 `TextBox`，`Edit` 只是 Win32 原生类名，不是合法标识）。编辑提示词会注入完整的合法类型清单（内置 44 个 + 启用模块贡献类型）；提案校验前会把 `Edit`、`输入框`、`编辑框`、`textbox` 等常见别名保守归一化为规范标识（仅当目标类型在当前允许集合内才生效），BYOK 生成侧对类型校验失败还会携带中文错误自动纠正重试一次；归一化与重试都无法识别的未知类型仍被中文阻断。提案对象携带生成时的 `designerAllowedControlTypes` 快照，应用接口复用同一集合，模块贡献控件不会在应用阶段被默认集合误拒。
布局提案还会校验 `projectId === designerProject.id`，并从当前嵌套工作区项目读取模型；模型未发生任何变化时不会创建“成功”提案。点击应用后，源码、设计器状态和 `window-designer.json` 会统一写入磁盘，避免重启后布局回退。

AI 编辑分流按用户提示中的窗口/控件目标与布局变更意图判断，不依赖当前活动文件扩展名。因此当前打开 `config.ini`、`.cpp` 等文件时提出“当前的界面太乱了，帮我美化一下”仍会进入设计器编辑提案；系统 AI 的上下文会优先包含活动文件和项目内 `.lcpp` 文件，避免只把配置文件交给模型。
...
# AI 点数预扣规则（2026-08-22，2026-09-11 更新）

云端 AI 请求的点数预冻结按有界输出估算（16,384 token）计算，不再随模型输出上限全额放大；显式 `maxOutputTokens` 会被限制在模型上限内（生产库模型输出上限已开至 DeepSeek 的 393,216 token），最终仍按供应商实际用量结算、多退少补。编辑流未指定输出上限时默认预算 32,768 token。
## AI 对话路由（2026-08-22）
- 打开 `.lcpp` 文件时，普通提问仍使用聊天接口；仅包含明确修改意图的请求才生成源码/设计器编辑草案，并要求用户审阅后应用。
- 聊天回复因输出长度上限被截断时，AI 助手会追加「回复达到输出长度上限」提示；模型只输出思考、未产出正文时，会看到中文错误提示且思考内容自动提升为回复正文，「AI 思考过程」折叠块随会话历史保存。
