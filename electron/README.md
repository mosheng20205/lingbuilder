...
> 2026-07-31：FBro 核心与 Bridge 升级为 2.0.0。新增结构化 v2 事件包、2 秒同步事件决策、独立 Chrome UI 实例状态、受管异步任务/缓冲/对象句柄，以及 events/session/transfer/automation/objects/network/vip 七个依赖子模块；v1 C ABI 和原 21 条中文命令继续兼容，核心现共有 42 条命令。objects 已扩展为 132 条高级命令；session 为 10 条，transfer 为 5 条。automation 现有 25 条命令，Frame 通过类型 16 的受管句柄提供主/焦点/按标识/按名称查找、列表 JSON、属性、父框架、所属浏览器、编辑动作、加载和脚本执行，普通 Win32 与 New_Emoji 共用相同 binding。`npm run module:fbro-coverage:check` 对 FBro 5.38.49 / CEF 135.0.21 的 77 个头文件、1079 个导出签名和 158 个事件/回调签名做漂移、codec、分类与实现状态门禁；当前 218 项已实现、857 项待实现、3 项内部不适用和 1 项高级替代不适用，接通 9 个事件。普通 Win32 MSVC x64 冒烟已验证 Frame、PDF、Cookie、图像、证书与生命周期；有效截图由 `LINGBUILDER_FBRO_VIP_KEY` 门控。Frame visitor/DOM/V8、真实 OSR、完整事件和其余网络/VIP 仍未接通，不得描述为全功能完成。
