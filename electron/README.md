...
窗口设计器工具箱还提供非可视“上下文菜单”和“弹出菜单”。上下文菜单绑定窗口或控件并响应原生右键消息；弹出菜单通过 `弹出菜单_显示` 或 `弹出菜单_在坐标显示` 主动打开。两者共用结构化菜单项模型，F5 和原生导出使用真实 Win32 `HMENU`，菜单项事件按稳定 ID 绑定；`菜单_取最后项目` 可读取最近选择项。
