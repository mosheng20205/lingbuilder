...
test('属性面板边框下拉锚定：统一选项来源、拖动开关、最大化禁用、缩放开关禁用', () => {
  assert.ok(designerSource.includes('DEFAULT_WINDOW_BORDER_STYLE, LING_WINDOW_BORDER_STYLE_OPTIONS, deriveLingWindowBorderStyle'));
...
test('边框切换与缩放 flag 同步 borderStyle 派生', () => {
  ...
  assert.ok(designerSource.includes('flag === 0x08 ? {'));
  assert.ok(designerSource.includes("window.borderStyle === 'normal-fixed' ? 'normal-resizable'"));
  assert.ok(designerSource.includes("window.borderStyle === 'frame-resizable' ? 'frame-fixed'"));
});
...
test('windowFrame 派生与内容偏移统一消费边框映射', () => {
  ...
  assert.ok(serviceSource.includes("captionKind === 'thin' ? 20 : DESIGNER_TITLE_BAR_HEIGHT"));
});
