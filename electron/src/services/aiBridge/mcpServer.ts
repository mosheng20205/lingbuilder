...
const MCP_INSTRUCTIONS = [
  ...
  '3. lingbuilder.modules.list 只返回模块摘要；需要某模块的完整命令签名、参数说明、公开常量（constants）和示例时，用 lingbuilder.module.info 按 moduleId 查询单个模块。模块公开常量在 .lcpp 源码中必须用 #常量名 引用（裸名不带引号），生成期物化为编译期常量。命令很多的大模块（如 new_emoji 数千条）不要整表翻页：module.info 的 demoExample 给出演示语料里的真实调用行（参数顺序可直接照抄），designerControls 给出该模块可设计的控件清单，传 control 取单个控件的完整属性/事件/代码创建契约（唯一命中时还会内联该控件的组件卡 componentGuide：可粘贴骨架 + 属性枚举取值 + 事件处理器命名 + 人工红线，并给出上游 EU_* API 文档路径），传 example 直接取回一条界面配方正文无头浏览器另有三种形态按需求选：EdgeView_创建无头实例(代理) 是隐窗伪无头（WebView2 无官方无头 API，离屏宿主从不可见；实例编号命令全可用，但截图/PDF 不可用）；CDP_启动浏览器(参数JSON,&就绪) 是真无头（本机 msedge/chrome --headless=new 独立进程，截图/PDF/网络拦截全真，配 CDP_停止浏览器/停止全部浏览器 回收，同步失败读 CDP_取最后错误）。同步失败读 CDP_取最后错误）。CEF3 无头是官方 OSR 的第四形态，真无头且不创建任何窗口：CEF3_创建无头浏览器(实例编号, 地址, 独立缓存目录, 代理地址, 视口宽, 视口高)（窗口项目也可用设计器「CEF3无头浏览器」非可视组件，实例编号在 CEF3 无头族内项目级唯一）。取内容两条路：CEF3无头_取主框架 拿框架句柄后接 CEF3框架_*/CEF3填表_*/CEF3DOM_*（句柄版命令零改动可用），或直接 CEF3无头_执行JS / CEF3无头_取页面文本 / CEF3无头_取页面源码；浏览器级控制（重绘/帧率/回调订阅位）用 CEF3无头_取浏览器句柄 的句柄接 lingbuilder.cef3.osr 模块的 CEF3OSR_*/CEF3离屏_*。三条红线：受管句柄判有效只用 != 0（64 位受管值按 int64 恒为负数，写 > 0 会把合法句柄全部判无效）；取内容前先 CEF3无头_等待加载完成，判断「真的在渲染」用 CEF3无头_等待出帧（加载完成≠已出帧，真机首帧晚约半秒）；截图未开放（CEF3离屏_订阅像素帧 只点亮订阅位、不外发帧内容），禁止创建任何窗口再隐藏来冒充无头。CEF3无头_执行JS 返回 DevTools 信封 JSON 不是裸值；控制台派发不了事件处理器，事件只能轮询 CEF3无头_取事件JSON。控制台入口自带无头泵窗口，异步完成处理器配合 EdgeView_泵消息(毫秒) 轮询派发；需要建窗但暂不显示用 EdgeView_创建弹窗浏览器初始隐藏(代理)。',
