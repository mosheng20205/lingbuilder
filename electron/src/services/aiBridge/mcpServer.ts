...
const MCP_INSTRUCTIONS = [
  ...
  '18. DLL（动态库）输出与构建：两条路径按需求选。A) 通用路径（推荐）：lingbuilder.project.create 用窗口模板（blank-window/hello-window/sqlite-crud-window）并传 outputType="dll"——项目里「公开」节的子程序会确定性导出为 DLL 接口（extern "C" + __cdecl，事件处理器/私有成员不导出，跨 DLL 边界只允许 POD 与文本签名），build.run 产出 <产物名>.dll + 同名导入库 .lib（位于构建输出目录 bin 下），不生成 wWinMain 与消息循环；B) C ABI 示例库：templateId="windows-dll" 创建 MSVC DLL 工程（DllApi.lcpp + DllMain.cpp + exports.def + vcxproj，产物 LingBuilder_GetApiVersion），目前只有 获取接口版本 的整数返回值确定性映射到导出函数，不要生成「任意中文过程自动成为导出函数」的代码。共用红线：DLL 项目没有运行入口——build.run 传 run=true 会被拒绝并提示改用生成，禁止再调 run.wait/run.log 等进程；DLL 构建与导出必须 MSVC（缺失 MSBuild 时构建报中文安装指引，含 LINGBUILDER_MSBUILD_PATH 环境变量修法），g++/clang 不能产出 DLL；outputType 是解决方案项目属性，创建后如需改回 exe 由用户在 IDE「构建属性」里操作，外部 AI 无对应工具。',
  '19. 动手读写任何项目文件前必须先确定目标项目：lingbuilder.workspace.list 首项的 projects[] 视图列出解决方案中全部已注册项目（id/name/sourceRoot/type/isWindowProject），startupProjectId 字段是当前启动项目；单项目工作区直接用该 projectId，多项目工作区禁止按目录名猜（默认项目的 src/ 占位与各项目的 src/<项目id>/ 嵌套并存，同名 MainWindow.lcpp 很常见，猜错会把改动写进别的项目且全链路不会报错），调 lingbuilder.lingcpp.diagnostics / edit.propose / build.run / native.* 时显式传 projectId，并在提案与回复里写明「本次改的是项目 X（projectId）」。projects[] 缺失时读 .lingbuilder/solution.json 核对；单项目工作区的 diagnostics / edit.propose 响应自带 projectHint 字段，直接引用其中的 projectId。',
  '20. 「零内嵌 C++」的文本与文件能力口径（不要再写 @ 内联 C++ 处理数字转文本、表格排版、字符串拼接或文件时间）：',
  '① 任意值转文本统一 到文本(值)（整数/长整数/小数/逻辑/文本，逻辑值给「真/假」）：它返回标准文本值，可直接参与拼接与比较，`显示 = "共 " + 到文本(个数) + " 条"` 与 `到文本(a) + 文本变量` 都合法；不要为转数字写 @ 行或 std::to_wstring。',
  '② 数组元素取值用 数组_取成员(数组, 下标)，返回文本类型的值可直接拼接（不要再用内联 C++ 取成员）。',
  '③ 报告表格排版用 std.text 定宽族：文本_填充右边(文本, 目标显示宽度[, 填充字符 = " "])、文本_填充左边(...)、文本_居中(...)、文本_取显示宽度(文本[, 中日韩按 2 列 = 真])。红线：目标宽度是「显示列数」不是字符数——中日韩全角字符 2 列、组合符号 0 列、其余 1 列，中英文混排必须用 文本_取显示宽度 算宽度（文本_取长度 只数字符数，用它对齐必然错位）；已够宽时原样返回、不截断。',
  '④ 数组拼串用 文本_连接(文本数组, 分隔符[, 起始下标 = 0, 数量])（文本_分割 的反向操作；数量省略或 <=0 表示连到末尾），拼 JSON/CSV 记录不要再手写「首元素不加逗号」的累加逻辑。',
  '⑤ 文本_分割(文本, 分隔符, 结果数组[, 忽略末尾空段 = 假])：默认保留末尾空段（"a|b|" 拆出 3 段，末段是空串）；累积拼接串（串 = 串 + 字段 + "|"）必须传第 4 参 真，否则段数比实际记录多 1（状态栏「已载入 3 个」而列表只有 2 项就是这么来的）；中间空段（"a||b"）两种取值都保留。',
  '⑥ 文件时间用 lingbuilder.fs.core：文件_取修改时间(路径) / 文件_取创建时间(路径) / 文件_取访问时间(路径) 返回「日期时间」（与 std.datetime 同一 64 位表示，可直接与 时间_取间隔 / 时间_增减 / 时间_取年份 配合比较），挑目录里最新的文件用 文件_取最新文件(目录, 通配符, 结果数组)（只写入 1 条最新文件完整路径，返回 1 成功 / 0 失败）；失败返回 0 并用 文件_取错误() 读中文原因；不要为取文件时间写 GetFileAttributesExW 之类的内联 C++。',
  '⑦ 多行编辑框（多行 TextBox / RichEdit）写入时只认 CRLF：控件_设置文本 会自动把裸 LF 补成 CRLF，源码里直接写 "\n" 即可，不要手工拼回车换行，也不要用内联 C++ 改换行符。',
  '⑧ 功能库需要缓存（例如只枚举一次磁盘、后续调用复用）时用项目全局变量：在“项目全局变量.lcpp”里写 `全局 文本型 名称 = 初值`（数组写 `全局 文本型 名称[]`），功能库与窗口源码都能直接读写；漏写「全局」关键字会被中文诊断拦住并给出改法，不要为此把状态塞进 @ 内联 C++。',
  '⑨ 字节集转十六进制：要「直接拿到十六进制文本」用 字节集_到十六进制文本(数据)（std.bytes，返回文本型，一步可显示/拼接）；字节集_到十六进制字节集(数据)（旧名 字节集_十六进制编码 仍作为别名可用）返回的是十六进制字节集而不是文本——把它当文本传给文本型形参会被实参类型门禁用中文诊断拦下并给出转换建议。',
  '约束：所有写入与构建都限定在工作区内；独立一行「结束」是块结束标记不是退出命令，显式退出用 结束()。'
].join('\n');

const TOOLS = [
  { name: 'lingbuilder.workspace.list', description: '列出 LingBuilder 工作区文件树。返回内容首项是合成的 workspace 根条目（type="workspace"，其 workspaceRoot 字段=当前 Bridge 工作区根绝对路径，ideVersion 字段=当前 LingBuilder IDE 版本）——开始任何读写前先读首项确认自己连到的工作区；stdio 模式下工作区=客户端启动目录，若与用户项目目录不符，请让客户端切换启动目录后重启 MCP 宿主，不要继续按错误工作区生成提案。首项的 projects[] 字段列出解决方案中全部已注册项目（id/name/sourceRoot/type/isWindowProject），startupProjectId=当前启动项目：动手前先据此确定目标项目，多项目工作区禁止按目录名猜。', inputSchema: objectSchema({}) },
  ...
  { name: 'lingbuilder.project.create', description: '预览或创建 LingBuilder 项目；不传 approved=true 时只返回项目文件、项目级模块清单和模块依赖预览，窗口项目额外返回设计器模型。省略 enabledModuleIds 时继承根目录 .lingbuilder/project-modules.json，传空数组表示仅使用基础模块；创建后使用 result.project.id 作为项目上下文，窗口项目再使用 result.designerProject 进行原生预览/导出/构建。窗口模板可传 outputType="dll" 直接创建「把「公开」子程序导出为 DLL 接口」的项目（控制台与 new_emoji 模板会拒绝；windows-dll 模板本身就是动态库，无需也不必传 exe）。windows-dll 项目不返回 designerProject，工作台导航直接定位到 DllApi.lcpp；windows-console 项目返回以“程序”类为宿主的最小设计器模型，工作台导航定位到 程序.lcpp，构建产物为控制台可执行文件（入口为“公开 整数型 启动()”子程序）；sqlite-crud-window 模板创建「表单输入 + 列表视图 + SQLite 增删改查」的完整可运行示例。', inputSchema: objectSchema({ name: { type: 'string', maxLength: 100 }, projectId: { type: 'string', maxLength: 80 }, templateId: { type: 'string', enum: ['blank-window', 'hello-window', 'sqlite-crud-window', 'new-emoji-fbro-browser-shell', 'windows-dll', 'windows-console'] }, windowTitle: { type: 'string', maxLength: 120 }, outputType: { type: 'string', enum: ['exe', 'dll'], description: '仅窗口模板有效：dll 把「公开」子程序导出为 DLL 接口并产出 .dll + 导入库 .lib；缺省 exe。' }, enabledModuleIds: { type: 'array', maxItems: 64, description: '省略时继承全局项目模块；显式传 [] 仅启用基础模块。', items: { type: 'string', minLength: 2, maxLength: 128 } }, openInWorkbench: { type: 'boolean' }, approved: { type: 'boolean' } }) },
