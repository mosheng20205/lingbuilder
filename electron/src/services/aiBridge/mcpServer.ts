...
const MCP_INSTRUCTIONS = [
  ...
  '3. lingbuilder.modules.list 只返回模块摘要；需要某模块的完整命令签名、参数说明、公开常量（constants）和示例时，用 lingbuilder.module.info 按 moduleId 查询单个模块。模块公开常量在 .lcpp 源码中必须用 #常量名 引用（裸名不带引号），生成期物化为编译期常量。命令很多的大模块（如 new_emoji 数千条）不要整表翻页：module.info 的 demoExample 给出演示语料里的真实调用行（参数顺序可直接照抄），designerControls 给出该模块可设计的控件清单，传 control 取单个控件的完整属性/事件/代码创建契约（唯一命中时还会内联该控件的组件卡 componentGuide：可粘贴骨架 + 属性枚举取值 + 事件处理器命名 + 人工红线，并给出上游 EU_* API 文档路径），传 example 直接取回一条界面配方正文无头浏览器另有两种形态按需求选：EdgeView_创建无头实例(代理) 是隐窗伪无头（WebView2 无官方无头 API，离屏宿主从不可见；实例编号命令全可用，但截图/PDF 不可用）；CDP_启动浏览器(参数JSON,&就绪) 是真无头（本机 msedge/chrome --headless=new 独立进程，截图/PDF/网络拦截全真，配 CDP_停止浏览器/停止全部浏览器 回收，同步失败读 CDP_取最后错误）。控制台入口自带无头泵窗口，异步完成处理器配合 EdgeView_泵消息(毫秒) 轮询派发；需要建窗但暂不显示用 EdgeView_创建弹窗浏览器初始隐藏(代理)。',
