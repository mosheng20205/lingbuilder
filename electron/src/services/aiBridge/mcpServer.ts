...
const TOOLS = [
  ...
  { name: 'lingbuilder.lingcpp.diagnostics', description: '返回 .lcpp 解析与语义诊断；项目诊断应同时传 projectId 和完整 designerProject，以校验 controlRef 与事件绑定。', inputSchema: objectSchema({ filePath: { type: 'string', minLength: 1 }, sourceCode: { type: 'string', maxLength: 2097152 }, projectId: { type: 'string', maxLength: 128 }, designerProject: { type: 'object', description: '当前项目的完整窗口设计器模型。' } }, ['filePath']) },
  { name: 'lingbuilder.edit.propose', description: '根据外部 AI 提供的完整文件草稿生成可预览提案；窗口项目可同时传入当前 designerProject 和修改后的 updatedDesignerProject，源码和布局将在同一提案中校验。', inputSchema: objectSchema({ filePath: { type: 'string', minLength: 1 }, instruction: { type: 'string', minLength: 1, maxLength: 4000 }, sourceCode: { type: 'string', maxLength: 2097152 }, projectId: { type: 'string', maxLength: 128 }, workspaceFiles: lingCppWorkspaceFilesSchema, designerProject: { type: 'object', description: '当前完整窗口设计器模型。' }, updatedDesignerProject: { type: 'object', description: '修改后的完整窗口设计器模型；涉及布局时必须传入。' }, files: { type: 'array', minItems: 1, maxItems: 5, description: '每个修改文件的完整新内容，不是 diff 或代码片段。', items: objectSchema({ filePath: { type: 'string', minLength: 1 }, updatedSource: { type: 'string', maxLength: 2097152 } }, ['filePath', 'updatedSource']) } }, ['filePath', 'instruction', 'files']) },
  ...
  { name: 'lingbuilder.project.create', description: '预览或创建 LingBuilder 项目；不传 approved=true 时只返回项目文件、项目级模块清单和模块依赖预览，窗口项目额外返回设计器模型。省略 enabledModuleIds 时继承根目录 .lingbuilder/project-modules.json，传空数组表示仅使用基础模块；创建后使用 result.project.id 作为项目上下文，窗口项目再使用 result.designerProject 进行原生预览/导出/构建。windows-dll 项目不返回 designerProject，工作台导航直接定位到 DllApi.lcpp。', inputSchema: objectSchema({ name: { type: 'string', maxLength: 100 }, projectId: { type: 'string', maxLength: 80 }, templateId: { type: 'string', enum: ['blank-window', 'hello-window', 'new-emoji-fbro-browser-shell', 'windows-dll'] }, windowTitle: { type: 'string', maxLength: 120 }, enabledModuleIds: { type: 'array', maxItems: 64, description: '省略时继承全局项目模块；显式传 [] 仅启用基础模块。', items: { type: 'string', minLength: 2, maxLength: 128 } }, openInWorkbench: { type: 'boolean' }, approved: { type: 'boolean' } }) },
...
function createProtocolServer(service: AiBridgeService, observe?: ObserveActivity, clientId = 'stdio'): Server {
  const server = new Server({ name: 'lingbuilder-ai-bridge', version: '0.6.0' }, { capabilities: { tools: {} } });
