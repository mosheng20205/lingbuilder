...
const TOKEN_PATTERN = /((?:u8|u|U|L)?"(?:(?:\\.)|[^"\\])*"|“[^”]*”|否则如果|如果结束|调试输出|输出调试文本|信息框|打开窗口|窗口_打开|载入窗口|载入新窗口|载入可视化设计|读取配置项|取运行目录|如果真|如果|否则|结束|返回|文本型|整数型|逻辑型|小数型|长整数型|双精度小数型|字节集|日期时间型|真|假|@|\d+(?:\.\d+)?|[\w\u4e00-\u9fa5]+|::|[＝=＋+\-*/（）(),，:;.<>\[\]{}])/gu;

const BUILTIN_COMMANDS = /^(调试输出|输出调试文本|信息框|打开窗口|窗口_打开|载入窗口|载入新窗口|载入可视化设计|读取配置项|取运行目录)$/u;
const LING_CPP_KEYWORDS = /^(如果真|否则如果|否则|如果结束|如果|结束|返回)$/u;
