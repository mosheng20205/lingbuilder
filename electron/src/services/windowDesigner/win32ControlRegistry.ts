export type Win32ControlModuleId = 'lingbuilder.win32.basic' | 'lingbuilder.win32.common-controls' | 'lingbuilder.new_emoji.ui' | 'lingbuilder.cef3.browser';
...
export const WIN32_CONTROL_DEFINITIONS: Win32ControlDefinition[] = [
  ...
  nonVisual({ type: 'ContextMenu', label: '上下文菜单', moduleId: 'lingbuilder.win32.common-controls', category: '非可视', icon: 'Menu', nativeClass: 'HMENU', nativeAdapter: 'context-menu-resource', defaultProps: { content: '菜单项', width: 0, height: 0 }, properties: [], events: [event('ItemSelected', '菜单项被选择', '菜单项被选择', 'command')] }),
  nonVisual({ type: 'PopupMenu', label: '弹出菜单', moduleId: 'lingbuilder.win32.common-controls', category: '非可视', icon: 'PanelTopOpen', nativeClass: 'HMENU', nativeAdapter: 'popup-menu-resource', defaultProps: { content: '菜单项', width: 0, height: 0 }, properties: [], events: [event('ItemSelected', '菜单项被选择', '菜单项被选择', 'command')] }),
  ...
  control({ type: 'CefBrowser', label: 'CEF3浏览器', moduleId: 'lingbuilder.cef3.browser', category: '媒体', icon: 'Globe', nativeClass: 'STATIC', nativeAdapter: 'cef3-browser', defaultProps: { content: 'CEF3浏览器', width: 480, height: 320, background: '#FFFFFF' }, properties: [text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '缓存目录', '.cef3/cache'), text('userAgent', 'User-Agent 标识'), bool('enableJs', '启用 JavaScript', true), bool('enableDevTools', '允许开发者工具', true), bool('loadImages', '加载图片', true), bool('enableWebGL', '启用 WebGL'), bool('muteAudio', '静音'), enumProp('proxyMode', '代理模式', 'system', ['system', 'none', 'custom']), text('proxyServer', '自定义代理地址')], events: [event('LoadStart', '开始加载', '开始加载', 'notify'), event('LoadEnd', '加载完成', '加载完成', 'notify'), event('LoadError', '加载失败', '加载失败', 'notify'), event('TitleChanged', '标题被改变', '标题被改变', 'notify'), event('AddressChanged', '地址被改变', '地址被改变', 'notify')], requiredLibraries: ['libcef.lib', 'libcef_dll_wrapper.lib'] })
];
