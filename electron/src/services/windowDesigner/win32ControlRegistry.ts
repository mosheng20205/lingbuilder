...
export const WIN32_CONTROL_DEFINITIONS: Win32ControlDefinition[] = [
  ...
  control({ type: 'ListView', label: '列表视图', moduleId: 'lingbuilder.win32.common-controls', category: '集合', icon: 'Table', nativeClass: 'SysListView32', nativeAdapter: 'listview', defaultProps: { content: '', width: 320, height: 180 }, properties: [{ key: 'columns', label: '列集合', type: 'columns', defaultValue: [] }, { key: 'items', label: '行项目', type: 'columns', defaultValue: [] }, enumProp('view', '视图模式', 'details', ['icon', 'smallIcon', 'list', 'details']), bool('gridLines', '显示网格线', true), bool('multiple', '允许多选'), text('imageListId', '图像列表 ID')], events: [event('SelectionChanged', '选择项被改变', '选择项被改变', 'notify'), event('DoubleClick', '被双击', '被双击', 'notify'), event('ColumnClick', '列被单击', '列被单击', 'notify')] }),
  control({ type: 'TreeView', label: '树形视图', moduleId: 'lingbuilder.win32.common-controls', category: '集合', icon: 'ListTree', nativeClass: 'SysTreeView32', nativeAdapter: 'treeview', defaultProps: { content: '', width: 260, height: 200 }, properties: [{ key: 'nodes', label: '节点集合', type: 'treeNodes', defaultValue: [] }, bool('showLines', '显示连接线', true), bool('checkBoxes', '显示复选框'), text('imageListId', '图像列表 ID')], events: [event('SelectionChanged', '选择节点被改变', '选择节点被改变', 'notify'), event('Expanded', '节点被展开', '节点被展开', 'notify'), event('Collapsed', '节点被折叠', '节点被折叠', 'notify'), event('DoubleClick', '被双击', '被双击', 'notify')] }),
  control({ type: 'TabControl', label: '选项卡', moduleId: 'lingbuilder.win32.common-controls', category: '容器', icon: 'PanelsTopLeft', nativeClass: 'SysTabControl32', nativeAdapter: 'tab', defaultProps: { content: '', width: 360, height: 240 }, properties: [{ key: 'tabs', label: '标签页', type: 'tabs', defaultValue: [{ id: 'page1', title: '标签页 1' }] }, number('selectedIndex', '当前页', 0, 0), text('imageListId', '图像列表 ID')], events: [event('SelectionChanged', '标签页被改变', '标签页被改变', 'notify')], isContainer: true }),
  ...
  control({ type: 'RichEdit', label: '富文本框', moduleId: 'lingbuilder.win32.common-controls', category: '输入', icon: 'TextCursorInput', nativeClass: 'RICHEDIT50W', nativeAdapter: 'richedit', defaultProps: { content: '富文本内容', width: 320, height: 180 }, properties: [bool('readOnly', '只读'), bool('wordWrap', '自动换行', true), bool('multiline', '多行', true), enumProp('scrollBars', '滚动条', 'vertical', ['none', 'horizontal', 'vertical', 'both']), text('rtfText', 'RTF 格式化文本')], events: [event('TextChanged', '内容被改变', '内容被改变', 'command'), event('SelectionChanged', '选择区被改变', '选择区被改变', 'notify'), ...COMMON_FOCUS_EVENTS], requiredLibraries: ['ole32.lib'] }),
