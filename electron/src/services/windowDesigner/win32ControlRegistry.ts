...
const enumProp = (key: string, label: string, defaultValue: string, values: string[], labels: Readonly<Record<string, string>>): Win32ControlPropertyDefinition => ({
  key, label, type: 'enum', defaultValue, options: values.map(value => option(value, labels[value] || value))
});
const event = (
  name: string,
  label: string,
...
const CEF3_CONTROL_EVENTS = CEF3_BROWSER_EVENTS.map(definition => event(
  definition.legacyDesignerId || definition.id,
  definition.name,
  definition.name,
  'notify'
));
const EDGEVIEW_CONTROL_EVENTS = EDGEVIEW_BROWSER_EVENTS.map(definition => event(
...
const COMMON_NATIVE_CONTROL_EVENTS = [...COMMON_MOUSE_EVENTS, ...COMMON_FOCUS_EVENTS];
// 颜色选择器的点击由选色流程接管，视频播放器的交互由 Media Foundation 子窗口承载；
// 两者只公开各自稳定的专用事件，避免给用户生成不会可靠触发的通用处理器。
const SPECIALIZED_EVENT_ONLY_CONTROL_TYPES = new Set(['ColorPicker', 'VideoPlayer']);
// 这些输出、媒体或容器窗口按当前 Win32 样式不会可靠取得键盘焦点，
// 因此不向事件面板暴露无法稳定触发的 GotFocus/LostFocus。
const NON_FOCUSABLE_NATIVE_CONTROL_TYPES = new Set([
  'Label',
  'Image',
  'AnimatedImage',
  'GroupBox',
...
export function supportsNativeControlFocusEvents(type: string): boolean {
  return !SPECIALIZED_EVENT_ONLY_CONTROL_TYPES.has(type) && !NON_FOCUSABLE_NATIVE_CONTROL_TYPES.has(type);
}
...
export const WIN32_CONTROL_DEFINITIONS: Win32ControlDefinition[] = [
  ...
  control({ type: 'ScrollBar', label: '滚动条', moduleId: 'lingbuilder.win32.basic', category: '输入', icon: 'MoveVertical', nativeClass: 'SCROLLBAR', nativeAdapter: 'scrollbar', defaultProps: { content: '', width: 20, height: 140 }, properties: [enumProp('orientation', '方向', 'vertical', ['horizontal', 'vertical'], { horizontal: '水平', vertical: '垂直' }), number('minimum', '最小值', 0), number('maximum', '最大值', 100), number('value', '当前值', 0)], events: [event('ValueChanged', '数值被改变', '数值被改变', 'scroll')] }),
  ...
  // 「时钟」非可视组件：WM_TIMER 周期计时，事件在界面线程触发，不冻结界面；周期 0=不计时。
  nonVisual({ type: 'Clock', label: '时钟', moduleId: 'lingbuilder.win32.basic', category: '非可视', icon: 'Clock', nativeClass: 'WM_TIMER', nativeAdapter: 'clock-resource', defaultProps: { content: '时钟', width: 0, height: 0 }, properties: [text('ownerWindowId', '所属窗口'), number('periodMilliseconds', '周期毫秒', 1000, 0), bool('startEnabled', '创建后自动计时')], events: [event('Elapsed', '周期到期', '周期到期', 'notify')] }),
  ...
  control({ type: 'FlatScrollBar', label: '平面滚动条', moduleId: 'lingbuilder.win32.common-controls', category: '输入', icon: 'MoveVertical', nativeClass: 'SCROLLBAR', nativeAdapter: 'flatscrollbar', defaultProps: { content: '', width: 20, height: 140 }, properties: [enumProp('orientation', '方向', 'vertical', ['horizontal', 'vertical'], { horizontal: '水平', vertical: '垂直' }), number('minimum', '最小值', 0), number('maximum', '最大值', 100), number('value', '当前值', 0)], events: [event('ValueChanged', '数值被改变', '数值被改变', 'scroll')] }),
  nonVisual({ type: 'PropertySheet', label: '属性页窗口', moduleId: 'lingbuilder.win32.common-controls', category: '非可视', icon: 'PanelTopOpen', nativeClass: '#32770', nativeAdapter: 'propertysheet', defaultProps: { content: '属性', width: 420, height: 320 }, properties: [text('title', '窗口标题', '属性'), { key: 'pages', label: '属性页', type: 'tabs', defaultValue: [] }], events: [event('Applied', '属性被应用', '属性被应用', 'notify')] }),
  control({
    type: 'EdgeBrowser', label: 'Edge浏览器', moduleId: 'lingbuilder.edgeview', category: '媒体', icon: 'Globe',
    nativeClass: 'STATIC', nativeAdapter: 'edgeview-browser',
    defaultProps: { content: 'Edge浏览器', width: 480, height: 320, background: '#FFFFFF' },
    properties: [
      text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '独立缓存目录'),
      enumProp('proxyMode', '代理模式', 'system', ['system', 'custom'], { system: '跟随系统', custom: '自定义' }), text('proxyServer', '自定义代理地址'),
      text('userAgent', 'User-Agent'), bool('enableScript', '启用脚本', true), bool('enableWebMessage', '启用网页消息', true),
      bool('enableDevTools', '启用开发者工具', true), bool('enableContextMenu', '启用右键菜单', true), bool('enableStatusBar', '启用状态栏', true),
      number('zoomFactor', '缩放百分比', 100, 25, 500), bool('muteAudio', '静音', false), color('defaultBackgroundColor', '网页背景色', '#FFFFFF'),
      bool('allowExternalDrop', '允许外部拖放', true), text('profileName', 'Profile 名称', '默认'), bool('inPrivate', '隐私模式', false),
      text('language', '浏览器语言', 'zh-CN'), enumProp('trackingPrevention', '跟踪保护', 'balanced', ['none', 'basic', 'balanced', 'strict'], { none: '关闭', basic: '基本', balanced: '平衡', strict: '严格' }),
      bool('enableAutofill', '启用自动填充', true), bool('enablePasswordAutosave', '启用密码保存', true),
      { ...bool('exclusiveUserDataFolderAccess', '独占用户数据目录'), description: '创建期选项，修改后需要重建控件。' },
      { ...bool('customCrashReporting', '自定义崩溃报告'), description: '创建期选项，修改后需要重建控件。' },
      { ...bool('environmentTrackingPrevention', '环境跟踪保护', true), description: 'Environment 创建期开关，修改后需要重建控件。' },
      { ...bool('browserExtensionsEnabled', '允许浏览器扩展'), description: '创建期选项，修改后需要重建控件。' },
      { ...enumProp('channelSearchKind', '通道搜索方式', 'stable-first', ['stable-first', 'least-stable-first'], { 'stable-first': '优先稳定通道', 'least-stable-first': '优先预览通道' }), description: '创建期选项，修改后需要重建控件。' },
      { ...number('releaseChannels', '发布通道掩码', 15, 0, 15), description: 'Stable/Beta/Dev/Canary 位掩码；修改后需要重建控件。' },
      { ...enumProp('scrollBarStyle', '滚动条样式', 'default', ['default', 'fluent-overlay'], { default: '默认', 'fluent-overlay': 'Fluent Overlay' }), description: '创建期选项，修改后需要重建控件。' },
      { ...text('scriptLocale', '脚本区域'), description: 'Controller 创建期脚本区域，修改后需要重建控件。' },
      { ...bool('allowHostInputProcessing', '允许宿主输入处理'), description: 'Controller 创建期选项，修改后需要重建控件。' },
      { ...bool('disableWebSecurity', '关闭跨域安全策略'), description: '创建期选项，修改后需要重建控件。' },
      { ...text('additionalBrowserArguments', '附加参数'), description: '浏览器启动参数，多个用空格分隔。 创建期选项，修改后需要重建控件。' }
    ],
    events: EDGEVIEW_CONTROL_EVENTS, requiredLibraries: ['ole32.lib']
  }),
  control({ type: 'CefBrowser', label: 'CEF3浏览器', moduleId: 'lingbuilder.cef3.browser', category: '媒体', icon: 'Globe', nativeClass: 'STATIC', nativeAdapter: 'cef3-browser', defaultProps: { content: 'CEF3浏览器', width: 480, height: 320, background: '#FFFFFF' }, properties: [text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '缓存目录', '.cef3/cache'), text('userAgent', 'User-Agent 标识'), bool('enableJs', '启用 JavaScript', true), bool('enableDevTools', '允许开发者工具', true), bool('loadImages', '加载图片', true), bool('enableWebGL', '启用 WebGL'), bool('muteAudio', '静音'), enumProp('proxyMode', '代理模式', 'system', ['system', 'none', 'custom'], { system: '跟随系统', none: '不使用代理', custom: '自定义' }), text('proxyServer', '自定义代理地址'), { ...text('jsQueryFunctions', 'JS交互查询函数'), description: '页面调用原生的查询通道（cefQuery），格式“查询函数名,取消函数名”，多条通道用 ; 分隔；留空不启用。必须在首次创建浏览器之前配置，「查询请求」事件的 channelIndex 标明来源通道。' }], events: CEF3_CONTROL_EVENTS, requiredLibraries: ['libcef.lib', 'libcef_dll_wrapper.lib'] }),
  control({ type: 'FBroBrowser', label: 'FBro指纹浏览器', moduleId: 'lingbuilder.fbro.browser', category: '媒体', icon: 'Fingerprint', nativeClass: 'STATIC', nativeAdapter: 'fbro-browser', defaultProps: { content: 'FBro指纹浏览器', width: 480, height: 320, background: '#FFFFFF' }, properties: [enumProp('processMode', '进程模式', 'in-process', ['in-process', 'independent-embedded', 'independent-window'], { 'in-process': '进程内', 'independent-embedded': '独立进程嵌入', 'independent-window': '独立进程窗口' }), text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '独立缓存目录'), text('userAgent', 'User-Agent 标识'), bool('enableJs', '启用 JavaScript', true), bool('enableDevTools', '启用开发者工具', true), bool('loadImages', '加载图片', true), bool('enableWebGL', '启用 WebGL', true), bool('muteAudio', '静音'), enumProp('proxyMode', '代理模式', 'system', ['system', 'none', 'custom'], { system: '跟随系统', none: '不使用代理', custom: '自定义' }), text('proxyServer', '自定义代理地址'), text('fingerprintProfile', '指纹配置 JSON'), { ...text('jsQueryFunctions', 'JS 交互函数'), description: '页面调用原生的 cefQuery 查询通道，格式“查询函数名,取消函数名”，分号分隔多条（如 cefQuery,cefQueryCancel;cefQuerytest,cefQueryCanceltest）；点「配置函数」用结构化弹窗编辑。生成程序时在浏览器初始化前自动注册，留空不启用。' }, { ...bool('disableGpu', '禁用 GPU'), description: '创建期启动开关，仅进程内模式生效。' }, { ...bool('disableGpuCache', '禁用 GPU 缓存'), description: '创建期启动开关，仅进程内模式生效。' }, { ...bool('disableGpuBlockList', '禁用 GPU 黑名单'), description: '创建期启动开关，仅进程内模式生效。' }, { ...bool('enableMediaStream', '启用媒体流'), description: '创建期启动开关，仅进程内模式生效。' }, { ...bool('enableSpeechInput', '启用语音输入'), description: '创建期启动开关，仅进程内模式生效。' }, { ...bool('enableAutoplay', '启用自动播放'), description: '创建期启动开关，仅进程内模式生效。' }, { ...bool('enableCrossFrame', '启用跨域模式'), description: '创建期启动开关（FBro 官方跨域/跨框架操作开关），仅进程内模式生效。用于跨域 iframe 填表与取元素；打开后同进程所有进程内浏览器都放开跨域限制，只在受控内网页面使用。' }, { ...bool('disableProxy', '禁用代理'), description: '创建期启动开关，忽略系统与命令行代理设置，仅进程内模式生效。' }], events: FBRO_CONTROL_EVENTS, requiredLibraries: ['LingBuilderFbroBridge.lib'] }),
  nonVisual({ type: 'FBroHeadlessBrowser', label: 'FBro无头浏览器', moduleId: 'lingbuilder.fbro.browser', category: '非可视', icon: 'Fingerprint', nativeClass: 'LB_FBRO_HANDLE', nativeAdapter: 'fbro-headless-resource', defaultProps: { content: 'FBro无头浏览器', width: 0, height: 0 }, properties: [text('ownerWindowId', '所属窗口'), text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '独立缓存目录'), text('extraInfoJson', '附加信息 JSON')], events: [], requiredLibraries: ['LingBuilderFbroBridge.lib'] }),
  nonVisual({ type: 'EdgeViewHeadlessBrowser', label: 'EdgeView无头浏览器', moduleId: 'lingbuilder.edgeview', category: '非可视', icon: 'Globe', nativeClass: 'EdgeView 实例编号', nativeAdapter: 'edgeview-headless-resource', defaultProps: { content: 'EdgeView无头浏览器', width: 0, height: 0 }, properties: [text('ownerWindowId', '所属窗口'), number('instanceId', '实例编号', 1, 1), text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '独立缓存目录'), text('userAgent', 'User-Agent'), text('proxyServer', '独立代理地址'), bool('autoStart', '窗口创建期自动启动', true)], events: [], requiredLibraries: ['ole32.lib'] }),
  nonVisual({ type: 'CefHeadlessBrowser', label: 'CEF3无头浏览器', moduleId: 'lingbuilder.cef3.browser', category: '非可视', icon: 'Globe', nativeClass: 'cef-windowless', nativeAdapter: 'cef-headless-resource', defaultProps: { content: 'CEF3无头浏览器', width: 0, height: 0 }, properties: [text('ownerWindowId', '所属窗口'), number('instanceId', '实例编号', 1, 1), text('url', '打开地址', 'https://www.baidu.com'), text('cacheDir', '独立缓存目录'), text('proxyServer', '独立代理地址'), number('viewWidth', '视口宽', 1280, 1), number('viewHeight', '视口高', 720, 1), bool('autoStart', '窗口创建期自动启动', true)], events: [], requiredLibraries: ['LingBuilderCefBridge.lib'] })
];
...
WIN32_CONTROL_DEFINITIONS.forEach(definition => {
  ...
  if (
    ...
    && (definition.moduleId === 'lingbuilder.win32.basic' || definition.moduleId === 'lingbuilder.win32.common-controls')
    && !SPECIALIZED_EVENT_ONLY_CONTROL_TYPES.has(definition.type)
  ) {
    const commonEvents = supportsNativeControlFocusEvents(definition.type)
      ? COMMON_NATIVE_CONTROL_EVENTS
      : COMMON_MOUSE_EVENTS;
    commonEvents.forEach(commonEvent => {
      if (!definition.events.some(existing => existing.name === commonEvent.name)) definition.events.push(commonEvent);
    });
  }
});

['Label', 'Image', 'AnimatedImage'].forEach(type => {
  const definition = WIN32_CONTROL_DEFINITIONS.find(item => item.type === type);
  if (definition && !definition.events.some(existing => existing.name === 'Click')) {
    definition.events.unshift(event('Click', '被单击', '被单击', 'command'));
  }
});
...
export interface Win32ConventionalControlEventBinding {
  eventName: string;
  handlerName: string;
}

/**
 * 源码常规命名「_控件名_事件中文后缀」的自动接线候选（命名规则与设计器事件面板一致）。
 * 生成器据此把未绑定但源码已定义的常规命名事件并入控件派发表；语言服务据此豁免
 * 「尚未在设计器中绑定」提示。两边必须消费同一出口，禁止各自推导。
 */
export function getConventionalControlEventBindings(
  controlType: string,
  controlName: string
): Win32ConventionalControlEventBinding[] {
  const definition = getWin32ControlDefinition(controlType);
  const normalizedControlName = controlName.trim().replace(/\s+/g, '');
  if (!definition || !normalizedControlName) return [];
  return (definition.events || []).map(definitionEvent => ({
    eventName: definitionEvent.name,
    handlerName: `_${normalizedControlName}_${definitionEvent.handlerSuffix}`
  }));
}
