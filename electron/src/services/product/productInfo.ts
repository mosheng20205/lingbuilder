...
export const LINGBUILDER_RELEASE_NOTES: readonly LingBuilderReleaseNote[] = [
  {
    version: '0.7.5',
    date: '2026-09-17',
    title: '内嵌资源模块、内存加载 DLL 与设计器内嵌资源面板',
    sections: [
      {
        title: '新增与改进',
        items: [
          '新增「内嵌资源模块」：把任意格式的项目文件（图片、文本、JSON、zip、字体、DLL 字节…）在构建期直接打进 EXE，运行期按逻辑名读取，程序分发只需一个 exe；提供取字节集、取文本、是否存在、取大小、列表、保存到文件、释放到临时目录与取错误信息共 8 条命令。',
          '窗口设计器属性面板新增「项目 / 内嵌资源」区域：可用「选择文件…」多选或「选择文件夹…」递归导入，文件会自动复制进项目源码根 resources 目录并回填逻辑名；也可以「扫描目录」直接引用工作区内已有文件；清单支持改逻辑名、改源文件、勾选「启动释放」与删除，未启用模块时面板顶部可一键启用。',
          '勾选「启动释放」的内嵌资源会在程序启动时自动释放到临时目录，方便只接受磁盘路径的第三方库；不勾选的资源全程不落盘。',
          '内存加载 DLL 全链路：新增「内存加载DLL模块」（加载、取函数地址、按序号取地址、取模块大小、是否已加载、卸载、取错误信息共 7 条命令），项目 DLL 命令声明支持「加载方式 = 内存」，DLL 以资源内嵌进 EXE 且不再生成导入库，exe 同目录不再出现该 DLL。',
          '内嵌资源的中文路径与中文文件名可以安全内嵌（构建期自动转成 ASCII 归档名），中文只出现在生成的资源清单里。'
        ]
      },
      {
        title: '问题修复',
        items: [
          '修复「窗口内嵌文件」在窗口图标选择「不显示」时被静默丢弃的问题：内嵌文件与图标开关已完全解耦，不再出现“声明了但构建后缺文件”。',
          '旧的「窗口内嵌文件」会自动迁移成新的项目内嵌资源（释放路径不变），构建日志给出中文提示，不再需要手工重写清单。',
          '修复项目 DLL 命令声明编辑器里修改任意一行命令的「库文件名」时，同一个库里其它命令的库文件名会跟着一起变化的问题（现在只影响被编辑的那一条）。',
          '修复该编辑器右键菜单「搜索 DLL 命令 / 展开所有 DLL 命令 / 折叠所有 DLL 命令」点击无反应的问题。'
        ]
      }
    ]
  },
  {
    version: '0.7.4',
    date: '2026-09-17',
    title: '内存加载 DLL（不落盘）与项目 DLL 编辑器修复',
    sections: [
      {
        title: '新增与改进',
        items: [
          '新增「内存加载DLL模块」：把 DLL 字节在内存中完成 PE 映射并调用导出函数，全过程不向磁盘释放 DLL 文件，适合把第三方 DLL 以单文件形式随程序分发；提供加载、取函数地址、按序号取地址、取模块大小、是否已加载、卸载与中文错误信息共 7 条命令。',
          '项目 DLL 命令声明新增「加载方式 = 内存」：DLL 以资源内嵌进 EXE，exe 同目录不再出现该 DLL、也不再生成导入库，中文命令的写法与普通声明完全一致；未启用模块、与「系统 DLL」混用、DLL 位数不符、加壳或超过数量/体积上限都会在构建前给出中文阻断提示。',
          '项目 DLL 命令声明编辑器新增「内存加载（内嵌进 EXE，不落盘）」开关与状态标记，并在顶部库名字段补充说明它与各命令「库文件名」的关系。'
        ]
      },
      {
        title: '问题修复',
        items: [
          '修复项目 DLL 命令声明编辑器里修改任意一行命令的「库文件名」时，同一个库里其它命令的库文件名会跟着一起变化的问题（现在只影响被编辑的那一条）。',
          '修复该编辑器右键菜单「搜索 DLL 命令 / 展开所有 DLL 命令 / 折叠所有 DLL 命令」点击无反应的问题。',
          '由命令行把命令移到新库时，架构 DLL 路径改为按库名生成，不再统一写成示例路径。',
          '内存加载的限制已在模块文档与智能助手规则中写明：手工映射的 DLL 不支持 MSVC C++ 异常（throw/catch），需要异常机制的 DLL 请继续使用默认的同目录加载。'
        ]
      }
    ]
  },
