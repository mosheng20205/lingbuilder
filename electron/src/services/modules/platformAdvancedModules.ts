...
const mail = createStandardModule({ id: 'lingbuilder.net.mail', name: 'SMTP邮件模块', category: '网络', description: '提供普通 SMTP 或局域网调试服务发送；当前版本不支持 TLS，遇到要求 STARTTLS 的服务会明确失败。', tags: ['邮件', 'SMTP'], commands: [
  ...
  command('SMTP_发送HTML邮件', [{ name: '主机', type: 'wideString', description: 'SMTP 服务器主机名或 IP。'}, { name: '端口', type: 'int', description: 'SMTP 端口，1 到 65535；本模块不启用 TLS。'}, { name: '用户名', type: 'wideString', description: '登录账号；空文本跳过认证。'}, { name: '密码', type: 'wideString', description: '登录密码。'}, { name: '发件人', type: 'wideString', description: '发件地址，不写尖括号。'}, { name: '收件人', type: 'wideString', description: '收件地址，不写尖括号。'}, { name: '主题', type: 'wideString', description: '邮件主题，按 UTF-8 Base64 编码字写入。'}, { name: 'HTML正文', type: 'wideString', description: '完整 HTML 文档或片段，按 UTF-8 传输。'}], 'bool', '发送正文为 HTML 格式的邮件（Content-Type: text/html），收件端以富文本呈现。'),
  command('SMTP_取错误', [], 'wideString', '返回最近 SMTP 错误或服务端响应。'),
  command('SMTP_发送附件邮件', [{ name: '主机', type: 'wideString', description: 'SMTP 服务器主机名或 IP。'}, { name: '端口', type: 'int', description: 'SMTP 端口，1 到 65535；本模块不启用 TLS。'}, { name: '用户名', type: 'wideString', description: '登录账号；空文本跳过认证。'}, { name: '密码', type: 'wideString', description: '登录密码。'}, { name: '发件人', type: 'wideString', description: '发件地址，不写尖括号。'}, { name: '收件人', type: 'wideString', description: '收件地址，不写尖括号。'}, { name: '主题', type: 'wideString', description: '邮件主题，按 UTF-8 Base64 编码字写入。'}, { name: '正文', type: 'wideString', description: '邮件正文；按 是否HTML 参数决定按纯文本或 HTML 解析。'}, { name: '是否HTML', type: 'bool', description: '真时正文按 text/html 传输，假时按 text/plain。'}, { name: '附件列表', type: 'array', description: '附件完整路径的文本型数组；每个文件按 Base64 编码为独立附件部分，路径中不能含有英文双引号。'}], 'bool', '发送带附件的邮件（multipart/mixed，正文可选纯文本或 HTML），全部附件上传成功才返回真。')
] });
