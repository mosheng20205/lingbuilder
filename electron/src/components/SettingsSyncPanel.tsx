...
export default function SettingsSyncPanel({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  const importBundle = () => { const filePath = window.prompt('输入工作区内设置同步包路径', '.lingbuilder/settings-sync.json'); if (!filePath) return; void run(async () => { const preview = (await call('/api/settings-sync/preview', { filePath })).preview; if (!window.confirm(`将导入 ${preview.scopes.join('、')} 设置，是否继续？`)) return; const value = await call('/api/settings-sync/import', { filePath, token: preview.token }); setMessage(`已同步：${value.result.scopes.join('、')}`); }); };
