...
它不替代业务层的数据迁移、访问控制、备份保留、密钥管理和恢复演练。数据库文件加密通过随附运行库的 `sqlite3_key` 导出提供（详见下文「加密数据库（SQLCipher 等多算法）」）；密码本身不能写入 SQL、日志或源码。
...
`SQLite_语句步进` 返回 `1` 表示得到一行，`0` 表示执行完成，`-1` 表示失败。只有最近一次步进返回 1 时才允许读取列的**值**，语句重置、完成或失败后读取会给出明确错误；`SQLite_取列数量` 与 `SQLite_取列名称` 属于结果集元数据，`SQLite_准备` 成功后即可读取，不要求先步进。列索引从 0 开始，参数索引从 1 开始。`SQLite_取列类型` 返回 `1=整数`、`2=小数`、`3=文本`、`4=字节集`、`5=NULL`，失败返回 0。

## 加密数据库（SQLCipher 等多算法）

2.1 起提供基于 `sqlite3_key` 的数据库加密；2.2 起加密算法可选。加密打开命令按 `SQLite_设置加密算法` 选定的算法开库（先执行对应 `PRAGMA cipher=…` 再注入密钥），默认 **SQLCipher**（SQLCipher 4 参数），因此：
...
- SQLCipher 3 兼容参数选择 `sqlcipher3`；老版 wxSQLite3/RC4 格式（部分旧系统使用）选择 `rc4`；另支持 `aes128`、`aes256`、`chacha20`。
...
| `SQLite_设置加密算法(算法)` | 2.2 新增：设置下一次加密打开使用的算法（进程级，对之后每次加密打开生效），支持 `sqlcipher`（默认）、`sqlcipher3`、`rc4`、`aes128`、`aes256`、`chacha20`；传空恢复默认 |
| `SQLite_探测加密算法(路径, 密码)` | 2.2 新增：只读依次尝试全部算法档位，返回能用该密码读出数据的算法名；全部失败返回空文本并记录中文错误 |
| `SQLite_打开加密库(路径, 密码)` | 兼容接口：按当前算法打开默认读写加密连接（不存在则创建），自动启用外键并设置 5 秒忙等待 |
| `SQLite_打开加密连接(路径, 密码, 模式, 忙等待毫秒)` | 打开独立 FULLMUTEX 加密连接，模式含义与 `SQLite_打开连接` 相同 |

打开旧格式加密库的推荐流程：

```text
文本型 算法 = SQLite_探测加密算法("旧系统.db", "密码")
如果 (算法 != "")
    SQLite_设置加密算法(算法)
    局部 SQLite连接 数据库 = SQLite_打开加密连接("旧系统.db", "密码", 2, 5000)
如果结束
```
...
- 打开现有加密库时会立即执行一次真实读取来校验密码；密码错误或文件不是当前算法的加密格式时返回失败，`SQLite_取错误()` 会包含 SQLite 原始说明（通常为“文件不是数据库”）与错误码，并提示可先用 `SQLite_探测加密算法` 识别算法。
...
| `SQLite_打开加密库(路径, 密码)` | 逻辑型 | 按当前算法打开默认加密连接 |
